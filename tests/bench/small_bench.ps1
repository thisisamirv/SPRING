...
$INPUT_DIR = Join-Path $ROOT_DIR "data\samples"
$OUTPUT_BASE = Join-Path $ROOT_DIR "out\tests\bench\small"
$LOG_DIR = Join-Path $OUTPUT_BASE "logs"
$OUTPUT_DIR = Join-Path $OUTPUT_BASE "runs"
$TEMP_ROOT_DIR = Join-Path $OUTPUT_BASE "temp"
...
function Initialize-Environment {
    New-Item -ItemType Directory -Path $INPUT_DIR, $LOG_DIR, $OUTPUT_DIR, $TEMP_ROOT_DIR -Force | Out-Null
...
function Invoke-SingleFileBenchmark($inputPath) {
    ...
    $VERIFY_DIR = (Join-Path $TEMP_ROOT_DIR "$INPUT_STEM.verify").Replace("/", "\")
    ...
    if (Test-Path $VERIFY_DIR) { Remove-Item $VERIFY_DIR -Recurse -Force }
    ...
    if ($INPUT_ABS.EndsWith(".gz")) {
        $baseline = Join-Path $VERIFY_DIR "baseline.fastq"
...
function Invoke-AssaySuite() {
    ...
    foreach ($s in $samples) {
        ...
        $temp_root = Join-Path $TEMP_ROOT_DIR "$stem.bench"
        $temp_auto = Join-Path $temp_root "auto"
        $temp_decomp = Join-Path $temp_root "decomp"
        $temp_dna = Join-Path $temp_root "dna"
        
        if (Test-Path $temp_root) { Remove-Item $temp_root -Recurse -Force }
        New-Item -ItemType Directory -Path $temp_auto, $temp_decomp, $temp_dna -Force | Out-Null
        ...
        $decomp_dir = Join-Path $temp_decomp "restored"
        ...
        for ($i = 0; $i -lt $files.Count; $i++) {
            ...
            $baseline = Join-Path $temp_decomp "baseline_$i.fastq"
