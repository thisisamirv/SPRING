...
- Changed the remaining compression-path side-stream spill points to stay in memory: preprocess now returns post-encode quality/ID/tail/ATAC buffers directly, long-read read-length/quality/read blocks are compressed from in-memory buffers instead of raw staging files, `reorder_compress_quality_id()` partitions reordered quality/ID batches in memory instead of using `.batch.*` scratch files, and ID compression now writes final `.bsc` output directly without temporary encoded intermediates.
