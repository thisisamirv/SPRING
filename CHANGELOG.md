...
* Added automatic multi-stage assay detection heuristic (`--assay auto`) that rapidly classifies sequencing chemistry and layout (RNA, ATAC, Methylation, DNA, ChIP, Single-cell variations) from the first 10,000 reads using a fast K-mer sketcher and FASTQ signatures.
* Added **unconditional ternary DNA encoding** for methylation assays (`--assay methyl`, `sc-methyl`): when global Cytosine frequency is low (<5%), SPRING2 switches to a highly optimized 3-symbol alphabet ({A, T, G} + N) for bit-packing the reference archive and singleton streams. This significantly improves compression ratios for bisulfite-converted data while maintaining strict losslessness via a sentinel-based escape for residual Cytosines.
