...
- Changed gzipped compression input handling to stream decompression directly into startup sampling, input pre-scan, and preprocessing without staging full plain FASTQ/FASTA temporary files on disk; large `.gz` inputs now flow through the normal chunked compression pipeline without requiring scratch space for a second uncompressed copy.
...
- Changed the encoder sequence/position aggregation path to keep per-thread contig sequence bytes and rebased positions in memory instead of spilling `read_seq.bin.*` and `read_pos.bin.*` shard files before the final sequence archive assembly; encoder now derives thread sequence lengths from in-memory metadata buffers and packs `read_seq.bin.bsc` directly from those buffers.
- Changed short-read sequence archive packing and unpacking to keep the monolithic packed sequence buffer in memory: encoder now compresses `read_seq.bin.bsc` directly from in-memory packed bytes instead of writing `read_seq.bin.packed`, and decompression now inflates the `.bsc` archive into memory before slicing per-thread packed chunks rather than round-tripping through that temporary file.
- Changed short-read post-encode quality/ID/tail/ATAC side-stream staging to stay in memory between preprocessing and reorder-compress: SPRING2 now captures raw `quality_*`, `id_*`, and `tail_*.bin` payloads plus compressed `atac_adapter_*.bin.bsc` streams into a `post_encode_side_stream_artifact`, deletes those temporary handoff files, and reorders/compresses the side streams from memory while still emitting the final archive-facing outputs.
- Changed preserve-order sc-RNA CB-prefix staging to stay in memory around the final `.bsc` streams: preprocessing now accumulates stripped `cb_prefix.dna` and `cb_prefix.qual` payloads in memory and writes `cb_prefix.dna.bsc` / `cb_prefix.qual.bsc` directly, while decompression restores those prefixes from in-memory decompressed bytes instead of round-tripping through raw temporary files.
- Changed short-read post-encode read-order handling to stay in memory: encoder now keeps `read_order_entries` only in `reordered_stream_artifact`, `reorder_compress_quality_id()` derives its reorder map from that in-memory order vector, `pe_encode()` rewrites paired-end order in place, and `reorder_compress_streams()` consumes the final in-memory order entries instead of reopening `read_order.bin` from the temp dir.
- Changed the remaining compression-path side-stream spill points to stay in memory: preprocess now returns post-encode quality/ID/tail/ATAC buffers directly, long-read read-length/quality/read blocks are compressed from in-memory buffers instead of raw staging files, `reorder_compress_quality_id()` partitions reordered quality/ID batches in memory instead of using `.batch.*` scratch files, single-end ID block merges now build the final monolithic `id_1.bsc` directly from in-memory packed block payloads instead of rereading per-block files from disk, and ID compression now writes final `.bsc` output directly without temporary encoded intermediates.
- Changed the short-read preprocess-to-reorder-to-encoder handoff to keep clean and `N` read streams in memory: preprocessing now returns encoded clean-read buffers plus merged `N`-read payload/order buffers in a `preprocess_artifact`, reorder consumes those buffers directly instead of reopening `input_clean_*`, `input_N*`, and `read_order_N*`, encoder reads `N`-read singleton payloads and order from the in-memory `reorder_encoder_artifact`, and obsolete reorder-side materialization for that handoff is no longer written to the temp dir.
- Changed grouped bundle orchestration to build the outer grouped tar directly from the nested member archives plus an in-memory `bundle.meta` payload, removing the extra `bundle/` staging directory that previously wrote grouped member archives and manifest to disk only to read them back during final archive assembly.
- Changed grouped decompression to keep nested member archives in memory: grouped bundles now read `bundle.meta` and nested `.sp` members directly from the outer tar, untar nested members from in-memory bytes for decode, and avoid staging grouped subarchives on disk before reconstruction.
- Changed grouped preview and `SpringReader` setup to read nested member archives from in-memory tar bytes: preview now parses grouped member `cp.bin` streams without extracting the bundle, and `SpringReader` now resolves the primary reads member from the outer tar in memory before preparing its decode workspace.
- Changed the decompression contract to use archive-member artifacts instead of pre-extracted directory trees: standard decompression, grouped nested-member decode, audit, and `SpringReader` now load tar members into memory-backed file maps and decode from those artifacts directly, reducing temp-workspace usage to the few remaining helper paths that still require scratch files.
...
- Fixed grouped `spring2 -p -a/--audit` handling so grouped bundles now execute the real archive audit path instead of a metadata-only preview; grouped preview output now also preserves and displays the archive note, and grouped decompression once again accepts all five explicit output paths (`R1`, `R2`, `R3`, `I1`, `I2`).
- Fixed audit robustness for truncated metadata by rejecting archives whose `cp.bin` parameter stream cannot be fully read, instead of continuing with partially parsed compression parameters.
