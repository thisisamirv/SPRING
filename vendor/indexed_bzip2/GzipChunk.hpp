...
namespace rapidgzip {
template <typename T_ChunkData> class GzipChunk final {
public:
  ...
  static void startNewSubchunk(std::vector<Subchunk> &subchunks,
                               const size_t encodedOffset) {
    const auto nextDecodedOffset =
        subchunks.empty()
            ? 0
  ...
  determineUsedWindowSymbolsForLastSubchunk(std::vector<Subchunk> &subchunks,
    ...
    if (std::all_of(usedWindowSymbols.begin(), usedWindowSymbols.end(),
                    [](bool p) { return !p; })) {
  ...
  static void finalizeWindowForLastSubchunk(ChunkData &chunk,
    ...
    const auto nextWindowOffset =
        subchunks.back().decodedOffset + subchunks.back().decodedSize;
    for (auto footer = chunk.footers.rbegin(); footer != chunk.footers.rend();
         ++footer) {
  ...
  static void finalizeChunk(ChunkData &chunk, std::vector<Subchunk> &&subchunks,
                            gzip::BitReader &bitReader,
                            const size_t nextBlockOffset) {

    subchunks.back().encodedSize =
        nextBlockOffset - subchunks.back().encodedOffset;
    if ((subchunks.size() >= 2) &&
  ...
  static void appendDeflateBlockBoundary(ChunkData &chunk,
                                         std::vector<Subchunk> &subchunks,
                                         gzip::BitReader &bitReader,
                                         const size_t encodedOffset,
    ...
    if (subchunks.back().decodedSize >= chunk.configuration.splitChunkSize) {
      subchunks.back().encodedSize =
          encodedOffset - subchunks.back().encodedOffset;
      finalizeWindowForLastSubchunk(chunk, subchunks, bitReader);
  ...
  [[nodiscard]] static ChunkData decodeChunkWithInflateWrapper(
      UniqueFileReader &&sharedFileReader, size_t const encodedOffsetInBits,
      size_t const exactUntilOffset, WindowView const initialWindow,
    ...
    while (true) {
      const auto suggestedDecodeSize =
          decodedSize.value_or(ALLOCATION_CHUNK_SIZE);
      deflate::DecodedVector subchunk(
          suggestedDecodeSize > alreadyDecoded
              ? std::min(ALLOCATION_CHUNK_SIZE,
                         suggestedDecodeSize - alreadyDecoded)
      ...
      for (; (nBytesRead < subchunk.size()) && !footer;
           nBytesRead += nBytesReadPerCall) {
        std::tie(nBytesReadPerCall, footer) = inflateWrapper.readStream(
            subchunk.data() + nBytesRead, subchunk.size() - nBytesRead);
    ...
    if (exactUntilOffset != inflateWrapper.tellCompressed()) {
      ...
      message << "The inflate wrapper offset ("
              << inflateWrapper.tellCompressed() << ") "
              << "does not match the requested exact stop offset: "
              << exactUntilOffset << ". "
              << "The archive or the index may be corrupted or the stop "
                 "condition might contain a bug. "
  ...
  template <typename InflateWrapper>
  [[nodiscard]] static ChunkData finishDecodeChunkWithInexactOffset(
      gzip::BitReader *const bitReader, size_t const untilOffset,
      WindowView const initialWindow, size_t const maxDecompressedChunkSize,
      ChunkData &&result, std::vector<Subchunk> &&subchunks) {
    ...
    if ((alreadyDecoded > 0) && !bitReader->eof()) {
      appendDeflateBlockBoundary(result, subchunks, *bitReader, nextBlockOffset,
                                 alreadyDecoded);
    }
    ...
    inflateWrapper.setFileType(result.configuration.fileType);
    inflateWrapper.setWindow(initialWindow);
    inflateWrapper.setStoppingPoints(static_cast<StoppingPoint>(
        StoppingPoint::END_OF_BLOCK | StoppingPoint::END_OF_BLOCK_HEADER |
    ...
    while (!stoppingPointReached) {
      ...
      while ((nBytesRead < buffer.size()) && !footer && !stoppingPointReached) {
        std::tie(nBytesReadPerCall, footer) = inflateWrapper.readStream(
            buffer.data() + nBytesRead, buffer.size() - nBytesRead);
        nBytesRead += nBytesReadPerCall;
        ...
        case StoppingPoint::END_OF_BLOCK_HEADER:
          if (((nextBlockOffset >= untilOffset) &&
               ...
               (inflateWrapper.compressionType() !=
                deflate::CompressionType::FIXED_HUFFMAN)) ||
        ...
        default:
          throw std::logic_error(
              "Got stopping point of a type that was not requested!");
        }
  ...
  decodeChunkWithRapidgzip(gzip::BitReader *const bitReader,
    ...
    const auto maxDecompressedChunkSize =
    ...
    while (true) {
      if (isAtStreamEnd) {
        ...
        case FileType::NONE:
        case FileType::BZIP2:
          throw std::logic_error("[GzipChunkFetcher::decodeChunkWithRapidgzip] "
                                 "Invalid file type!");
        case FileType::BGZF:
        case FileType::GZIP:
        ...
        if (error != Error::NONE) {
          ...
          std::stringstream message;
          message << "Failed to read gzip/zlib header at offset "
                  << formatBits(headerOffset)
...
#ifdef LIBRAPIDARCHIVE_WITH_ISAL
        return finishDecodeChunkWithInexactOffset<IsalInflateWrapper>(
            bitReader, untilOffset, {}, maxDecompressedChunkSize,
            std::move(result), std::move(subchunks));
#endif

        didReadHeader = true;
        block->reset(VectorView<uint8_t>{});
      ...
      if (cleanDataCount >= deflate::MAX_WINDOW_SIZE) {
        return finishDecodeChunkWithInexactOffset<IsalInflateWrapper>(
            ...
            maxDecompressedChunkSize, std::move(result), std::move(subchunks));
      }
#endif
      ...
      if (auto error = block->readHeader(*bitReader); error != Error::NONE) {
        ...
        if ((error == Error::END_OF_FILE) &&
            (bitReader->tell() == result.encodedOffsetInBits)) {
          break;
        }

        std::stringstream message;
        message << "Failed to read deflate block header at offset "
                << formatBits(result.encodedOffsetInBits)
                << " (position after trying: " << formatBits(bitReader->tell())
                << ": " << toString(error);
        throw std::domain_error(std::move(message).str());
      }

      if (((nextBlockOffset >= untilOffset) && !block->isLastBlock() &&
      ...
      if (totalBytesRead > 0) {
        appendDeflateBlockBoundary(result, subchunks, *bitReader,
                                   nextBlockOffset, totalBytesRead);
      }

      size_t blockBytesRead{0};
      while (!block->eob()) {
        const auto [bufferViews, error] =
            block->read(*bitReader, std::numeric_limits<size_t>::max());
        if (error != Error::NONE) {
          std::stringstream message;
          message << "Failed to decode deflate block at "
                  << formatBits(result.encodedOffsetInBits)
        ...
        if (blockBytesRead > 256_Mi) {
          throw std::runtime_error("A single deflate block that decompresses "
                                   "to more than 256 MiB was "
                                   "encountered. This is not supported to "
      ...
      if (block->isLastBlock()) {
        ...
        case FileType::BZIP2:
          throw std::logic_error(
              "Cannot decode stream if the file type is not specified!");
        ...
        case FileType::GZIP: {
          ...
          if (didReadHeader) {
            if (streamBytesRead != footer.gzipFooter.uncompressedSize) {
              std::stringstream message;
              message << "Mismatching size (" << streamBytesRead
                      << " <-> footer: " << footer.gzipFooter.uncompressedSize
                      << ") for gzip stream!";
              throw std::runtime_error(std::move(message).str());
            }
          }
        ...
        footer.blockBoundary.encodedOffset = bitReader->tell();
        result.appendFooter(footer);
  ...
  [[nodiscard]] static ChunkData
  decodeChunk(UniqueFileReader &&sharedFileReader, size_t const blockOffset,
              size_t const untilOffset,
    ...
    if (initialWindow) {
      ...
      const auto window = initialWindow->decompress();
      return decodeChunkWithRapidgzip(&bitReader, untilOffset, *window,
                                      chunkDataConfiguration);
    }

    const auto tryToDecode = [&](const std::pair<size_t, size_t> &offset)
        -> std::optional<ChunkData> {
      try {

        bitReader.seekTo(offset.second);
        auto result = decodeChunkWithRapidgzip(
            &bitReader, untilOffset, std::nullopt, chunkDataConfiguration);
        result.encodedOffsetInBits = offset.first;
        result.maxEncodedOffsetInBits = offset.second;
        result.encodedSizeInBits =
            result.encodedEndOffsetInBits - result.encodedOffsetInBits;
    ...
    const auto findNextDynamic = [&](size_t beginOffset, size_t endOffset) {
      if (beginOffset >= endOffset) {
        return std::numeric_limits<size_t>::max();
      }
      bitReader.seekTo(beginOffset);
      return blockfinder::seekToNonFinalDynamicDeflateBlock(bitReader,
                                                            endOffset);
    };

    const auto findNextUncompressed = [&](size_t beginOffset,
                                          size_t endOffset) {
      if (beginOffset >= endOffset) {
      ...
      bitReader.seekTo(beginOffset);
      return blockfinder::seekToNonFinalUncompressedDeflateBlock(bitReader,
                                                                 endOffset);
    };
    ...
    for (auto chunkBegin = blockOffset; chunkBegin < untilOffset;
      ...
      const auto chunkEnd =
          std::min(static_cast<size_t>(chunkBegin + CHUNK_SIZE), untilOffset);
      ...
      while ((uncompressedOffsetRange.first < chunkEnd) ||
        ...
        if (dynamicHuffmanOffset < uncompressedOffsetRange.first) {
          ...
          dynamicHuffmanOffset =
              findNextDynamic(dynamicHuffmanOffset + 1, chunkEnd);
        } else {
          offsetToTest = uncompressedOffsetRange;
          uncompressedOffsetRange = findNextUncompressed(
              uncompressedOffsetRange.second + 1, chunkEnd);
        }

        const auto tBlockFinderStop = now();
        if (auto result = tryToDecode(offsetToTest); result) {
          result->statistics.blockFinderDuration =
              duration(tBlockFinderStart, tBlockFinderStop);
    ...
    std::stringstream message;
    message << "Failed to find any valid deflate block in ["
            << formatBits(blockOffset) << ", " << formatBits(untilOffset)
            << ")";
