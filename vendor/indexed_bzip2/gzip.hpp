...
[[nodiscard]] inline const char *toString(FileType fileType) {
  switch (fileType) {
  case FileType::NONE:
    return "None";
  case FileType::BGZF:
...
[[nodiscard]] inline bool hasCRC32(FileType fileType) {
  switch (fileType) {
  case FileType::NONE:
  case FileType::BZIP2:
...
[[nodiscard]] inline std::string getOperatingSystemName(uint8_t code) noexcept {
  switch (code) {
  case 0:
...
inline std::pair<Header, Error> readHeader(gzip::BitReader &bitReader) {
  Header header;
  ...
  try {
    ...
    header.modificationTime =
        static_cast<uint32_t>(bitReader.read<4 * BYTE_SIZE>());
...
inline Error checkHeader(gzip::BitReader &bitReader) {
  const auto readBytes = bitReader.read<3 * BYTE_SIZE>();
  if (readBytes != MAGIC_BYTES_GZIP) {
  ...
  const auto flags = bitReader.read<BYTE_SIZE>();
  bitReader.read<4 * BYTE_SIZE>();
  bitReader.read<BYTE_SIZE>();
  bitReader.read<BYTE_SIZE>();

  const auto skipZeroTerminatedString = [&bitReader]() -> Error {
    for (size_t i = 0; i < MAX_ALLOWED_FIELD_SIZE; ++i) {
      if (bitReader.eof()) {
...
inline Footer readFooter(gzip::BitReader &bitReader) {
...
readHeader(const std::function<uint64_t()> &readByte) {
  ...
  try {
    ...
    header.compressionLevel =
        static_cast<CompressionLevel>((flags >> 6U) & 0b11U);
  } catch (const gzip::BitReader::EndOfFileReached &) {
    return {header, readPartialHeader ? Error::INCOMPLETE_GZIP_HEADER
                                      : Error::END_OF_FILE};
  }
