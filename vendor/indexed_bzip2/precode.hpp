...
/**
 * @file Contains some compile-time computation of magic constants for the
 * precode huffman codings. This whole file is only used in tests, benchmarks,
 * and SingleLUT.hpp, which is now also only used in tests and benchmarks. I.e.,
 * this file should not increase normal compile-times or binary size.
 */
...
/**
 * This alternative version tries to reduce the number of instructions required
 * for creation so that it can be used with MSVC, which is the worst in
 * evaluating @ref createPrecodeFrequenciesValidLUT constexpr and runs into
 * internal compiler errors or out-of-heap-space errors.
 * This alternative implementation uses the fact only very few of the LUT
 * entries are actually valid, meaning we can reduce instructions by
 * initializing to invalid and then iterating only over the valid possibilities.
 * @param depth A depth of 1 means that we should iterate over 1-bit codes,
 * which can only be 0,1,2.
 * @param freeBits This can be calculated from the histogram but it saves
 * constexpr instructions when the caller updates this value outside.
 */
...
constexpr void iterateValidPrecodeHistograms(
  ...
  for (uint32_t count = 0; count <= std::min(remainingCount, freeBits);
    ...
    /* The first layer may not be fully filled or even empty. This does not fit
