...
template <typename T> struct Statistics {
  ...
  constexpr Statistics(const Statistics &other) = default;

  template <typename Container,
            typename = std::enable_if_t<
                !std::is_same_v<std::decay_t<Container>, Statistics<T>>, void>>
  constexpr explicit Statistics(const Container &values) noexcept {
  ...
  constexpr Statistics(const Iterator &begin, const Iterator &end) noexcept {
  ...
  [[nodiscard]] constexpr double standardDeviation() const noexcept {
    return std::sqrt(variance());
  }

  constexpr void merge(const Statistics &other) noexcept {
    if (other.count == 0) {
      return;
    }
  ...
  constexpr void merge(T value) noexcept {
  ...
  formatAverageWithUncertainty(bool includeBounds = false,
    ...
    const auto roundToUncertainty = [magnitude](auto value) {
    ...
    result << roundToUncertainty(average()) << " +- "
           << roundToUncertainty(uncertainty);
    if (includeBounds) {
...
public:
  template <typename Container>
  explicit Histogram(const Container &container, uint16_t binCount,
                     std::string unit = {})
      : m_statistics(container), m_bins(binCount, 0), m_unit(std::move(unit)) {
  ...
  Histogram(T min, T max, uint16_t binCount, std::string unit = {})
      : m_statistics(std::array<T, 2>{min, max}), m_bins(binCount, 0),
  ...
  constexpr bool merge(T value) {
    ...
    if ((value < m_statistics.min) || (value > m_statistics.max) ||
        m_bins.empty()) {
      return false;
    }

    const auto unitValue =
        static_cast<double>(value - m_statistics.min) /
  ...
  [[nodiscard]] constexpr const auto &statistics() const noexcept {
    return m_statistics;
  }

  [[nodiscard]] constexpr double binStart(size_t binNumber) const noexcept {
    return m_statistics.min +
           static_cast<double>(m_statistics.max - m_statistics.min) /
               static_cast<double>(m_bins.size()) *
               static_cast<double>(binNumber);
  }

  [[nodiscard]] constexpr double binCenter(size_t binNumber) const noexcept {
    return m_statistics.min +
           static_cast<double>(m_statistics.max - m_statistics.min) /
               static_cast<double>(m_bins.size()) *
               (static_cast<double>(binNumber) + 0.5);
  }

  [[nodiscard]] constexpr double binEnd(size_t binNumber) const noexcept {
    return m_statistics.min +
           static_cast<double>(m_statistics.max - m_statistics.min) /
               static_cast<double>(m_bins.size()) *
               (static_cast<double>(binNumber) + 1);
  }

  [[nodiscard]] constexpr const auto &bins() const noexcept { return m_bins; }

  [[nodiscard]] std::string plot() const {
    if (m_bins.empty()) {
      return {};
    }
    ...
    for (size_t i = 0; i < m_bins.size(); ++i) {
      ...
      histogramBar << std::setw(m_barWidth) << std::left
                   << std::string(binVisualSize, '=');

      const auto countLabel =
          bin > 0 ? "(" + std::to_string(bin) + ")" : std::string();

      result << binLabel.str() << " |" << histogramBar.str() << " "
             << countLabel << "\n";
    }
...
private:
  [[nodiscard]] std::string formatLabel(double value) const {
    std::stringstream result;
