...
#include "../cloudflare_zlib/zlib.h"
...
toString(const CompressionStrategy compressionStrategy) {
  ...
  case CompressionStrategy::FILTERED:
    return "Filtered"sv;
  case CompressionStrategy::RUN_LENGTH_ENCODING:
    return "Run-Length Encoding"sv;
  case CompressionStrategy::HUFFMAN_ONLY:
    return "Huffman Only"sv;
  case CompressionStrategy::FIXED_HUFFMAN:
    return "Fixed Huffman"sv;
  }
  return {};
}
...
[[nodiscard]] ResultContainer compressWithZlib(
    const VectorView<uint8_t> toCompress,
  ...
  stream.next_in =
      const_cast<Bytef *>(reinterpret_cast<const Bytef *>(toCompress.data()));
  stream.avail_out = 0;
  ...
  deflateInit2(&stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, windowBits, 8,
               static_cast<int>(compressionStrategy));

  if (!dictionary.empty()) {
    deflateSetDictionary(
        &stream,
        const_cast<Bytef *>(reinterpret_cast<const Bytef *>(dictionary.data())),
        dictionary.size());
  }
  ...
  while (status == Z_OK) {
    ...
    stream.next_out =
        reinterpret_cast<Bytef *>(output.data() + output.size() - CHUNK_SIZE);
...
public:
  ...
  ~ZlibInflateWrapper() { inflateEnd(&m_stream); }
...
private:
  [[nodiscard]] size_t getUnusedBits() const {
    ...
    return m_stream.avail_in * BYTE_SIZE +
           (static_cast<uint32_t>(m_stream.data_type) & 0b11'1111U);
  }
  ...
  Footer readFooter() {
    ...
    throw std::logic_error(
        "[ZlibInflateWrapper::readFooter] Invalid file type!");
  }
...
private:
  ...
  /* 2^15 = 32 KiB window buffer and minus signaling raw deflate stream to
   * decode. n in [8,15] -n for raw inflate, not looking for zlib/gzip header
   * and not generating a check value! n + 16 for gzip decoding but not zlib n +
   * 32 for gzip or zlib decoding with automatic detection 0 for automatic
   * window size detection based on the zlib header. We set it to -15 to always
   * force raw deflate decoding so that we can decode the header and footer
   * ourselves. */
...
inline void ZlibInflateWrapper::initStream() {
  m_stream = {};
...
inline void ZlibInflateWrapper::refillBuffer() {
  if ((m_stream.avail_in > 0) || (m_bitReader.tell() >= m_encodedUntilOffset)) {
    return;
  }

  if (m_bitReader.tell() % BYTE_SIZE != 0) {
    /* This might be called at the very first refillBuffer call when it does not
     * start on a byte-boundary. */
    const auto nBitsToPrime = BYTE_SIZE - (m_bitReader.tell() % BYTE_SIZE);
    if (inflatePrime(&m_stream, int(nBitsToPrime),
                     int(m_bitReader.read(nBitsToPrime))) != Z_OK) {
    ...
    /* This might be called at the very last refillBuffer call, when it does not
     * end on a byte-boundary. */
    if (inflatePrime(&m_stream, int(remainingBits),
                     int(m_bitReader.read(remainingBits))) != Z_OK) {
      throw std::runtime_error("InflatePrime failed!");
    }
    return;
  }
...
ZlibInflateWrapper::readStream(uint8_t *const output, size_t const outputSize) {
  ...
  /* Do not check for avail_out == 0 here so that progress can still be made on
   * empty blocks as might appear in pigz files or at the end of BGZF files.
   * Note that zlib's inflate should return Z_BUF_ERROR anyway if the output
   * buffer is full. It might be that the check was only here to idiomatically
   * avoid a "while ( true )". */
  while (true) {
    refillBuffer();
...
std::array<std::byte, SIZE> ZlibInflateWrapper::readBytes() {
...
inline Footer ZlibInflateWrapper::readGzipFooter() {
...
inline Footer ZlibInflateWrapper::readZlibFooter() {
...
inline int getZlibWindowBits(FileType fileType, int windowSize) {
  switch (fileType) {
  case FileType::NONE:
  case FileType::BZIP2:
    break;
...
inline void ZlibInflateWrapper::readHeader() {
  ...
  case FileType::BZIP2:
    throw std::logic_error(
        "[ZlibInflateWrapper::readHeader] Invalid file type!");
  case FileType::DEFLATE:
    break;
  case FileType::BGZF:
  case FileType::GZIP: {

    if (inflateReset2(&m_stream, getZlibWindowBits(m_fileType, 15)) != Z_OK) {
    ...
    if (getHeaderError != Z_OK) {
      std::stringstream message;
      message << "Failed to initialize gzip header structure (error: "
              << getHeaderError << "). Inconsistent zlib stream object?";
    ...
    if (m_stream.next_out != oldNextOut) {
      throw std::logic_error("Zlib wrote some output even though we only "
                             "wanted to read the gzip header!");
    }
    break;
  }
  case FileType::ZLIB: {
