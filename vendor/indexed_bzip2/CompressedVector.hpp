...
compress(const VectorView<typename Container::value_type> toCompress,
  ...
  case CompressionType::GZIP:
    ...
    try {
      ...
      message << "[Warning] Compression with ISA-L failed unexpectedly with: "
              << exception.what() << "\n";
      message << "[Warning] Will use zlib as a fallback. Please report this "
                 "bug anyway.\n";
  ...
  throw std::invalid_argument(
      std::string("Only gzip compression and none are currently supported") +
...
public:
  ...
  explicit CompressedVector(Container &&toCompress,
                            const CompressionType compressionType)
      : m_compressionType(compressionType),
        ...
        m_data(compressionType == CompressionType::NONE
                   ...
                   : std::make_shared<Container>(
                         compress<Container>(toCompress, compressionType))) {}

  explicit CompressedVector(
      const VectorView<typename Container::value_type> toCompress,
      const CompressionType compressionType)
      : m_compressionType(compressionType),
        m_decompressedSize(toCompress.size()),
  ...
  explicit CompressedVector(Container &&compressedData,
  ...
  [[nodiscard]] CompressionType compressionType() const noexcept {
  ...
  [[nodiscard]] size_t compressedSize() const noexcept {
  ...
  [[nodiscard]] size_t decompressedSize() const noexcept {
  ...
  [[nodiscard]] bool empty() const noexcept { return m_decompressedSize == 0; }

  [[nodiscard]] bool operator==(const CompressedVector &other) const {
