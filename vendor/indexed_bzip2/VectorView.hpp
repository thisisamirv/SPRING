...
public:
  ...
  [[nodiscard]] constexpr T front() const noexcept { return *m_data; }
...
template <typename T, size_t T_size> class ArrayView {
public:
  using value_type = T;

public:
  explicit constexpr ArrayView(const std::array<T, T_size> &array) noexcept
      : m_data(array.data()) {}
...
template <typename T, size_t T_size> class WeakArray {
public:
  using value_type = T;

public:
  constexpr explicit WeakArray(T *data) noexcept : m_data(data) {}
