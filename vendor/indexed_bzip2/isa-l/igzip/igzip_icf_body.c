...
extern uint64_t gen_icf_map_lh1(struct isal_zstream *, struct deflate_icf *,
                                uint64_t);
extern void set_long_icf_fg(uint8_t *, uint64_t, uint64_t,
                            struct deflate_icf *);
...
void set_long_icf_fg_base(uint8_t *next_in, uint64_t processed,
                          uint64_t input_size,
    ...
    if (len >= 8 + LEN_OFFSET) {
      ...
      while (match_len > match_lookup->lit_len &&
        ...
        write_deflate_icf(match_lookup,
                          match_len > LEN_MAX ? LEN_MAX : match_len, dist_code,
                          dist_extra);
        match_lookup++;
        next_in++;
        match_len--;
...
uint64_t gen_icf_map_h1_base(struct isal_zstream *stream,
  ...
  uint8_t *file_start =
      (uint8_t *)((uintptr_t)stream->next_in - stream->total_in);
  uint32_t hash;
...
void icf_body_hash1_fillgreedy_lazy(struct isal_zstream *stream) {
  ...
  matches_next_icf =
      compress_icf_map_g(stream, matches_next_icf, matches_end_icf);

  while (matches_next_icf >= matches_end_icf) {
    input_size = MATCH_BUF_SIZE;
    input_size =
        (input_size > stream->avail_in) ? stream->avail_in : input_size;
