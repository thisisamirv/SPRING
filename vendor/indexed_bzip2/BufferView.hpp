...
namespace rapidgzip {
class BufferViewFileReader : public FileReader {
public:
  explicit BufferViewFileReader(const void *const buffer, const size_t size)
      : m_buffer(reinterpret_cast<const std::byte *>(buffer)), m_size(size) {}

  explicit BufferViewFileReader(const std::vector<char> &buffer)
      : m_buffer(reinterpret_cast<const std::byte *>(buffer.data())),
        m_size(buffer.size()) {}

  explicit BufferViewFileReader(const std::vector<unsigned char> &buffer)
      : m_buffer(reinterpret_cast<const std::byte *>(buffer.data())),
        m_size(buffer.size()) {}

  explicit BufferViewFileReader(const std::vector<std::byte> &buffer)
      : m_buffer(buffer.data()), m_size(buffer.size()) {}
  ...
  [[nodiscard]] bool eof() const override { return m_bufferPosition >= m_size; }
  ...
  [[nodiscard]] size_t read(char *buffer, size_t nMaxBytesToRead) override {
    ...
    const auto nBytesToReadFromBuffer =
        std::min(m_size - m_bufferPosition, nMaxBytesToRead);
  ...
  [[nodiscard]] std::optional<size_t> size() const override { return m_size; }

  [[nodiscard]] size_t tell() const override { return m_bufferPosition; }
