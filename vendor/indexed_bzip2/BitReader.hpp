 ...
 *       This might need another abstraction layer which keeps chunks of data of
 * the file until it has been read once! Normally, the BZ2 reader should indeed
 * read everything once, meaning nothing should "leak". That abstraction layer
 * would also open the file only a single time and then hold a mutex locked
 * shared_ptr to it. This bit reader, returns bits in the order appropriate for
 * bzip2, i.e., it goes over all bytes in order and then returns the bits of
 * each byte >starting from the most significant<. This is contrary to the usual
 * bit numbering starting from the least-significant one and also contrary to to
 * DEFLATE (RFC 1951)!
...
public:
  ...
  /**
   * If it is too large, then the use case of only reading one Bzip2 block per
   * opened BitReader will load much more data than necessary because of the too
   * large buffer. The size should also be a multiple of the block size of the
   * underlying device. Any power of 2 larger than 4096 (4k blocks) should be
   * safe bet. 4K is too few, and will lead to a 2x slowdown in some test
   * because of the frequent buffer refills.
   */
  ...
  /**
   * This exception is thrown by refillBitBuffer when the I/O buffer has been
   * emptied. Because the I/O buffer is rather large compared to the bit buffer,
   * it only empties infrequently. Using an exception instead of conditionals
   * **doubled** performance in a synthetic BitReader benchmark and improved the
   * gzip decoder performance by ~20%!
   */
  class BufferNeedsToBeRefilled : public BitReaderException {};

  /**
   * An exception that may be returned by @ref BitReader::peek. An exception is
   * used instead of std::optional because this only happens once for the whole
   * input stream, so very rarely, and it is expensive to still check the
   * optional each time when it isn't needed 99.999% of the time! A try-catch
   * block instead can act as a kind of zero-cost conditional in case nothing is
   * thrown and is sufficiently cheap for the very rare case that an exception
   * has to be thrown.
   */
...
public:
  explicit BitReader(UniqueFileReader fileReader,
      ...
      : /* The UniqueFileReader input argument sufficiently conveys and ensures
         * that the file ownership is taken. But, because BitReader has a fileno
         * getter returning the underlying fileno, it is possible that the file
         * position is changed from the outside. To still keep correct behavior
         * in that case, we have to to make it a SharedFileReader, which keeps
         * track of the intended file position. */
...
public:
  ...
  /**
   * @note This function is not as cheap as one might thing because it
   * dynamically tests for EOF using the position and size instead of checking
   * an internal (cached) flag!
   */
  ...
  /**
   * Forcing to inline this function is superimportant because it depends even
   * between gcc versions, whether it is actually inlined or not but inlining
   * can save 30%! Note that splitting part of this method off into readSafe
   * made the compiler actually inline this now small function and thereby sped
   * up runtimes significantly!
   */
  forceinline BitBuffer read(bit_count_t bitsWanted) {
    /* Handling bitsWanted == 0 here would incur a 75% slowdown for the
     * benchmark reading single bits! Just let the caller handle that case.
     * Performance comes first, especially at this steep price for safety. */
    assert(bitsWanted > 0);
    /* Reading the whole buffer is not allowed because it would require another
     * expensive rarely used branch! */
...
private:
  BitBuffer read2(bit_count_t bitsWanted) {
    ...
    /* Read out the remaining bits from m_bitBuffer to the lowest bits in @ref
     * bits. This is copy-pasted from @ref peekUnsafe because for MSB, we don't
     * have to do the expensive bitBufferSize() == 0 branching! */
    ...
    /* If the system endianness matches the BitReader endianness and the byte
     * buffer contains enough bytes for the requested number of bits, then
     * refill the whole bit buffer with one unaligned memory read. This makes
     * the assumption that read2 is only ever called when all the current bit
     * buffer bits are not enough. */
    ...
    try {
      ...
      try {
        ...
        /* When fillBitBuffer does not throw, then it has been filled almost
         * completely and it is ensured that we have enough bits as long as
         * fewer than the bit buffer size were requested. Removing this if from
         * the non-throwing frequent path, improves performance measurably! */
...
public:
  /**
   * This is a performant unchecked helper to seek forward the same amount that
   * has already been peeked. Calling this function without calling peek
   * beforehand with the same number of bits may corrupt the BitReader!
   */
  ...
  [[nodiscard]] size_t read(char *outputBuffer, size_t nBytesToRead) override {
    ...
    if (UNLIKELY(outputBuffer == nullptr)) [[unlikely]] {
       ...
       *    b) Refill byte buffer and read from it to avoid many small calls to
       * FileReader::read. */
      ...
      if (UNLIKELY((nBytesToReadFromFile > 0) && m_file)) [[unlikely]] {
        ...
        if (nBytesToRead < std::min<size_t>(1_Ki, m_bufferRefillSize)) {
          /* Because nBytesToRead < m_bufferRefillSize, refilling the buffer
           * once will suffice to read the requested amount of bytes or else we
           * have reached EOF. */
          refillBuffer();
          readFromBuffer(outputBuffer + nBytesRead, nBytesToRead - nBytesRead);
        } else {
          if ((nBytesToReadFromFile > 0) && m_file) {
            ...
            /* Clear the byte buffer because the assumed invariant that the byte
             * buffer contains the data of m_file->tell() - m_inputBuffer.size()
             * is wrong after reading from the file and will result in a bug
             * when seeking back and when the supposed offset is thought to be
             * inside the buffer. This can be triggered now after adding sparse
             * windows because InflateWrapper will refill the buffer with this
             * method and getUsedWindowSymbols will seek back. Before sparse
             * windows, it might have been virtually impossible to trigger this.
             * @todo This understanding gives two optimization ideas:
             *       1. For InflateWrapper + getUsedWindowSymbols, reduce the
             * byte buffer size to something
             *          <= 32 KiB instead of 128 KiB to avoid unnecessarily
             * large refills.
             *       2. Check that InflateWrapper only calls this function with
             * byte-aligned offsets so that it doesn't trigger the possibly
             * slower other path that has to shift everything by a constant
             * amount of bits. */
            m_inputBufferPosition = 0;
            m_inputBuffer.clear();
          }
    ...
    if (UNLIKELY(bitsWanted > bitBufferSize())) [[unlikely]] {
      if constexpr (!MOST_SIGNIFICANT_BITS_FIRST &&
        ...
        if (LIKELY(m_inputBufferPosition + sizeof(BitBuffer) <
          ...
          /* There is no way around this special case because of the damn
           * undefined behavior when shifting! */
          if (bitBufferSize() == 0) {
          ...
          /* Load new bytes directly to the left of the (virtually) shrunk bit
           * buffer. This is possibly because read but still "loaded"
           * (m_originalBitBufferSize) bits are to the right. */
      ...
      try {
        /* In the case of the shortcut for filling the bit buffer by reading
         * 64-bit, don't inline the very rarely used fallback to keep this
         * function rather small for inlining. */
        if constexpr (!MOST_SIGNIFICANT_BITS_FIRST &&
                      (ENDIAN != Endian::UNKNOWN)) {
          /* This point should only happen rarely, e.g., when the byte buffer
        ...
        try {
          ...
          /* When fillBitBuffer does not throw, then it has been filled almost
           * completely and it is ensured that we have enough bits as long as
           * fewer than the bit buffer size were requested. Removing this if
           * from the non-throwing frequent path, improves performance
           * measurably! */
          if (UNLIKELY(bitsWanted > bitBufferSize())) [[unlikely]] {
            throw EndOfFileReached();
          }
...
public:
  ...
  [[nodiscard]] size_t tell() const override {
    ...
    /* Subtract the unread bits that have already been "moved" from the byte
     * buffer to the bit buffer. */
...
private:
  ...
  /**
   * Decreases m_originalBitBufferSize by CHAR_BIT until it is as close to
   * bitBufferSize() as possible and clears all bits outside of
   * m_originalBitBufferSize.
   */
  ...
  void refillBitBuffer() {
    ...
    if (bitBufferSize() == 0) {
      ...
      if constexpr (!MOST_SIGNIFICANT_BITS_FIRST) {
        ...
        /* Always checking for m_originalBitBufferSize for this damn bit shift
         * would be too cost-prohibitive. It should never happen because in this
         * branch we know that bitBufferSize() > 0 and at any point in time
         * m_originalBitBufferSize >= bitBufferSize() should be true! Run unit
         * tests in debug mode to ensure that the assert won't be triggered. */
  ...
  void fillBitBuffer() {
    /* Using an ad-hoc destructor to emulate a 'finally' does not seem to hurt
     * performance when comparing it to manually copy-pasting the shift in all
     * outer catch clauses. */
    struct ShiftBackOnReturn {
      ...
      ~ShiftBackOnReturn() noexcept {
        /* Move LSB bits (which are filled left-to-right) to the left if so
         * necessary so that the format is the same as for MSB bits! */
        if constexpr (!MOST_SIGNIFICANT_BITS_FIRST) {
    ...
    /* Refill buffer one byte at a time to enforce endianness and avoid
     * unaligned access. */
      ...
      if constexpr (MOST_SIGNIFICANT_BITS_FIRST) {
        ...
        m_bitBuffer |=
            (static_cast<BitBuffer>(m_inputBuffer[m_inputBufferPosition++])
             << m_originalBitBufferSize);
        /**
         * Avoiding the single shift before and after the loop for LSB by
         * modifying how the bits are appended slows it down by ~10%, probably
         * because one additional shift per loop iteration is necessary:
         * @verbatim
         * m_bitBuffer <<= CHAR_BIT;
         * m_bitBuffer |= ( static_cast<BitBuffer>(
         * m_inputBuffer[m_inputBufferPosition++] )
  ...
  [[nodiscard]] forceinline BitBuffer peekUnsafe(bit_count_t bitsWanted) const {
    ...
    if constexpr (MOST_SIGNIFICANT_BITS_FIRST) {
      ...
      /* Always checking for bitBufferSize() for this damn bit shift would be
       * too cost-prohibitive. It should only happen when the caller tries to
       * read, e.g., 0 bits, in which case undefined behavior for the shift
       * result value does not matter. Run unit tests in debug mode to ensure
       * that the assert won't be triggered. */
...
private:
  ...
  size_t m_inputBufferPosition =
      0; /** stores current position of first valid byte in buffer */
...
public:
   ...
   *
   * m_bitBuffer stores the last read bits from m_inputBuffer on the >right
   * side< (if not fully filled). The bits are to be read from >left to right<
   * up to a maximum of m_bitBufferSize. This means that not the least
   * significant n bits are to be returned on read but the most significant.
   * E.g., for the following example requesting 3 bits should return 011, i.e.,
   * start from m_bitBufferSize and return the requested amount of bits to the
   * right of it. After that, decrement m_bitBufferSize.
   *
   * @verbatim
   *        result = 0b011
   *        bitsWanted = 3
   ...
   * +-------------------+
   *        ^   ^  ^
   *        |   |  m_bitBufferSize - bitsWanted = 5 = m_bitBufferSize after
   * reading bitsWanted |   m_bitBufferSize = 8 m_originalBitBufferSize = 11
   * @endverbatim
   *
   * For MOST_SIGNIFICANT_BITS_FIRST == false (gzip):
   *
   * Bit buffer stores the last read bits from m_inputBuffer on the >left side<
   * of m_bitBuffer (if not fully filled). Basically, this works in a mirrored
   * way of MOST_SIGNIFICANT_BITS_FIRST == true in order to be able to only
   * require one size and not an additional offset position for the bit buffer.
   * Because the bits are to be read from >right to left<, the left-alignment
   * makes the left-most bits always those valid the longest. I.e., we only have
   * to decrement m_bitBufferSize.
   *
   * @verbatim
   *   result = 0b111
   *   bitsWanted = 3
   ...
   * @endverbatim
   *
   * In both cases, the amount of bits wanted are extracted by shifting to the
   * right and and'ing with a bit mask.
   */
  BitBuffer m_bitBuffer = 0;

  /**
   * Performance consideration for the used type were done with taskset because
   * else the timings did vary more.
   * @verbatim
   * m rapidgzip && for i in $( seq 30 ); do
   *     taskset --cpu-list 5 src/tools/rapidgzip -P 1 -d -o /dev/null
   * 10xSRR22403185_2.fastq.gz done 2>&1 | tee output && uncertainValue $( sed
   * -nr 's|.* ([0-9.]+) MB/s|\1|p' output )
   * @endverbatim
   *
   * Results:
   * @verbatim
   ...
   * @endverbatim
   *
   * Note that the range of bit buffer size should [0,64], i.e., whether it is
   * 8-bit or 16-bit or 8-bit signed or anything else, it does not matter. None
   * of them represent the exact allowed range and are all larger. Therefore,
   * using anything else for performance reasons makes sense to me. Changing the
   * width of m_originalBitBufferSize to 32-bit does not help, it even worsens
   * the performance back to ~415 MB/s. Well, this is probably VERY
   * compiler-dependent. No idea what it is thinking.
   */
  // uint32_t m_bitBufferSize = 0;  // size of bitbuffer in bits
  /**
   * Same time measurements as for @ref m_bitBufferSize. Storing m_bitBufferFree
   * instead safes one subtractions on every peekUnsafe! It was always:
   * m_bitBuffer >> ( MAX_BIT_BUFFER_SIZE - m_bitBufferSize ) and not simply is:
   ...
   * @endverbatim
   * Note that this might slow down the BZ2 decoder, but I guess at this point
   * it isn't the main goal anymore and it is much slower anyway, so probably
   * the BitReader isn't the bottleneck. If it turns out to be, then it will be
   * hard to include both variants in the same class via constexpr ifs, it might
   * be necessary to split the BitReader class into two versions.
   */
  uint32_t m_bitBufferFree{MAX_BIT_BUFFER_SIZE};

  uint8_t m_originalBitBufferSize =
      0; // size of valid bitbuffer bits including already read ones
};
...
inline size_t BitReader<MOST_SIGNIFICANT_BITS_FIRST, BitBuffer>::seek(
  ...
  if (origin == SEEK_END) {
    ...
    if (!fileSize.has_value()) {
      ...
      /* Because we have seeked to get the full size, we have to force a full
       * seek to restore a valid file position for the next refillBuffer call.
       */
  ...
  /* Currently, buffer-only is not supported, use BufferedFileReader as a
   * memory-only file reader! */
  if (!m_file) {
    throw std::logic_error("File has already been closed!");
  }

  /* Performance optimizations for faster seeking inside the buffer to avoid
   * expensive refillBuffer calls. */
  if (positiveOffsetBits >= tell()) {
...
BitReader<MOST_SIGNIFICANT_BITS_FIRST, BitBuffer>::fullSeek(size_t offsetBits) {
  ...
  if (seekable()) {
    ...
    /* Note that the performance optimizations above allow to seek at exactly
     * the file end without an exception. Therefore, also allow that here for
     * consistency. */
    if ((m_file->eof() &&
