...
struct FileRange {
  ...
  [[nodiscard]] bool operator==(const FileRange &other) const noexcept {
    return (offset == other.offset) && (size == other.size) &&
           (offsetIsLine == other.offsetIsLine) &&
           (sizeIsLine == other.sizeIsLine);
  }
};

inline std::ostream &operator<<(std::ostream &out, const FileRange &range) {
  out << range.size << (range.sizeIsLine ? "L" : "") << "@" << range.offset
      << (range.offsetIsLine ? "L" : "");
  return out;
}

[[nodiscard]] inline const char *skipWhitespaces(const char *const first,
                                                 const char *const last) {
  constexpr std::string_view WHITESPACES{" \t"};
  const std::string_view view{first,
...
[[nodiscard]] inline const char *readNumber(const char *const first,
                                            const char *const last,
                                            size_t &value, bool &valueIsLine) {
  const auto result = std::from_chars(first, last, value);
  if ((result.ec != std::errc()) || (result.ptr == first)) {
    throw std::invalid_argument(
        "Failed to parse number at the start of the remaining expression: " +
  ...
  const std::string_view unitString{
      current, static_cast<size_t>(std::distance(current, last))};

  size_t longestMatch{0};
  size_t longestMatchFactor{1};
  bool longestMatchIsLine{false};
  for (const auto suffix :
       {std::string_view("B"), std::string_view("L"), std::string_view("")}) {
...
parseFileRanges(const std::string_view expression) {
   ...
   * @endverbatim
   * Any whitespace between these states is ignored. Empty sizes or offsets are
   * not allowed, i.e., SIZE_END may not skip to TUPLE_END and so on. I
   * introduced the state machine to avoid duplicate end-of-string checks.
   * Thanks to the state machine, we can simply enter the next loop iteration,
   * which checks for end-of-string, after reading each part or skipping over
  ...
  for (; current != expressionEnd;
       current = skipWhitespaces(current, expressionEnd)) {
    switch (state) {
    case State::TUPLE_END:
      ...
      range.size = 0;
      if (startsWith(
              std::string_view(current, static_cast<size_t>(std::distance(
                                            current, expressionEnd))),
              INFINITY_STRING)) {
        range.size = std::numeric_limits<size_t>::max();
        current += INFINITY_STRING.size();
      } else {
        current =
            readNumber(current, expressionEnd, range.size, range.sizeIsLine);
      }
    ...
    case State::SIZE_END:
      if (*current != OFFSET_PREFIX) {
        ...
        message << "Expected " << OFFSET_PREFIX << " after a size at position "
                << std::distance(expression.data(), current)
    ...
    case State::OFFSET_SEPARATOR_END:
      range.offset = 0;
      current =
          readNumber(current, expressionEnd, range.offset, range.offsetIsLine);
    ...
    case State::OFFSET_END:
      if (*current != SEPARATOR) {
        ...
        message << "Expected " << SEPARATOR
                << " after an size@offset tuple at position "
                << std::distance(expression.data(), current)
                << " in expression: " << expression;
        throw std::invalid_argument(std::move(message).str());
      }
  ...
  if ((state != State::TUPLE_END) && (state != State::OFFSET_END)) {
    throw std::invalid_argument(
        "Incomplete size@offset tuple at end of expression: " +
