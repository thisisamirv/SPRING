...
namespace rapidgzip {
[[nodiscard]] inline int getRequiredBitMaskSize() {
  ...
  while (einvalError) {
    ...
    if ((result != 0) && (errno != EINVAL)) {
      std::stringstream msg;
      msg << "An unexpected error occurred on schet_getaffinity: " << result
...
inline void pinThreadToLogicalCore(int logicalCoreId) {
  ...
  if (result != 0) {
    ...
    msg << "When trying to pin current thread running on logical core "
        << sched_getcpu() << " to " << logicalCoreId
        << ", sched_setaffinity returned " << result << " and errno " << errno
        << " (" << std::strerror(errno) << "). "
