 ...
 * checks whether there is a matching code, which is calculated ad-hoc from
 * m_minimumCodeValuesPerLevel. If the code matches, it gets the symbol for
 * current code length from the corresponding subarray. Note that reading the
 * bits one by one is also necessary to reverse the codes.
 */
...
protected:
  /**
   * Contains the alphabet, first sorted by code length, then by given alphabet
   * order. E.g., it could look like this:
   * @verbatim
   * +-------+-----+---+
   * | B D E | A F | C |
   * +-------+-----+---+
   ...
   * which might be redundant but fast. */
