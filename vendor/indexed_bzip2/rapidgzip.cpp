...
void printRapidgzipHelp(const cxxopts::Options &options) {
  std::cout << options.help() << "\n"
            << "If no file names are given, rapidgzip decompresses from "
               "standard input to standard output.\n"
            << "If the output is discarded by piping to /dev/null, then the "
               "actual decoding step might\n"
...
template <typename Reader> void printIndexAnalytics(const Reader &reader) {
  ...
  for (auto it = offsets.begin(), nit = std::next(offsets.begin());
    ...
    if (nextEncodedOffset - encodedOffset > 0) {
      encodedOffsetSpacings.merge(
          static_cast<double>(nextEncodedOffset - encodedOffset) / CHAR_BIT /
          1e6);
      decodedOffsetSpacings.merge(
          static_cast<double>(nextDecodedOffset - decodedOffset) / 1e6);
    }
  }

  std::cerr << "[Seekpoints Index]\n"
            << "    Encoded offset spacings: ( min: "
            << encodedOffsetSpacings.min << ", "
            << encodedOffsetSpacings.formatAverageWithUncertainty()
            << ", max: " << encodedOffsetSpacings.max << " ) MB\n"
            << "    Decoded offset spacings: ( min: "
            << decodedOffsetSpacings.min << ", "
  ...
  if (gzipIndex.windows) {
    ...
    const auto compressedWindowSize = std::accumulate(
        windows->begin(), windows->end(), size_t(0),
        [](size_t sum, const auto &kv) {
          return sum + (kv.second ? kv.second->compressedSize() : 0);
        });
    const auto decompressedWindowSize = std::accumulate(
        windows->begin(), windows->end(), size_t(0),
        [](size_t sum, const auto &kv) {
          return sum + (kv.second ? kv.second->decompressedSize() : 0);
        });
    std::cerr << "    Windows Count: " << windows->size() << "\n"
              << "    Total Compressed Window Size: "
              << formatBytes(compressedWindowSize) << "\n"
              << "    Total Decompressed Window Size: "
              << formatBytes(decompressedWindowSize) << "\n";
  }

  const auto &newlineOffsets = reader->newlineOffsets();
  if (!newlineOffsets.empty()) {
    /* The newline offsets in the index must be sorted! This is a sanity check
     * for debugging. */
    const auto lessLineOffset = [](const auto &a, const auto &b) {
      return a.lineOffset < b.lineOffset;
    };
    const auto sorted = std::is_sorted(newlineOffsets.begin(),
                                       newlineOffsets.end(), lessLineOffset);
    std::cerr << "    Newline Offsets Sorted: " << std::boolalpha << sorted
              << "\n";

    const auto maxLineOffset = std::accumulate(
        newlineOffsets.begin(), newlineOffsets.end(), uint64_t(0),
        [](auto a, auto offset) { return std::max(a, offset.lineOffset); });
...
template <typename ChunkData,
          typename ReadFunctor = std::function<size_t(
              std::unique_ptr<rapidgzip::ParallelGzipReader<ChunkData>> &)>>
[[nodiscard]] DecompressErrorCode
decompressParallel(const Arguments &args, UniqueFileReader inputFile,
                   const ReadFunctor &readFunctor) {
  using Reader = rapidgzip::ParallelGzipReader<ChunkData>;
  auto reader = std::make_unique<Reader>(
      std::move(inputFile), args.decoderParallelism, args.chunkSize);

  const auto gatherLineOffsets =
      args.gatherLineOffsets ||
      (!args.indexSavePath.empty() &&
  ...
  reader->setKeepIndex(!args.indexSavePath.empty() ||
                       !args.indexLoadPath.empty() || args.keepIndex);
  reader->setWindowSparsity(args.windowSparsity);
  reader->setNewlineCharacter(gatherLineOffsets ? std::make_optional('\n')
                                                : std::nullopt);
  if ((args.indexFormat == IndexFormat::GZTOOL) ||
      (args.indexFormat == IndexFormat::GZTOOL_WITH_LINES)) {
    /* Compress with zlib instead of gzip to avoid recompressions when exporting
     * the index. */
  ...
  if (!args.indexLoadPath.empty()) {
    reader->importIndex(
        std::make_unique<StandardFileReader>(args.indexLoadPath));

    if (args.verbose &&
        (!args.indexSavePath.empty() || !args.indexLoadPath.empty())) {
      printIndexAnalytics(reader);
    }
  }

  /* If we have file ranges referencing newlines we have to gather newlines
   * before calling readFunctor. */
  const auto hasNewlineRange =
      args.fileRanges &&
      std::any_of(args.fileRanges->begin(), args.fileRanges->end(),
                  [](const auto &range) {
                    return (range.size > 0) &&
                           (range.offsetIsLine || range.sizeIsLine);
                  });
  ...
  /* Ensure that we have the line offsets because the readFunctor is not
   * required to read the whole file. */
  ...
  if (args.verbose && args.indexLoadPath.empty() &&
      !args.indexSavePath.empty()) {
    printIndexAnalytics(reader);
  }
...
parseInputFileSpecification(const cxxopts::ParseResult &parsedArgs) {
  ...
  if (parsedArgs.count("input") == 1) {
    ...
    if (!inputFilePath.empty() && !fileExists(inputFilePath)) {
      std::cerr << "Input file could not be found! Specified path: "
                << inputFilePath << "\n";
      return {};
    }
  }
  ...
  if (inputFilePath.empty() && !stdinHasInput()) {
    std::cerr << "Either stdin must have input, e.g., by piping to it, or an "
...
int rapidgzipCLI(int argc, char const *const *argv) {
  /* Cleaned, checked, and typed arguments. */
  Arguments args;

  /**
   * @note For some reason implicit values do not mix very well with positional
   * parameters! Parameters given to arguments with implicit values will be
   * matched by the positional argument instead!
   */
  cxxopts::Options options("rapidgzip", "A gzip decompressor tool based on the "
                                        "rapidgzip backend from ratarmount");
  options.add_options("Decompression")(
      "c,stdout", "Output to standard output. This is the default, when "
                  "reading from standard input.")(
      "f,force", "Force overwriting existing output files. "
                 "Also forces decompression even when piped to /dev/null.")(
      "i,input",
      "Input file. If none is given, data is read from standard input.",
      cxxopts::value<std::string>())(
      "o,output",
      "Output file. If none is given, use the input file name with '.gz' "
      "stripped or '<input file>.out'. "
      "If no input is read from standard input and not output file is given, "
      "then will write to standard output.",
      cxxopts::value<std::string>())(
      ...
      "If an optional integer >= 1 is given, then that is the number of "
      "decoder threads to use. "
      "Note that there might be further threads being started with "
      "non-decoding work. "
      "If 0 is given, then the parallelism will be determined automatically.",
      cxxopts::value<unsigned int>()->default_value("0"))(
      "ranges",
      "Decompress only the specified byte ranges. Example: "
      "10@0,1KiB@15KiB,5L@20L,inf@40L to decompress "
      "the first 10 bytes, 1024 bytes at offset 15 KiB, as well as the 5 lines "
      "after skipping the first "
  ...
  options.add_options("Advanced")(
      ...
      "verify",
      "Verify CRC32 checksum during decompression. Will slow down "
      "decompression and there are already "
      "some implicit and explicit checks like whether the end of the file "
      "could be reached and whether "
      "the stream size is correct. If no action is specified, this will do "
      "nothing. Use --test to "
      "force checksum verification.",
      cxxopts::value(args.crc32Enabled)->implicit_value("true"))(
      "no-verify",
      "Do not verify CRC32 checksum during decompression. Might speed up "
      "decompression and there are "
      "already some implicit and explicit checks like whether the end of the "
      "file could be reached "
      "and whether the stream size is correct.",
      cxxopts::value(args.crc32Enabled)->implicit_value("false"))(
      "io-read-method",
      "Option to force a certain I/O method for reading. By default, pread "
      "will be used "
      "when possible. Possible values: pread, sequential, locked-read",
      cxxopts::value<std::string>()->default_value("pread"))(
      "index-format",
      "Option to select an output index format. Possible values: gztool, "
      "gztool-with-lines, "
      "indexed_gzip.",
      cxxopts::value<std::string>()->default_value("indexed_gzip"))(
      "sparse-windows",
      "On by default. Reduce index compressibility by zeroing out window data "
      "that is not "
      "referenced in the subsequent stream.",
      cxxopts::value(args.windowSparsity)->implicit_value("true"))(
      "no-sparse-windows",
      "Do not zero out unreferenced data in index windows. This may very "
      "slightly improve "
      "first-time decompression speed, but may also lead to higher memory "
      "usage. It can also "
      "be useful to create exported index that are binary identical to other "
      "those created "
      "by other tools such as gztool to debug indexes. In normal usage the "
      "sparsity is an "
  ...
  options.add_options("Output")("h,help", "Print this help message.")(
      ...
      "oss-attributions-yaml", "Display open-source software licenses in YAML "
                               "format for use with Conda.");

  /* These options are offered because just piping to other tools can already
   * bottleneck everything! */
  options.add_options("Actions")("d,decompress",
                                 "Force decompression. Only for compatibility. "
                                 "No compression supported anyways.")(
      "import-index", "Uses an existing gzip index.",
      cxxopts::value<std::string>())(
      "export-index", "Write out a gzip index file.",
      cxxopts::value<std::string>())("count", "Prints the decompressed size.")(
      "l,count-lines",
      "Prints the number of newline characters in the decompressed data.")(
      "t,test",
      "Verify decompression and checksums, if not disabled with --no-verify. "
      "Note that this does not have any additional effect when combined with "
      "--decompress "
      "or --ranges because it is equivalent to --decompress -o /dev/null.");

  options.parse_positional({"input"});

  /* cxxopts allows to specify arguments multiple times. But if the argument
   * type is not a vector, then only the last value will be kept! Therefore, do
   * not check against this usage and simply use that value. Arguments may only
   * be queried with as if they have (default) values. */
  ...
  const auto getParallelism = [](const auto p) {
    return p > 0 ? p : availableCores();
  };
  args.decoderParallelism =
      getParallelism(parsedArgs["decoder-parallelism"].as<unsigned int>());
  ...
  if (parsedArgs.count("version") > 0) {
    std::cout << "rapidgzip, CLI to the parallelized, indexed, and seekable "
                 "gzip decoding library rapidgzip version "
  ...
  if (parsedArgs.count("oss-attributions") > 0) {
    std::cout << "# " << thirdparty::cxxopts::name << "\n\n"
              ...
              << thirdparty::zlib::url << "\n\n"
              << thirdparty::zlib::license << "# " << thirdparty::zlib_ng::name
              << "\n\n"
  ...
  const auto decompress = (parsedArgs.count("decompress") > 0) ||
                          (parsedArgs.count("ranges") > 0) || doTest;

  /* Parse ranges. */
  if (parsedArgs.count("ranges") > 0) {
    args.fileRanges = parseFileRanges(parsedArgs["ranges"].as<std::string>());

    /* Check whether the index needs to be kept because the ranges do not
     * traverse the file in order. */
    if (args.fileRanges->size() > 1) {
      for (size_t i = 0; i + 1 < args.fileRanges->size(); ++i) {
        if (args.fileRanges->at(i).offset + args.fileRanges->at(i).size >
            args.fileRanges->at(i + 1).offset) {
          args.keepIndex = true;
          break;
        }
      }
    ...
    /* Check whether some ranges are lines and enable line offset gathering in
     * that case. */
    for (const auto &range : *args.fileRanges) {
      if ((range.size > 0) && (range.offsetIsLine || range.sizeIsLine)) {
        /* Because we cannot arbitrarily convert lines to offsets, we cannot
         * easily determine whether backward seeking is necessary. Therefore
         * keep the index if line offsets are used. */
        args.keepIndex = true;
  ...
  /* Automatically determine output file path if none has been given and not
   * writing to stdout. */
  if (outputFilePath.empty() && !inputFilePath.empty() && !doTest &&
      !writeToStdOut) {
    const std::string &suffix = ".gz";
    if (endsWith(inputFilePath, suffix, /* case sensitive */ false)) {
      outputFilePath = std::string(
          inputFilePath.begin(),
          inputFilePath.end() -
      ...
      if (!quiet && decompress) {
        std::cerr
            << "[Warning] Could not deduce output file name. Will write to '"
            << outputFilePath << "'\n";
      }
    }
  }

  if (decompress && (outputFilePath != "/dev/null") &&
      fileExists(outputFilePath) && !force) {
    std::cerr << "Output file '" << outputFilePath
  ...
  if (!args.indexLoadPath.empty() && args.indexSavePath.empty() &&
      (args.decoderParallelism == 1) && !quiet) {
  ...
  if (parsedArgs.count("index-format") > 0) {
    ...
    if (indexFormat == "indexed_gzip") {
      ...
      throw std::invalid_argument("Invalid index format string: " +
                                  indexFormat);
    }

    if (args.indexSavePath.empty() && !quiet) {
      std::cerr << "[Warning] The index format has no effect without "
                   "--export-index!\n";
    }
  }
  ...
  if (!decompress && !countBytes && !countLines && args.indexSavePath.empty()) {
    std::cerr
        << "No suitable arguments were given. Please refer to the help!\n\n";
    printRapidgzipHelp(options);
    return 1;
  }
  ...
  if (decompress) {
    if (writeToStdOut ||
        (!doTest && outputFilePath.empty() && inputFilePath.empty())) {
      stdoutFile =
          std::make_unique<OutputFile>(/* empty path implies stdout */ "");
    }
  ...
  if (decompress && !doTest && args.verbose) {
    std::cerr << "Decompress "
              << (inputFilePath.empty() ? "<stdin>" : inputFilePath.c_str())
              << " -> ";
    if (stdoutFile) {
      std::cerr << "<stdout>";
      if (outputFile) {
  ...
  const auto writeAndCount =
      [outputFileDescriptor, stdoutFileDescriptor, countLines, &newlineCount,
       &totalBytesRead, &writeDuration](
          const std::shared_ptr<rapidgzip::ChunkData> &chunkData,
          size_t const offsetInChunk, size_t const dataToWriteSize) {
        for (const auto fileDescriptor :
             {stdoutFileDescriptor, outputFileDescriptor}) {
          if (fileDescriptor == -1) {
            continue;
          }

          const auto tWriteStart = now();
          const auto errorCode = writeAll(chunkData, fileDescriptor,
                                          offsetInChunk, dataToWriteSize);
        ...
        if (countLines) {
          ...
          for (auto it = DecodedData::Iterator(*chunkData, offsetInChunk,
            ...
            newlineCount +=
                countNewlines({reinterpret_cast<const char *>(buffer), size});
          }
        }
      };
  ...
  const auto hasOutputFiles =
      (outputFileDescriptor != -1) || (stdoutFileDescriptor != -1);
  if (args.indexSavePath.empty() && countBytes && !countLines && !decompress &&
      !hasOutputFiles) {
    /* Need to do nothing with the chunks because decompressParallel returns the
     * decompressed size. Note that we use rapidgzip::ChunkDataCounter to speed
     * up decompression. Therefore an index will not be created and there also
     * will be no checksum verification! */
    ...
    const auto readRange = [&totalBytesRead, hasOutputFiles, countLines,
                            &writeAndCount](const auto &reader,
                                            const size_t size) {
      /* An empty functor will lead to decompression to be skipped if the index
       * is finalized! */
    ...
    const auto readLines = [&](const auto &reader, const size_t lineCount,
      ...
      const auto forwardLineWrites = [&lastLineOffset, &remainingLineCount,
                                      newlineCharacter, &writeFunctor](
                                         const std::shared_ptr<
                                             rapidgzip::ChunkData> &chunkData,
        ...
        for (auto it = DecodedData::Iterator(*chunkData, offsetInChunk,
          ...
          const auto &[buffer, size] = *it;
          const auto result =
              findNthNewline({reinterpret_cast<const char *>(buffer), size},
          ...
          if (remainingLineCount == 0) {
            if (result.position == std::string_view::npos) {
              throw std::logic_error(
                  "Find n-th line should return a valid position when "
        ...
        if (nBytesRead > dataToWriteSize) {
          throw std::logic_error(
              "Shouldn't have read more bytes than specified in the chunk.");
        }
    ...
    errorCode = decompressParallel<
        rapidgzip::
            ChunkData>(args, std::move(inputFile), [&](const auto &reader) {
      if (!args.fileRanges) {
        if (!hasOutputFiles && countLines &&
          ...
          /* Strictly speaking, we should be able to return here, but in case
           * the newline offset semantics change or when only partial index
           * should be loaded (not supported yet), then the subsequent "read
           * until file end" should cover these cases. */
        }
      ...
      for (const auto &range : *args.fileRanges) {
        ...
        if (((range.offsetIsLine && (range.offset > 0)) || range.sizeIsLine) &&
            reader->newlineOffsets().empty()) {
          throw std::logic_error(
              "There should be newline offset information at this point!");
        }

        /* Seek to line or byte offset. Note that line 0 starts at byte 0 by
         * definition. */
        if (range.offsetIsLine && (range.offset > 0)) {
          ...
          auto newlineOffset =
              std::lower_bound(newlineOffsets.begin(), newlineOffsets.end(),
          ...
          if (newlineOffset == newlineOffsets.begin()) {
            throw std::logic_error(
                "Bisection may never point to the first element because "
          ...
          if (newlineOffset->lineOffset >= range.offset) {
            throw std::logic_error(
                "Bisection should have returned a line offset prior to the "
  ...
  if (outputFile) {
    ...
    outputFile
        .reset(); // Close the file here to include it in the time measurement.
  }

  const auto t1 = now();
  if (args.verbose) {
    std::cerr << "Spent " << writeDuration << " s writing to output.\n";
    std::cerr << "Decompressed in total " << totalBytesRead << " B in "
              << duration(t0, t1) << " s -> "
              << static_cast<double>(totalBytesRead) / 1e6 / duration(t0, t1)
              << " MB/s\n";
  }
