...
namespace rapidgzip {
/**
 * Rpmalloc does worse than standard malloc (Clang 13) for the case when using
 * 128 cores, chunk size 4 MiB with imported index of Silesia (compression ratio
 * ~3.1), i.e., the decompressed chunk sizes are ~12 MiB and probably deviate
 * wildly in size (4-100 MiB maybe?). I guess that this leads to overallocation
 * and memory slab reuse issues in rpmalloc. Allocating memory chunks in much
 * more deterministic sizes seems to alleviate this problem immensely!
 *
 * Problematic commit:
 *     dd678c7 2022-11-06 mxmlnkn [performance] Use rpmalloc to increase
 * multi-threaded malloc performance
 *
 * Approximate benchmark results for:
 *     rapidgzip -P $( nproc ) -o /dev/null -f --export-index index
 * silesia-256x.tar.pigz taskset --cpu-list 0-127 rapidgzip -P 128 -d -o
 * /dev/null --import-index index silesia-256x.tar.pigz
 *
 * Commit:
 ...
 *
 * It seems to be pretty stable across magnitudes as long as the number of
 * allocations doesn't get too large and as long as the allocation chunk size is
 * much smaller than the decompressed data chunk size. 1 MiB seems like the
 * natural choice because the optimum (compressed) chunk size is around 4 MiB
 * and it would also be exactly one hugepage if support for that would ever be
 * added. Beware that each chunk is only as large as one gzip stream. And bgzip
 * creates gzip streams that are only ~64 KiB each! Therefore, when decoding
 * bgzip while importing the index, we need to account for this here and avoid
 * frequent overallocations and resizes, which slow down the program immensely!
 *
 * Test with:
...
/**
 * This class adds higher-level capabilities onto @ref deflate::DecodedData,
 * which was only intended for returning decompression results and aggregating
 * them during decompression of a single deflate block. This class instead is
 * intended to aggregate results from multiple deflate blocks, possibly even
 * multiple gzip streams. It is used to hold the chunk data for parallel
 * decompression. It also adds some further metadata like deflate block and
 * stream boundaries and helpers for creating evenly distributed checkpoints for
 * a gzip seek index.
 *
 * Specialized use cases can optimize memory usage or add post-processing steps
 * by implementing the two
 * @ref append methods, @ref applyWindow, and @ref finalize. The shadowed
 * methods in the base class should be called from the reimplemented methods in
 * order to keep default functionality. This call can also be knowingly omitted,
 * e.g., for only counting bytes instead of appending them.
 *
 * - @ref append is called by @ref GzipChunkFetcher after each deflate::Block
 * call back, which could be every block or up to maximum 32 KiB of decompressed
 * data.
 * - @ref finalize is called after the first stage of decompression has
 * finished. At this point, the number of elements in the chunk is finalized.
 * Elements can be 16-bit wide markers.
 * - @ref applyWindow is called during the second decompression stage and the
 * ChunkData will hold the fully decompressed data after this call.
 */
struct ChunkData : public deflate::DecodedData {
  ...
  struct Configuration {
    ...
    /** This should be used to decide what kind of footer to expect and what to
     * do after the footer. */
    ...
    /**
     * This is used by the chunk decoding implementations, but it feels more
     * correct to have this stored here, because it affects the chunk
     * configuration and in the future it might be cleaner to check for a
     * maximum size inside ChunkData::append instead, or offer a getter, which
     * could be overwritten / replaced with dynamic checks for stopping
     * decompression preemptively, e.g., if the decompressed data is known to be
     * wrong.
     */
...
public:
  ...
  void append(deflate::DecodedDataView const &toAppend) {
    ...
    /* Ignore data with markers. Those will be CRC32 computed inside @ref
     * applyWindow. */
    if (crc32s.back().enabled()) {
  ...
  void applyWindow(WindowView const &window,
    ...
    /* This is expensive! It adds 20-30% overhead for the FASTQ file! Therefore
     * disable it. The result for this statistics for: SRR22403185_2.fastq.gz
     ...
     * %) 4GiB-base64.gz Total decompressed bytes                 :
     * 4'294'967'296 Non-marker symbols                       : 4'272'350'980
     * (99.4734 %) Replaced marker symbol buffers           :    22'616'316
     * (0.526577 %) Actual marker symbol count in buffers    :       162'330
     * (0.717756 %) CTU-13-Dataset.tar.gz Total decompressed bytes :
     * 79'747'543'040 Non-marker symbols                       : 55'838'926'274
     * (70.0196 %) Replaced marker symbol buffers           : 23'908'616'766
     * (29.9804 %) Actual marker symbol count in buffers    :  2'868'357'239
     * (11.9972 %) wikidata-20220103-all.json.gz Total decompressed bytes :
     * 1'428'353'996'731 Non-marker symbols                       :
     * 23'033'941'599 (1.61262 %) Replaced marker symbol buffers           :
     * 1'405'320'055'132 (98.3874 %) Actual marker symbol count in buffers    :
     * 863'915'563'663 (61.4746 %)
     *
     * -> An alternative format that uses a mix of 8-bit and 16-bit and maybe a
     * separate 1-bit buffer to store which byte is which, would reduce memory
     * usage, and therefore also allocation overhead by 80%! Or maybe run-time
     * encode it a la: <n 8-bit values> <8-bit value> ... <m 16-bit values> This
     * would hopefully speed up window applying because hopefully long runs of
     * 8-bit values could simply be memcopied and even runs of 16-bit values
     * could be processed in a loop. This kind of compression would also add
     * overhead though and it proabably would be too difficult to do inside
     * deflate::Block, so it should probably be applied in post in
     *    ChunkData::append( DecodedDataViews ). This might be something that
     * could be optimized with SIMD, the same applies to the equally necessary
     * new ChunkData::applyWindow method.
     *    -> The count could be 7-bit so that the 8-th bit can be used to store
     * the 8/16-bit value flag. In the worst case: interleaved 8-bit and 16-bit
     * values, this would add an overhead of 25%: <n><8><n><16hi><16lo>
     * <n><8>... Ideally a format that has no overhead even in the worst-case
     * would be nice. This would be possible by using 4-bit values for <n> but
     * then the maximum runlength would be 3-bit -> 7, which seems insufficient
     * as it might lead to lots of slow execution branching in the applyWindow
     * method.
     */
    ...
    if (crc32s.front().enabled() &&
      ...
      /* Markers should only appear up to the first gzip footer because
       * otherwise a new gzip stream would have started. A new gzip stream must
       * not contain markers because there are no unresolvable back-references!
       * Because of this, it is safe to only update the first CRC32. Beware that
       * we do not only have to compute the CRC32 of markers but also for data
       * that has been been converted from dataWithMarkers inside
    ...
    /* Replace markers in and compress the resulting fully-resolved window
     * provided by each subchunk, i.e., at the end of each subchunk. In
     * benchmarks with random base64 data and ISA-L, this takes roughly 0.5 ms
     * per 32 KiB window (0.048s for 97 compressed windows). */
  ...
  void finalize(size_t newEncodedEndOffsetInBits) {
    ...
    if (toProcessSize > 0) {
      ...
      /* Note that the data with markers ought not cross footer boundaries
       * because after a footer, a new gzip stream begins, which should be known
       * to not contain any unresolvable backreferences. That's why we can
       * simply merge the CRC32 for the cleaned data with the first CRC32. */
      crc32s.front().prepend(crc32);
   ...
   * @return true if it was appended, false if the last boundary is identical to
   * the given one.
   */
  ...
  /**
   * @return When true is returned, @ref GzipChunkFetcher will queue the call to
   * @ref applyWindow in the thread pool. After the call to @ref applyWindow,
  ...
  /**
   * Chunks smaller than the returned value should not be created. In practice,
   * this currently means that such small chunks are appended to the previous
   * one. This means however that some chunks can grow larger than
   * configuration.splitChunkSize.
   */
  ...
  /**
   * Implement a kind of virtual method by using a std::function member because
   * making ChunkData polymorphic had catastrophic impact on the performance for
   * unknown reason.
   */
...
public:
  ...
  /* This should only be evaluated when it is unequal
   * std::numeric_limits<size_t>::max() and unequal Base::encodedOffsetInBits.
   * Then, [Base::encodedOffsetInBits, maxEncodedOffsetInBits] specifies a valid
   * range for the block offset. Such a range might happen for finding
   * uncompressed deflate blocks because of the byte-padding. */
  size_t maxEncodedOffsetInBits{std::numeric_limits<size_t>::max()};
  /* Initialized with size() after thread has finished writing into ChunkData.
   * Redundant but avoids a lock because the marker replacement will momentarily
   * lead to different results returned by size! */
  size_t decodedSizeInBytes{0};
  /* This is currently only set in @ref finalize and used in @ref
   * setEncodedOffset to initialize
   * @ref encodedSizeInBits. */
  size_t encodedEndOffsetInBits{std::numeric_limits<size_t>::max()};

  Configuration configuration;

  /* Decoded offsets are relative to the decoded offset of this ChunkData
   * because that might not be known during first-pass decompression. */
...
protected:
  /**
   * Takes ChunkData& as first argument instead of capturing this in order to
   * avoid having to implement custom move and copy constructors.
   */
...
inline void ChunkData::setEncodedOffset(size_t offset) {
  ...
  /* Adjust the encoded offset of the first subchunk because it may have been a
   * range at the time of splitting. */
  if (!m_subchunks.empty()) {
...
ChunkData::split([[maybe_unused]] const size_t spacing) const {
  ...
  /* blockBoundaries does not contain the first block begin but all thereafter
   * including the boundary after the last block, i.e., the begin of the next
   * deflate block not belonging to this ChunkData. */
  if ((nBlocks <= 1) || blockBoundaries.empty()) {
    return {wholeChunkAsSubchunk};
  }

  /* The idea for partitioning is: Divide the size evenly and into subchunks and
   * then choose the block boundary that is closest to that value. */
  ...
  /* The first and last boundaries are static, so we only need to find nBlocks -
   * 1 further boundaries. */
  for (size_t iSubchunk = 1; iSubchunk < nBlocks; ++iSubchunk) {
    ...
    /* If there are duplicate decodedOffset, min_element returns the first, so
     * skip over empty blocks (pigz). Using the last block with the same
     * decodedOffset makes handling the last block after this for-loop easier.
     */
    ...
    /* For very small spacings, the same boundary might be found twice. Avoid
     * empty subchunks because of that. */
  ...
  if ((lastBoundary.decodedOffset < decodedSizeInBytes) || result.empty()) {
    ...
    /* Enlarge the last subchunk encoded size to also encompass the empty blocks
     * before the chunk end. Assuming that blockBoundaries contain the boundary
     * at the chunk end and knowing that the loop above always searches for the
     * last boundary with the same decodedOffset, this branch shouldn't happen.
     */
...
/**
 * Tries to use writeAllSpliceUnsafe and, if successful, also extends lifetime
 * by adding the block data shared_ptr into a list.
 *
 * @note Limitations:
 *  - To avoid querying the pipe buffer size, it is only done once. This might
 * introduce subtle errors when it is dynamically changed after this point.
 *  - The lifetime can only be extended on block granularity even though chunks
 * would be more suited. This results in larger peak memory than strictly
 * necessary.
 *  - In the worst case we would read only 1B out of each block, which would
 * extend the lifetime of thousands of large blocks resulting in an out of
 * memory issue.
 *    - This would only be triggerable by using the API. The current CLI and not
 * even the Python interface would trigger this because either they don't splice
 * to a pipe or only read sequentially.
 * @note It *does* account for pages to be spliced into yet another pipe buffer
 * by waiting for buffer size amount of data being written before freeing, and
 * likely reusing, the memory.
 */
...
/**
 * This subclass of @ref ChunkData only counts the decompressed bytes and does
 * not store them.
 */
struct ChunkDataCounter final : public ChunkData {
  ...
  void finalize(size_t newEncodedEndOffsetInBits) {
     ...
     * because DecodedData::size() would return 0! Instead, it is updated inside
     * append. */
  ...
  /**
   * No splitting necessary for memory reduction because we don't hold the
   * results anyway.
   */
