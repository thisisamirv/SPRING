...
/**
 * @note Calls to this class are not thread-safe! Even though they use threads
...
public:
  ...
  /**
   * The fetching strategy should support parallelization via prefetching for
   * sequential accesses while avoiding a lot of useless prefetches for random
   * or multi-stream sequential accesses like those occurring via ratarmount.
   * The fetching strategy does not have to and also should not account for
   * backward and strided accesses because the prefetch and cache units are very
   * large and striding or backward accessing over multiple megabytes should be
   * extremely rare.
   */
  using ChunkFetcher =
...
public:
   ...
   * {
   *     for chunkSize in 128 $(( 1*1024 )) $(( 2*1024 )) $(( 4*1024 )) $((
   * 8*1024 )) $(( 16*1024 )) $(( 32*1024 )); do echo "Chunk Size: $chunkSize
   * KiB" for i in $( seq 5 ); do src/tools/rapidgzip --chunk-size $chunkSize -v
   * -P 0 -d -c "$1" 2>rapidgzip.log | wc -c grep "Decompressed in total"
   * rapidgzip.log done done
   * }
   *
   * m rapidgzip
   ...
   * probably because even work division becomes a problem relative to the file
   * size. Furthermore, caching behavior might worsen for larger chunk sizes.
   *
   * @verbatim
   * wget https://sun.aei.polsl.pl/~sdeor/corpus/silesia.zip
   * mkdir -p silesia && ( cd silesia && unzip ../silesia.zip )
   * tar -cf silesia.tar silesia/  # 211957760 B -> 212 MB, 203 MiB, gzip 66 MiB
   * -> compression factor: 3.08 for (( i=0; i<40; ++i )); do cat 'silesia.tar';
   * done | pigz > 40xsilesia.tar.gz m rapidgzip benchmarkWc 40xsilesia.tar.gz
   *
   ...
   * @endverbatim
   *
   * Beware, on 2xAMD EPYC CPU 7702, when decoding with more than 64 cores, the
   * optimum is at 2 MiB instead of 4-8 MiB! Maybe these are NUMA domain +
   * caching issues combined?
   *
   ...
   *  - RAM: 2x16GiB DIMM DDR4 3600 MHz (0.3 ns), 2x32GiB DIMM DDR4 3600 MHz
   * (0.3 ns)
   *
   * AMD EPYC CPU 7702:
   ...
   *
   * -> That EPYC processor is the same generation Zen 2 and therefore has
   * identical L1 and L2 caches and the L3 cache size is even higher, so it must
   * be a NUMA issue.
   *
   * Non-compressible data is a special case because it only needs to do a
   * memcpy.
   *
   * @verbatim
   ...
   * @endverbatim
   *
   * Another set of benchmarks that exclude the bottleneck for writing the
   * results to a pipe by using the rapidgzip option --count-lines. Note that in
   * contrast to pugz, the decompressed blocks are still processed in sequential
   * order. Processing them out of order by providing a map-reduce like
   * interface might accomplish even more speedups.
   *
   * @verbatim
   * m rapidgzip
   * for chunkSize in 128 $(( 1*1024 )) $(( 2*1024 )) $(( 4*1024 )) $(( 8*1024
   * )) $(( 16*1024 )) $(( 32*1024 )); do echo "Chunk Size: $chunkSize KiB" for
   * i in $( seq 5 ); do src/tools/rapidgzip -v --chunk-size $chunkSize -P 0
   * --count-lines 4GiB-base64.gz 2>rapidgzip.log grep "Decompressed in total"
   * rapidgzip.log done done
   *
   * spacing | bandwidth / (MB/s)
   * --------+--------------------
   ...
   * @endverbatim
   *
   * The factor 2 amount of read data can be explained with the BitReader always
   * buffering 128 KiB! Therefore if the work chunk is too small, it leads to
   * this problem.
   * @note Beware the actual result of wc -l! With the wrong vmsplice usage, it
   * returned random results for chunk sizes smaller than 4 MiB or even for
   * higher chunk sizes with alternative malloc implementations like mimalloc.
   * @note The optimum at ~8 MiB for incompressible data vs ~4 MiB for base64
   * data with a compression ratio ~1.3 might be explainable with a roughly
   * equal decompressed block size. In general, we would like the chunk size to
   * be measured in decompressed data because the decompressed bandwidth is much
   * more stable than the compressed bandwidth over a variety of data.
   * @todo We might be able to reduce this overhead by buffering up to
   * untilOffset and then only increase the buffer in much smaller steps, e.g.,
   * 8 KiB. This might actually be easy to implement by making the BitReader
   * chunk size adjustable.
   * @todo Possibly increase the chunk size to 4 or 8 MiB again after
   * implementing an out of memory guard for high compression ratios so that
   * CTU-13-Dataset.tar.gz can be decompressed with less than 30 GB of RAM!
   *       Rebenchmark of course whether it makes sense or not anymore. E.g.,
   * speeding up the block finder might enable smaller chunk sizes.
   *
   * @verbatim
   * for (( i=0; i<10; ++i )); do cat 'silesia.tar'; done | lbzip2 >
   * 10xsilesia.tar.bz2 stat --format=%s -L 10xsilesia.tar.bz2 546 315 457
   ...
   * @endverbatim
   *
   * There simply is not enough work to distribute. That's why it is slow for
   * larger chunk sizes. For smaller chunk sizes it becomes slow because some
   * chunks won't find anything to decode but they still count towards the
   * maximum cached chunk size.
   * @todo They shouldn't count towards that limit because they don't consume
   * much memory anyway. Maybe test those somehow and move them into a different
   * lookup cache, or simply don't count them. The latter might be expensive if
   * they become too many and if it isn't a simply bool check. Unfortunately, it
   * isn't even easily possible to check for exception. We would have to call
   * future::get() in a try-catch-block and repackage the result thereafter or
   * change the ChunkFetcher interface to not return blocks. Lots of work. Or
   * simply don't use chunk sizes smaller than 1 MiB because compressed bzip2
  ...
  explicit ParallelGzipReader(UniqueFileReader fileReader,
    ...
    if (fileSize && (m_chunkSizeInBytes * 2U * parallelization > *fileSize)) {
       ...
       * Multiply a factor of two, to give the thread pool more time to be
       * filled out. Bound the minimum chunk size because of the block finder
       * overhead for gzip, because <900kB chunks might not have any real work
       * to do, and to avoid many threads being started for very small files.
       * This formula is mostly optimized for silesia.tar.bz2.
       * Speed isn't that important for small gzip files because it decompresses
       * many times faster. In the first place, this implementation is intended
       * towards very large files not small files. */
    ...
    if (!m_sharedFileReader->seekable()) {
      /* The ensureSharedFileReader helper should wrap non-seekable file readers
       * inside SinglePassFileReader. */
...
#ifdef WITH_PYTHON_SUPPORT
  /* These constructor overloads are for easier construction in the
   * Cython-interface. For C++, the FileReader constructor would have been
   * sufficient. */
  ...
  /**
   * @param nBytesToRead This parameter can be performance-critical! Very small
   * calls must be avoided because lots of checks such as for closed() can
   * become expensive as they might require locking mutexes! Optimal inclusive
   * ranges for number of bytes per call on Ryzen 3900X 12-core:
   *        - parallelization=1            : [8 KiB, 256 MiB]  no threading and
   * therefore any value is alright!
   *        - parallelization=24           : [8 MiB, 256 MiB] smaller 128 B
   * becomes unusably slow!
   *        - parallelization=24 with index: [32 KiB, 4 MiB]  smaller 128 B
   * becomes unusably slow! Therefore, my recommendation would be to simply use
   * 4 or 8 MiB, but [128 KiB, 256 MiB] is generally fine if you can live with
   * ~20 % slowdowns. See also the comment for DECOMPRESSION_BUFFER_SIZE in
   * benchmarkGzip.cpp!
   */
  ...
  size_t read(const int outputFileDescriptor = -1,
    ...
    if ((outputFileDescriptor == -1) && (outputBuffer == nullptr)) {
      /* An empty std::function gives that read method options to optimize,
       * e.g., via seeking. */
  ...
  size_t read(const WriteFunctor &writeFunctor,
    ...
    while ((nBytesDecoded < nBytesToRead) && !eof()) {
      ...
      auto *const singlePassFileReader =
          dynamic_cast<SinglePassFileReader *>(file);
      if (singlePassFileReader != nullptr) {
        /* Release only up to the beginning of the currently used chunk in order
         * to theoretically enable to clear the full cache and then continue
         * again. This effectively require a recomputation of the current chunk
         * if it was not fully read yet. */
        singlePassFileReader->releaseUpTo(
            /* floor int division */ chunkData->encodedOffsetInBits / CHAR_BIT);
      }
  ...
  size_t seek(long long int offset, int origin = SEEK_SET) override {
    ...
    /* m_blockMap is only accessed by read and seek, which are not to be called
     * from different threads, so we do not have to lock it. */
    ...
    /* Jump to furthest known point as performance optimization. Note that even
     * if that is right after the last byte, i.e., offset == size(), then no
     * eofbit is set even in ifstream! In ifstream you can even seek to after
     * the file end with no fail bits being set in my tests! */
    m_atEndOfFile = false;
    m_currentPosition =
  ...
  /**
   * This is the first instance for me where returning a const value makes sense
   * because it contains a shared pointer to the WindowMap, which is not to be
   * modified. Making GzipIndex const forces the caller to deep clone the index
   * and WindowMap for, e.g., the setBlockOffsets API, which destructively moves
   * from the WindowMap.
   */
  ...
  gzipIndex(bool withLineOffsets = false) {
    ...
    if (!archiveSize && !m_indexIsImported) {
      /* If the index was imported, then this warning is moot and using the last
       * chunk offset is sufficient. */
    ...
    /* Heuristically determine a checkpoint spacing from the existing
     * checkpoints. */
  ...
  /**
   * Same as @ref blockOffsets but it won't force calculation of all blocks and
   * simply returns what is available at call time.
   * @return vectors of block data: offset in file, offset in decoded data
   *         (cumulative size of all prior decoded blocks).
   */
  ...
  void setMaxDecompressedChunkSize(uint64_t maxDecompressedChunkSize) {
    /* Anything smaller than the chunk size doesn't make much sense. Even that
     * would be questionable. as it would lead to slow downs in almost every
     * case. */
  ...
  /**
   * Must only be changed before doing any read call! Else, some of the chunks
   * will already have been processed with the existing newline format.
   *
...
private:
  void setBlockOffsets(const std::map<size_t, size_t> &offsets) {
    /**
     * @todo Join very small consecutive block offsets until it roughly reflects
     * the chunk size? Because currently, the version with the BGZI index is
     * slower than without! rapidgzip -d -o /dev/null 4GiB-base64.bgz >
...
public:
  void setBlockOffsets(const GzipIndex &index) {
    ...
    if (index.hasLineOffsets && newlineCharacter) {
      ...
      /* Checkpoints should already be sorted and therefore also the newline
       * offsets. Check just to be sure. We are not sorting here because it may
       * be impossible to sort by line offsets and uncompressed offsets for
    ...
    for (size_t i = 0; i < index.checkpoints.size(); ++i) {
      ...
      /**
       * Skip emission of an index, if the next checkpoint would still let us be
       * below the chunk size. Always copy the zeroth index as is necessary for
       * a valid index!
       *
       * This is for merging very small index points as might happen when
       * importing BGZF indexes. Small index will lead to relatively larger
       * overhead for the threading and will degrade performance: Merge n
       * blocks: 0 -> ~3.3 GB/s, Total existing blocks: 65793 2 -> ~5.0 GB/s,
       * Total existing blocks: 32897 4 -> ~5.7 GB/s, Total existing blocks:
       * 16449 8 -> ~6.0 GB/s, Total existing blocks: 8225 16 -> ~6.8 GB/s,
       * Total existing blocks: 4113 32 -> ~6.8 GB/s, Total existing blocks:
       * 2057 64 -> ~7.2 GB/s, Total existing blocks: 1029 128 -> ~6.9 GB/s,
       * Total existing blocks: 515
       *
      ...
      /* Copy window data.
       * For some reason, indexed_gzip also stores windows for the very last
       * checkpoint at the end of the file, which is useless because there is
       * nothing thereafter. But, don't filter it here so that exportIndex
       * mirrors importIndex better.
       * Bgzip indexes do not have windows because they are not needed, so we do
       * not have to insert anything into the WindowMap in that case. Bgzip
       * indexes will be detected by the magic bytes and in that case windows
       * should never be looked up in the WindowMap in the first place. */
  ...
  void gatherLineOffsets() {
    /* Check whether the newline information has already been collected from an
     * imported index or earlier call. */
    if (!m_newlineCharacter) {
      return;
    }
    ...
    /* If it was already toggled on, simply read until the end to gather all
     * offsets. */
    ...
    /* Block offset is complete, check if line offsets are complete by checking
     * the last one. */
    ...
    /* This may be necessary when the block map has been finalized because an
     * index without line information has been imported! In that case, we need
     * to gather line information manually like a user would. */

    /* Collect line offsets until the next chunk offset has been added to the
     * map. Then, we can look for the line number at that exact chunk offset and
     * insert it and clear our temporary results. */
    ...
    const auto collectLineOffsets =
        [this, &processedLines, &newlineOffsets, &processedBytes,
         newlineCharacter = m_newlineCharacter.value()](
          ...
          /* Iterate over the requested data range of the chunk and collect byte
           * offsets for every newline. */
          using rapidgzip::deflate::DecodedData;
          ...
          /* Iterate over all found newline offsets and start inserting an
           * actual byte->newline offset pair but only once per chunk to reduce
           * the index size. */
          ...
          while (it != newlineOffsets.end()) {
            ...
            if (!chunkInfo.contains(*it)) {
              /* I don't think this can happen. It happens when the currently
processed chunk
-                        * is not yet registered in the chunk map. */
   ...
   * @note Bzip2 is block based and blocks are currently read fully, meaning
   * that the granularity of the returned position is ~100-900kB. It's only
  ...
  /**
   * Closes all threads and saves the work. They will be restarted when needed
   * again, e.g., on seek or read. This is intended for use with fusepy. You can
   * start a ParallelGzipReader use it to create the block map and print out
   * user output and then you join all threads before FUSE forks the process.
   * FUSE requires threads to be created after it forks, it seems:
   * @see
   * https://github.com/libfuse/libfuse/wiki/FAQ#how-should-threads-be-started
   * Personally, the only problem I observed was background process not
   * finishing even after unmounting, however, contrary to the FAQ it seems that
   * threads were not joined because the file system seemed to work.
   */
  void joinThreads() {
    m_chunkFetcher.reset();
    m_blockFinder.reset();
  ...
  /**
   * Index-keeping can be disabled as a memory usage optimization when it will
   * never be needed. Currently, this will clear windows for chunks that have
   * been fully decompressed once. Trying to seek in the file with this option
   * enabled will throw an error!
   */
...
private:
  ...
  BlockFinder &blockFinder() // NOLINT(misc-no-recursion)
  {
    /* This guard makes the warned-about recursion via setBlockFinderOffsets
     * safe. */
    if (m_blockFinder) {
  ...
  ChunkFetcher &chunkFetcher() {
    ...
    /* As a side effect, blockFinder() creates m_blockFinder if not already
     * initialized! */
    blockFinder();
  ...
  void setBlockFinderOffsets(
      const std::map<size_t, size_t> &offsets) // NOLINT(misc-no-recursion)
  {
    if (offsets.empty()) {
      throw std::invalid_argument(
    ...
    /* The last block is not pushed because "std::next( it )" is end but last
     * block must be EOS anyways or else BlockMap will not work correctly
     * because the implied size of that last block is 0! */
  ...
  void processCRC32(const std::shared_ptr<ChunkData> &chunkData,
    ...
    if ((m_nextCRC32ChunkOffset == 0) && m_blockFinder) {
      const auto [offset, errorCode] =
          m_blockFinder->get(/* block index */ 0, /* timeout */ 0);
    ...
    /* As long as CRC32 is enabled, this should not happen and we filter above
     * for !m_crc32.enabled(). */
...
private:
  ...
  size_t m_currentPosition = 0; /**< the current position as can only be
                                   modified with read or seek calls. */
  ...
  /**
   * The window map should contain windows to all encoded block offsets inside
   * @ref m_blockMap. The windows are stored in a separate map even though all
   * keys should be identical because BlockMap is too "finished". I don't see
   * how to generically and readably add generic user data / windows to it.
   * Furthermore, the windows might potentially be written out-of-order while
   * block offsets should be inserted in order into @ref m_blockMap.
   */
  ...
  /**
   * Note that the uncompressed offset can point to any byte offset inside the
   * line depending on how the chunks are split. Only the offset to the 0-th
   * line is exact of course. To get any other line beginning exactly, you need
   * to start from the previous line and search for the next newline. Note also
   * that not all line offsets have to be in this vector. That's why it is a
   * vector of pairs and not a simply vector of values. Line offsets are only
   * available at spacings. To get an exact line offset, you need to start
   * reading from the next smaller one and skip over as many newline characters
   * as necessary.
   */
