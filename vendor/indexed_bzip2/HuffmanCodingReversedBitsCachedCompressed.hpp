...
namespace rapidgzip {
/**
 * This version uses a large lookup table (LUT) to avoid loops over the
 * BitReader to speed things up a lot. The problem is that the LUT creation can
 * take a while depending on the code lengths.
 * - During initialization, it creates a LUT. The index of that array are a
 * fixed number of bits read from BitReader. To simplify things, the fixed bits
 * must be larger or equal than the maximum code length. To fill the LUT, the
 * higher bits the actual codes with shorter lengths are filled with all
 * possible values and the LUT table result is duplicated for all those values.
 * This process is slow.
 * - During decoding, it reads MAX_CODE_LENGTH bits from the BitReader and uses
 * that value to access the LUT, which contains the symbol and the actual code
 * length, which is <= MAX_CODE_LENGTH. The BitReader will be seeked by the
 * actual code length. The "compressed" part of the name references the fact
 * that the symbol and code length are stored not as a pair but in a bit-packed
 * manner in the LUT. This reduces the LUT size by 50% for Symbol = uint16_t
 * (value is uint16_t instead of std::pair<uint8_t, uint16_t> and uint16_t
 * aligns to 2 B, which effectively increases the pair size to 4 B inside the
 * array).
 */
...
public:
  ...
  initializeFromLengths(const VectorView<BitCount> &codeLengths) {
     ...
     * And for decoding 13403 deflate blocks in 5.7s, this makes a total
     * overhead of 0.38s (6.6%). The actual block decoding as opposed to header
     * reading, takes roughly 400µs (total over blocks: 5.3s)
     *  -> This adds up to the observed timings and shows that the header
     * reading is still more than a magnitude faster and could still do some
     * more setup if it reduces decoding more than that!. So it isn't all that
     * large but also doesn't improve speed by all that much either :( Maybe try
     * smaller lookup table to stay in L1 cache? The test processor, a Ryzen
     * 3900X has L1 Cache: 64K (per core) L2 Cache: 512K (per core) L3 Cache:
     * 64MB (shared) So, theoretically it shouldn't exceed the L1 cache size but
     * who knows. */
    // const auto t0 = now();
    ...
    //  Some timings for small.gz: 2.3676e-05 s, 2.186e-05 s, 2.3607e-05
    //  s, 2.1791e-05 s, 2.3606e-05 s, 3.6038e-05 s
...
private:
   ...
   * length, which only requires ceil(log2(15)) = 4 bits. This scheme is ~5%
   * faster than storing the length and symbol as a pair probably because of
   * multiple reasons:
   *  - any pair < 64-bit probably has to use some bit shifts anyway so not much
   * more work
   *  - using 8-bit length and 16-bit symbol yields non-aligned access quite
   * frequently
   *  - the space reduction by 33% might improve L1 cache hit rates or cache
   * line utilization. */
