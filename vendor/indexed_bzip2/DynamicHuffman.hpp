...
#include "CountAllocatedLeaves.hpp"

namespace rapidgzip::blockfinder {

template <uint8_t bitCount> constexpr bool isDeflateCandidate(uint32_t bits) {
  ...
  for (uint32_t i = 0; i < result.size(); ++i) {
    ...
    if (result[i] == 0) {
      result[i] = -static_cast<uint8_t>(
          1U + nextDeflateCandidate<CACHED_BIT_COUNT - 1U>(i >> 1U));
    }
  }
  return result;
}();
...
template <>
inline constexpr std::array<int8_t, 1U << 15U>
    NEXT_DYNAMIC_DEFLATE_CANDIDATE_LUT<15U> = {
#include "NEXT_DYNAMIC_DEFLATE_CANDIDATE_LUT_15.csv"
};

template <uint8_t CACHED_BIT_COUNT = OPTIMAL_NEXT_DEFLATE_LUT_SIZE>
[[nodiscard]] size_t seekToNonFinalDynamicDeflateBlock(
  ...
  try {
    ...
    constexpr auto ALL_PRECODE_BITS =
        PRECODE_COUNT_BITS + MAX_PRECODE_COUNT * PRECODE_BITS;
    static_assert(
        (ALL_PRECODE_BITS == 61) && (ALL_PRECODE_BITS >= CACHED_BIT_COUNT) &&
            (ALL_PRECODE_BITS <= std::numeric_limits<uint64_t>::digits) &&
            (ALL_PRECODE_BITS <= gzip::BitReader::MAX_BIT_BUFFER_SIZE),
        "It must fit into 64-bit and it also must fit the largest possible "
        "jump in the LUT.");
    auto bitBufferPrecodeBits = bitReader.read<ALL_PRECODE_BITS>();

    for (size_t offset = oldOffset; offset < untilOffset;) {
      auto nextPosition =
          NEXT_DYNAMIC_DEFLATE_CANDIDATE_LUT<CACHED_BIT_COUNT>[bitBufferForLUT];
      const auto bitsToLoad = std::abs(nextPosition);

      if (nextPosition <= 0) {
        nextPosition = -nextPosition;

        const auto next4Bits = bitBufferPrecodeBits &
                               nLowestBitsSet<uint64_t, PRECODE_COUNT_BITS>();
        const auto next57Bits =
            (bitBufferPrecodeBits >> PRECODE_COUNT_BITS) &
            nLowestBitsSet<uint64_t, MAX_PRECODE_COUNT * PRECODE_BITS>();

        const auto precodeError =
            PrecodeCheck::CountAllocatedLeaves::checkPrecode(next4Bits,
                                                             next57Bits);
        ...
        if (UNLIKELY(precodeError == Error::NONE)) [[unlikely]] {
          ...
          const auto literalCodeCount =
              257 + ((bitBufferForLUT >> 3U) & nLowestBitsSet<uint64_t, 5>());
          const auto distanceCodeCount =
              1 + ((bitBufferForLUT >> 8U) & nLowestBitsSet<uint64_t, 5>());
          const auto codeLengthCount = 4 + next4Bits;
          const auto precodeBits =
              next57Bits &
              nLowestBitsSet<uint64_t>(codeLengthCount * PRECODE_BITS);
          ...
          auto error = precodeHC.initializeFromLengths(
              {codeLengthCL.data(), codeLengthCL.size()});
          ...
          if (LIKELY(error == Error::NONE)) [[likely]] {
            ...
            error = readDistanceAndLiteralCodeLengths(
                literalCL, bitReader, precodeHC,
                literalCodeCount + distanceCodeCount);

            bitReader.seekTo(offset + 13 + ALL_PRECODE_BITS);
          }

          if (UNLIKELY(literalCL[deflate::END_OF_BLOCK_SYMBOL] == 0))
              [[unlikely]] {
            error = Error::INVALID_CODE_LENGTHS;
          }

          if (UNLIKELY(error == Error::NONE)) [[unlikely]] {
            if (!checkHuffmanCodeLengths<MAX_CODE_LENGTH>(VectorView<uint8_t>(
                    literalCL.data() + literalCodeCount, distanceCodeCount)) ||
          ...
          if (oldTell != bitReader.tell()) {
            std::cerr << "Previous position: " << oldTell
                      << " new position: " << bitReader.tell() << "\n";
      ...
      if constexpr (CACHED_BIT_COUNT > 13) {
        ...
        bitBufferForLUT |=
            ((bitBufferPrecodeBits >> DUPLICATED_BITS) &
             nLowestBitsSet<uint64_t>(bitsToLoad))
            << static_cast<uint8_t>(CACHED_BIT_COUNT - bitsToLoad);
      } else {
        bitBufferForLUT |=
            (bitBufferPrecodeBits & nLowestBitsSet<uint64_t>(bitsToLoad))
            << static_cast<uint8_t>(CACHED_BIT_COUNT - bitsToLoad);
      }

      bitBufferPrecodeBits >>= bitsToLoad;
      bitBufferPrecodeBits |=
          bitReader.read(bitsToLoad)
