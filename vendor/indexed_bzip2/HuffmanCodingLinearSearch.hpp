 ...
 * - During initialization, it stores all codes and their lengths in two
 * vectors.
 * - During decoding, it reads the bits one by one and for each intermediary,
 * checks whether there is a matching code with the current length in the
 * vectors. Note that reading the bits one by one is also necessary to reverse
 * the codes.
 */
...
public:
  /**
   * Creates a Huffman coding for the alphabet (0, ..., codeLengths.size() - 1)
   * using the respective bit lengths for the Huffman codes.
  ...
  Error initializeFromLengths(const VectorView<BitCount> &codeLengths) {
    ...
    /* Huffman tree give variable-length binary codes for each leaf node
     * representing a symbol Maximum code length: ??? Further restrictions on
     * Huffman tree by Deflate:
     *  - Shorter codes lexicographically precede longer codes.
     *  - All codes of a given bit length have lexicographically consecutive
     * values, in the same order as the symbols they represent With this, the
     * Huffman tree can be exactly specified as a vector, whose elements
     * represent the ordered alphabet's symbol's code lengths, e.g, for alphabet
     * ABCD: (2,1,3,3) -> 0b10, 0b0, 0b110, 0b111
     */
    ...
    /* Bit lengths of zero mean, that the particular "letter" is not used at all
     * and should be skipped for calculating the codes. However, the value must
     * be zero for the initial condition of the next algorithms to work. */
    bitLengthFrequencies[0] = 0;

    /* Check whether these values make sense. A bit length can only represent
     * finite amount of codes: Bit length 1 -> 2 if no longer bit length, 1 if
     * there are longer bit lengths, 0 if any of the next levels contain more
     * than 2^(level-1) leafs/symbols! It is easier to check this in reverse! If
     * the last level is completely filled with symbols, then the level before
     * it cannot have any leaf nodes. Or more generically, half the amount of
     * possible values of the previous level are filled up for being parents to
     * the leaf nodes of the next level. */
    ...
    for (int bitLength = bitLengthFrequencies.size() - 1; bitLength > 0;
       ...
       *   -> can store 256 values but because of the previous one only 256 -
       * 112/2 = 200
       *      -> not even utilized fully ...?
       * fixed tree has 279 - 256 + 1 = 24 symbols of length 7
       *   -> can store 128 - 152 / 2 = 52
       *      -> also not fully utilized ... Why use such a suboptimal fixed
       * tree?! */
      maxSymbolsPossible =
          (uint8_t(1) << (bitLength - 1)) -
          ceilDiv(*frequency, 2); // ( *frequency >> 2U ) + ( frequency & 1 );
    }

    /**
     * Find the minimum code value for each bitLength to start assigning values
     * from. This is basically the above test in reverse plus assuming that
     * lower tree levels are filled from the left, i.e., the highest code values
     * on a level are all non-leaf nodes.
     * @verbatim
     ...
     *   100 <- B   C -> 101        2
     * @endverbatim
     * This would be encoded as (ABC) -> (1,3,3) => frequencies: (1,2,3) ->
     * (1,0,2). The smallest code length value on level 2 (or l in general), is
     * the smallest non-leaf node on level l-1 with a 0 appended in binary,
     * i.e., multiplied by 2. The smallest non-leaf node on a level is given by
     * the smallest code value on the previous level plus the number of leaf
     * nodes on that level, i.e., symbols with the previous code length.
     */
    ...
    /* Now, begin assigning the alphabet consecutively to the codes starting
     * from the minimumCodes.
     * @todo It might be possible to merge this with the
     * minimumCodeValuesPerLevel creation loop to avoid that intermediary vector
     * and hopefully shave off time. Note that this for loop destructibly works
     * on minimumCodeValuesPerLevel anyway.
     * @todo The minimum codes alone might even be enough to decode Huffman
     * codes! */
  ...
  decode(BitReader &bitReader) const {
    ...
    for (BitCount bitLength = m_minCodeLength; bitLength <= m_maxCodeLength;
       ...
       * @todo faster lookup than simple linear search, e.g., binning by length
       * and binary search between.
       *       -> then again binary search is not even necessary because the
       * values in each bin are given sequentially. Might be possible to simply
       * add the lookup value to the minimumValueCode or so! */
