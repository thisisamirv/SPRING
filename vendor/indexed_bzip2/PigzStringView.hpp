...
public:
  ...
  [[nodiscard]] size_t find() override {
    while (m_blockOffsets.empty() && !m_fileReader->eof() &&
           !m_fileReader->fail() && !m_fileReader->closed()) {
      if (foundFirstBlock) {
        analyzeNextChunk();
      } else {
        findFirstBlock();
...
private:
  void findBlockOffsets(const std::string_view &stringView,
                        std::size_t offset) {
    for (auto position = stringView.find(EMPTY_DEFLATE_BLOCK.data(), 0,
                                         EMPTY_DEFLATE_BLOCK.size());
         position != std::string_view::npos;
         position = stringView.find(EMPTY_DEFLATE_BLOCK.data(), position + 1,
                                    EMPTY_DEFLATE_BLOCK.size())) {
      if ((position >= 1)

          && ((static_cast<uint8_t>(stringView[position - 1]) & 0b1110'0000) ==
              0)) {
  ...
  void analyzeNextChunk() {
    constexpr std::size_t nBytesToRetain =
        ceilDiv(MAGIC_BIT_STRING_SIZE, CHAR_BIT) - 1;
    static_assert(nBytesToRetain == 4,
                  "Assuming bit string size of 35 for empty deflate block.");
    const auto checkBoundary = m_bufferSize > 0;
    /* We want to be able to find strings even if only their first byte is in
     * the last buffer or even if only their last byte is in the next buffer and
     * of course all cases between. */
    ...
    /* Always read chunks of BUFFER_SIZE in order to have aligned I/O and memory
     * accesses. */
    ...
    if (checkBoundary) {
      ...
      findBlockOffsets({boundaryBuffer.data(), boundaryBufferSize},
                       boundaryBufferOffset);
    }
  ...
  void findFirstBlock() {
    ...
    gzip::BitReader bitReader(
        std::make_unique<BufferedFileReader>(std::move(buffer)));
    ...
    if ((rapidgzip::gzip::checkHeader(bitReader) == rapidgzip::Error::NONE) &&
      ...
      /* Do not seek directly to one byte after the found offset in order to
       * keep I/O aligned. */
    ...
    /* If the first block couldn't be found, then don't even try to search for
     * the others because the first block would be missing, i.e., it would lead
     * to incomplete data! */
