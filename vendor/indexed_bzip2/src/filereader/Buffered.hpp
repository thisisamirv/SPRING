...
public:
  explicit BufferedFileReader(UniqueFileReader fileReader,
                              size_t bufferSize = 128_Ki)
      : m_maxBufferSize(bufferSize), m_file(std::move(fileReader)) {}

  explicit BufferedFileReader(const std::vector<char> &inMemoryFileContents,
                              size_t bufferSize = 128_Ki)
      : m_maxBufferSize(bufferSize),
        m_buffer(inMemoryFileContents.begin(), inMemoryFileContents.end()) {}

  explicit BufferedFileReader(const VectorView<char> &inMemoryFileContents,
                              size_t bufferSize = 128_Ki)
      : m_maxBufferSize(bufferSize),
        m_buffer(inMemoryFileContents.begin(), inMemoryFileContents.end()) {}

  explicit BufferedFileReader(AlignedBuffer inMemoryFileContents,
                              size_t bufferSize = 128_Ki)
      : m_maxBufferSize(bufferSize), m_buffer(std::move(inMemoryFileContents)) {
  }
  ...
  [[nodiscard]] bool fail() const override { return m_file && m_file->fail(); }
  ...
  [[nodiscard]] int fileno() const override {
    ...
    throw std::invalid_argument(
        "Trying to get fileno of an in-memory or closed file!");
  }
  ...
  [[nodiscard]] size_t read(char *buffer, size_t nMaxBytesToRead) override {
    ...
    refillBuffer();
    return nBytesRead +
           readFromBuffer(buffer + nBytesRead, nMaxBytesToRead - nBytesRead);
  }
  ...
  size_t seek(long long int offset, int origin = SEEK_SET) override {
    ...
    if (!m_file) {
      throw std::invalid_argument(
          "Cannot seek outside of in-memory file range!");
    }
...
private:
  void refillBuffer() {
    ...
    m_bufferPosition = 0;
    m_originalBufferOffset =
        m_file->seek(m_originalBufferOffset + m_buffer.size());
    m_buffer.resize(m_maxBufferSize);
    const auto nBytesRead = m_file->read(m_buffer.data(), m_buffer.size());
    m_buffer.resize(nBytesRead);
  }

  size_t readFromBuffer(char *buffer, size_t nMaxBytesToRead) {
    if (m_bufferPosition >= m_buffer.size()) {
      return 0;
    }

    const auto nBytesToReadFromBuffer =
        std::min(m_buffer.size() - m_bufferPosition, nMaxBytesToRead);
    if (buffer != nullptr) {
      std::memcpy(buffer, m_buffer.data() + m_bufferPosition,
                  nBytesToReadFromBuffer);
    }
