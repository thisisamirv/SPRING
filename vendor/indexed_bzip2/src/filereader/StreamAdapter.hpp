...
/**
 * This class implements the std::streambuf interface by forwarding calls to the
 * appropriate FileReader methods.
 *
 * It manages a buffer called "associated character sequence" buffering the
 * access to the underlying FileReader, called "controlled character sequence".
 * The interface is range-like and consists of a begin, end, and current pointer
 * for the get and put area. Because the FileReader is read-only, the put area
 * as well as the overflow method is not supported.
 *
 * @todo Implement xsgetn, putbackfail for more efficient reading.
 * @todo Specialize for ParallelGzipReader after adding an API that returns the
 * ChunkData pointer. This would avoid another unnecessary copy and the
 * ChunkData would simply be the buffer.
 */
...
protected:
  /**
   * @return Number of characters that can be read from the internal buffer
   * instead of the underlying file.
   * @note This implementation seems trivial to me. I don't understand why the
   * default implementation only returns -1 instead of this generic
   * implementation, probably because there is no interface for something like
   * "is open".
   * @note "If showmanyc returns -1, underflow() and uflow() will definitely
   * return Traits::eof or throw." So, it seems that -1 is a stronger 0, i.e.,
   * if not only the associated but also the controlled character sequence is at
   * the end.
  ...
  int_type underflow() override {
    ...
    setg(/* begin */ m_buffer.data(), /* current */ m_buffer.data(),
         /* end */ m_buffer.data() + nBytesRead);
    return traits_type::to_int_type(*gptr());
  }
  ...
  pos_type seekoff(off_type offset, std::ios_base::seekdir anchor,
    ...
    /* We cannot simply forward to FileReader::tell or seek because the
     * FileReader is positioned at the end of the buffer! While this class'
     * position somewhere inside the buffer. This means that SEEK_CUR would be
     * relative to some different offset and return wrong absolute offsets!
     * std::istream::tellg calls seekoff( 0, std::ios_base::cur,
     * std::ios_base::in ) and should not clear the buffer in general. */
    if (anchor == std::ios_base::cur) {
      ...
      /* It is allowed to seek at the end of the buffer. In that case, the next
       * read call should simply trigger underflow. However, it is not allowed
       * to seek even further past the end of the buffer! This <= bufferSize
       * comparison instead of < also allows to handle tellg with an empty
       * buffer, e.g., before the first read, more gracefully. */
      if ((newPosition >= 0) && (newPosition <= bufferSize)) {
        setg(/* begin */ eback(), /* current */ eback() + newPosition,
             /* end */ egptr());
        const auto tellOffset =
            /* file offset corresponds to past-end of buffer offset */ m_file
                ->tell() -
            (egptr() - gptr());
...
public:
  ...
  /**
   * @note The destructor also closes, so calling this method should be rarely
   * necessary.
   */
