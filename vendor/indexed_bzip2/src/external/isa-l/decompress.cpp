...
int decompress_file(void) {
  ...
  if (ret != ISAL_DECOMP_OK) {
    std::cerr << "igzip: Error invalid gzip header found for file "
              << infile_name << "\n";
    goto decompress_file_cleanup;
  }
  ...
  do {
    ...
    if (ret != ISAL_DECOMP_OK) {
      std::cerr << "igzip: Error encountered while decompressing file "
                << infile_name << " : " << ret << "\n";
      goto decompress_file_cleanup;
    }
  ...
  while (state.avail_in > 0 && state.next_in[0] == 31) {
    ...
    do {
      ...
      ret = isal_inflate(&state);
      if (ret != ISAL_DECOMP_OK) {
        std::cerr << "igzip: Error while decompressing extra concatenated  "
                     "gzip files on "
                  << infile_name << "\n";
        goto decompress_file_cleanup;
      }
  ...
  if (state.block_state != ISAL_BLOCK_FINISH)
    std::cerr << "igzip: Error " << infile_name
              << " does not contain a complete gzip file\n";
  else
    success = 1;
