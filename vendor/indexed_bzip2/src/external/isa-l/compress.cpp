...
std::vector<char> compressWithISAL(const std::vector<char> &toCompress) {
  /* > Parameter avail_out must be large enough to fit the entire compressed
   * output. > Max expansion is limited to the input size plus the header size
   * of a stored/raw block. */
  std::vector<char> compressed(toCompress.size() + 1000);

  isal_zstream stream;
  isal_deflate_stateless_init(&stream);
  stream.next_in =
      reinterpret_cast<uint8_t *>(const_cast<char *>(toCompress.data()));
  ...
  if (stream.avail_out >= compressed.size()) {
    throw std::logic_error("Something went wrong. Avail_out should be smaller "
                           "than it was before.");
  }
...
int main(int argc, char **argv) {
  ...
  std::cerr << "Compressed " << toCompress.size() << " B down to "
            << compressed.size() << " B\n";
