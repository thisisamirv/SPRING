...
public:
  ...
  void emplaceShared(const size_t encodedBlockOffset,
    ...
    if (m_windows.empty()) {
      ...
      /* Last value is smaller, so it is given that there is no collision and we
       * can "append" the new value with a hint in constant time. This should be
       * the common case as windows should be inserted in order of the offset!
       */
       ...
       * We would have to test at least for empty windows being reinserted
       * because it happens in the common use case of opening a RapidgzipFile
       * object, which inserts the very first block, and then loading an index!
       * Further windows might also be inserted if the file is opened in a
       * buffered manner, which could insert windows up to the buffer size
       * without having read anything yet. Comparing the decompressed contents
       * will also fail when overwriting non-compressed windows with
       * asynchronically compressed and made-sparse windows. I am not even sure
       * anymore why I did want to test for changes. I guess it was a
       * consistency check, but it becomes too complex and error-prone now. */
  ...
  [[nodiscard]] SharedWindow get(size_t encodedOffsetInBits) const {
    /* Note that insertions might invalidate iterators but not references to
     * values and especially not the internal pointers of the vectors we are
     * storing in the values. Meaning, it is safe to simply return a WindowView
  ...
  [[nodiscard]] bool operator==(const WindowMap &other) const {
    ...
    for (const auto &[offset, window] :
         m_windows) { // NOLINT(readability-use-anyofallof)
...
private:
  ...
  /**
   * As soon as a window for an encoded block offset has been inserted it must
   * contain valid data, i.e., actual data, often exactly
   * deflate::MAX_WINDOW_SIZE, or either it is empty because no window is
   * required because we are at the start of a gzip stream! Initially, this was
   * std::unordered_map to ensure O(1) insertion speed. However, this makes
   * releaseUpTo take a possibly very long time after an index has been
   * imported. Using std::map with insertion/emplace hint also can achieve O(1)
   * and according to benchmarks can even be ~20% faster than unordered_map when
   * all those emplace hints are perfect. This should normally be the case
   * because windows should be inserted in order as are the offsets, i.e., the
   * hint can always be end():
   */
  Windows m_windows;
};
} // namespace rapidgzip
