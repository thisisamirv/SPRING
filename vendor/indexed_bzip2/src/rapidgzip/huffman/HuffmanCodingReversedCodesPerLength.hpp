 ...
 * - During initialization, it stores all symbols (for each code) sorted by
 * length in an array and also stores offsets to jump to subarrays of symbols
 * with a given code length. The subarray size is given by the next offset. This
 * avoids going over all elements all the time and also already implements usage
 * of maximum-sized and manually managed memory chunks by using std::array to
 * avoid heap allocations. IN ADDITION to HuffmanCodingSymbolsPerLength it also
 * stores precalcualted reversed codes per length similar to the symbols. This
 * avoids having to read bit-by-bit up to m_minCodeLength.
 * - During decoding, it reads m_minCodeLength at once and after that one by one
 * and for each intermediary, checks whether there is a matching code with the
 * current length in the corresponding subarray.
 */
...
protected:
  ...
  initializeCodingTable(const VectorView<BitCount> &codeLengths,
    ...
    /* Calculate cumulative frequency sums to be used as offsets for each
     * code-length for the code-length-sorted alphabet vector. */
    size_t sum = 0;
...
public:
  ...
  initializeFromLengths(const VectorView<BitCount> &codeLengths) {
    ...
    BaseType::initializeMinimumCodeValues(
...
protected:
   ...
   *   CL=3   CL=4  CL=5
   * @endverbatim
   * The starting index for a given code length (CL) can be queried with
   * m_offsets.
   */
  alignas(8) std::array<Symbol, MAX_SYMBOL_COUNT> m_symbolsPerLength{};
  alignas(8) std::array</* reversed code */ HuffmanCode,
                        MAX_SYMBOL_COUNT> m_codesPerLength{};

  /* +1 because it stores the size at the end as well as 0 in the first element,
   * which might be redundant but fast. Stores m_maxCodeLength - m_minCodeLength
   * offsets +1 size. The array is oversized because else we would have to use a
   * std::vector with costly heap-allocations. */
