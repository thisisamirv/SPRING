...
public:
  ...
  /* Either ceil(log2(MAX_SYMBOL_COUNT)) or std::numeric_limits<Symbol>::digits
   * - ceil(log2(MAX_CODE_LENGTH)), but the ceil o log2 composition is hard to
   * calculate at compile-time. floor o log2 would be position of first non-zero
   * bit. */
...
public:
  /**
   * @note Reusing this struct by calling this method multiple times is allowed.
   * All members will be reinitialized.
   */
  constexpr Error
  initializeFromLengths(const VectorView<BitCount> &codeLengths) {
     ...
     *  - this implementation is unable to detect invalid encoded symbols, which
     * only are possible for the single symbol case.
     *  - this implementation is only used for the literal encoding right now
     * for which a single symbol makes no sense because then that symbol would
     * have to be the end of block symbol and why should there be empty dynamic
     * blocks? Those are very space-wasting.
     */
     ...
     * @endverbatim
     * Urgh, it is difficult to find a stable formula for the optimal double
     * cache size :/. I might need the expected deflate block size as well as
     * take into account the whole code length statistics holistically. E.g., 2
     * * m_minCodeLength allows for 2 min code length values to be cached. 2 *
     * m_minCodeLength + 1 allows the above **and** combinations of the next
     * most frequent with the most- frequent, which should still be pretty
     * common. 2 * m_minCodeLength + 2 probably has a bad performance because
     * the accounted cases become increasingly rare, e.g., it includes the case
     * of two less common symbols, which are expected exponentially less than a
     * single one. Furthermore, this optimum might change and maybe even become
     * more stable, when implementing two-staged lookup. For 2 * minCodeLength +
     * 1: readDynamicHuffmanCoding : 0.000170131 s (15.3217 %) readData :
     * 0.00094026 s (84.6783 %) minCodeLength is almost always 6 for base64 data
     * in my tests but maxCodeLength can go up to 9.
     */
    m_cachedBitCount =
    ...
    /* Measuring the time here instead of before the if above, leads to a 40%
     * performance penalty!!?? I.e., measuring more code, yields faster times
     * than measuring only a part of the whole... Modern complex processors and
     * compiler optimizations are fun. */
    /* const auto t0 = now() */
    ...
    // Fast
    // const auto nElements = m_doubleCodeCache.size() * sizeof(
    // m_doubleCodeCache[0] ) / sizeof(uint64_t); for ( size_t i = 0; i <
    // nElements; ++i ) {
    //    reinterpret_cast<uint64_t*>( m_doubleCodeCache.data() )[i] =
    //    0xFFFF'FFFF'FFFF'FFFFULL;
    //}
    // Slow
    // std::memset( m_doubleCodeCache.data(), 0xFF, m_doubleCodeCache.size() *
    // sizeof( m_doubleCodeCache[0] ) );
    ...
    for (size_t i = 0; i < size;) {
      ...
      /* Do not greedily decode two symbols at once if the first symbol is a
       * special deflate LZ77 symbol, which will consume some of the next bits!
       */
      if ((length + this->m_minCodeLength > m_cachedBitCount) ||
        ...
        for (size_t i2 = 0; i2 < size;) {
          ...
          /* Store only one symbol if the Huffman code of the second would be
           * truncated because of the limited bit count for the cache. */
          ...
          if (totalLength > m_cachedBitCount) {
            ...
            /* Using SIMD for this loop actually worsens timings. Probably too
             * short or because of the necessary code rearrangement for the
             * while condition for the required canonical form. */
    ...
    const std::function<void(uint16_t, uint8_t)> fillCache =
        /**
         * @param mergedCode contains one or multiple Huffman codes in the @ref
         * mergedCodeLength lowest bits.
         * @param symbolsOffset contains the index pointing to the symbol vector
         * in @ref m_bitFieldCacheStorage represented by mergedCode. This is
         * used to avoid reinserting the same symbol vector for all garbage bits
         * for further Huffman codes not fitting inside CACHED_BIT_COUNT
         */
          ...
          for (BitCount k = 0;
            ...
            for (HuffmanCode subIndex = 0;
              ...
              if (mergedCodeLength + length <= CACHED_BIT_COUNT) {
                ...
                /* The check that m_bitFieldCache[newMergedCode] does not
                 * already have data is necessary because all bits higher than
                 * CACHED_BIT_COUNT in newMergedCode are zeroed out, which can
                 * result in duplicates. However, preemptively stopping the
                 * iteration when the total code length has been exceeded would
                 * also be wrong because the Huffman tree is kinda traversed in
                 * a depth-first order while we are interested in the
                 * breadth-first values up to a threshold depth. Because the
                 * depth is now limited to 2, this should be relatively rare!
                 */
  ...
  decode(gzip::BitReader &bitReader) const {
    ...
    try {
      ...
      /* Casting value to int improves speed ~2% probably because of the relaxed
       * bound-checking because signed integer overflow is undefined behavior
       * (and therefore can be assumed to never happen by the compiler). */
...
private:
  ...
  /* note that Symbol is uint16_t but MAX_SYMBOL_COUNT = 512 only requires 9
   * bits, i.e., we have 7 unused bits, which can be used to store the code
   * length, which only requires ceil(log2(15)) = 4 bits, or 5 bits because we
   * want to store the code length sum for both symbols in only the first
   * symbol. Using std::array<std::array<Symbol, 2>, ( 1UL << CACHED_BIT_COUNT
   * )> instead of a one-dimensional array with the same size reduces speed for
   * base64.gz by 10%! */
  alignas(
