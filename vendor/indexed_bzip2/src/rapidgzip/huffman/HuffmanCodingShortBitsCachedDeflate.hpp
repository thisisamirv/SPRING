...
/**
 * This started as a copy of @ref HuffmanCodingShortBitsCached, however, it
 * incorporates deflate-specific things:
 *  - Default arguments for HuffmanCode and Symbol types as well as assuming
 * REVERSE_BITT=true.
 */
...
public:
  ...
  initializeFromLengths(const VectorView<BitCount> &codeLengths,
    ...
    for (size_t symbol = 0; symbol < codeLengths.size(); ++symbol) {
      ...
      if (symbol <= 255) {
      ...
      } else if (UNLIKELY(symbol == END_OF_BLOCK_SYMBOL /* 256 */))
          [[unlikely]] {
        ...
        insertIntoCacheWithDistance(reversedCode, cacheEntry, distanceHC,
                                    ...
                                    cacheEntry.bitsToSkip);
      } else if (symbol < 285U) {
        const auto lengthCode = static_cast<uint8_t>(symbol - 261U);
        const auto extraBitCount = lengthCode / 4; /* <= 5 */
        /* Loop over all possible extra bits or skip filling if it does not fit
         * into cache. We need left-over bits for the extra bits and at least
         * one extra bit for the distance code, likely even more but that will
         * be tested inside @ref insertIntoCacheWithDistance. */
        if (length + extraBitCount + 1 <= m_lutBitsCount) {
          ...
          for (uint8_t extraBits = 0; extraBits < (1U << extraBitCount);
            ...
            insertIntoCacheWithDistance(reversedCode | (extraBits << length),
                                        cacheEntry, distanceHC,
                                        static_cast<uint8_t>(symbol - 257U),
                                        cacheEntry.bitsToSkip - extraBitCount);
          }
        } else {
  ...
  template <typename BitReader, typename DistanceHuffmanCoding>
  [[nodiscard]] forceinline CacheEntry
  decode(BitReader &bitReader, const DistanceHuffmanCoding &distanceHC) const {
    try {
      ...
      if (cacheEntry.distance == 0xFFFEU) {
        return interpretSymbol(bitReader, distanceHC,
                               cacheEntry.symbolOrLength + 257U);
      }
      return cacheEntry;
       ...
       * length. */
      return interpretSymbol(bitReader, distanceHC,
                             BaseType::decode(bitReader).value());
    }
  }
...
private:
    ...
    for (BitCount k = 0; k <= this->m_maxCodeLength - this->m_minCodeLength;
      ...
      if (minCode <= code) {
        ...
        if (subIndex < m_offsets[k + 1]) {
          return interpretSymbol(bitReader, distanceHC,
                                 this->m_symbolsPerLength[subIndex]);
        }
      }
  ...
  template <typename BitReader, typename DistanceHuffmanCoding>
  [[nodiscard]] forceinline constexpr CacheEntry
  interpretSymbol(BitReader &bitReader, const DistanceHuffmanCoding &distanceHC,
                  Symbol symbol) const {
    CacheEntry cacheEntry{};
    ...
    const auto [distance, error] =
        getDistance(CompressionType::DYNAMIC_HUFFMAN, distanceHC, bitReader);
    if (error != Error::NONE) {
      throw error;
    }
  ...
  template <typename DistanceHuffmanCoding>
  forceinline void insertIntoCacheWithDistance(
      HuffmanCode reversedCode, const CacheEntry &cacheEntry,
      const DistanceHuffmanCoding &distanceHC, const uint8_t lengthSymbol,
    ...
    for (auto paddedCode = reversedCode; paddedCode <= maximumPaddedCode;
      ...
      const auto freeBits =
          (paddedCode >> length) &
          nLowestBitsSet<HuffmanCode>(distanceHC.maxCodeLength());
      const auto &[distanceCodeLength, symbol] =
          distanceHC.codeCache()[freeBits];
      if ((distanceCodeLength == 0) || (distanceCodeLength > fillerBitCount) ||
          (symbol > 29U)) {
        m_codeCache[paddedCode] = cacheEntry;
      ...
      if (symbol <= 3U) {
        ...
        if (static_cast<uint16_t>(distanceCodeLength + extraBitCount) <=
            fillerBitCount) {
          const auto extraBits = (paddedCode >> (length + distanceCodeLength)) &
                                 nLowestBitsSet<HuffmanCode>(extraBitCount);
          m_codeCache[paddedCode] = cacheEntry;
          m_codeCache[paddedCode].bitsToSkip =
              length + distanceCodeLength + extraBitCount;
