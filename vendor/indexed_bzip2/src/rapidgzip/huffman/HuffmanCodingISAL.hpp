...
public:
  ...
  initializeFromLengths(const VectorView<uint8_t> &codeLengths) {
    ...
    std::array<uint32_t, LIT_LEN_ELEMS + 2>
        code_list{}; /* The +2 is for the extra codes in the static header */
...
private:
  static void write_huff_code(huff_code *const huff_code, uint32_t const code,
    ...
    huff_code->code_and_length =
        code |
        (length << 24U); // NOLINT(cppcoreguidelines-pro-type-union-access)
  }
...
public:
  ...
  decode(gzip::BitReader &bitReader) const {
    ...
    /**
     * I have also tested other fixed peek sizes, such as 24 and 48 but they all
     * were significantly slower. It seems that 32 is most amenable when it
     * comes to refilling the bit buffer.
     * @verbatim
     * peek<32>                                      : 431.67 | 439.15 +- 0.09 |
     * 442.87 peek<ISAL_DECODE_LONG_BITS(12)> and peek<...> : 419.05 | 424.46 +-
     * 0.07 | 427.36
     * @endverbatim
     */
    uint64_t nextBits{0};
    try {
      ...
      /* This should only happen in the error case or for raw deflate streams
       * because those don't have any footer acting as a kind of buffer to
       * ensure that peek always works. */
    ...
    const auto next12Bits =
        nextBits &
        N_LOWEST_BITS_SET_LUT<uint64_t>[ISAL_DECODE_LONG_BITS /* 12 */];
    ...
    /* If a symbol is not found, do a lookup in the long code list starting from
     * the hint in next_sym. > If bit 15 is set, the i  corresponds to the first
     * DECODE_LOOKUP_SIZE bits of a Huffman code which has > length longer than
     * DECODE_LOOKUP_SIZE. In this case, bits 0 through 8 > represent an offset
     * into long_code_lookup table and bits 9 through 12 > represent the maximum
     * length of a Huffman code starting with the bits in the index i. Ergo, the
     * maximum length itself is stored in 4 bits and therefore cat at most
     * be 16. And probably, because it never is double-cached in the long table,
     * it can at most be deflate::MAX_CODE_LENGTH = 15. In practice, I have
     * encountered peek sizes of up to 20. So, maybe it also includes the
     * distance count? With a distance code, it should only need up to 13
     * further bits, so 32 bits in the peek above should still be sufficient.
     * But as I am not 100% sure, I cannot get rid of the fallback to
     * bitReader.peek. It probably will never be called and the if also does not
     * seem to add any measurable overhead, probably because branch prediction
     * is never wrong.
     */
