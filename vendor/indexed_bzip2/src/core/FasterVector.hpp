...
#include <cstddef>
#include <cstring>
#if defined(LIBRAPIDARCHIVE_WITH_RPMALLOC) && __has_include(<rpmalloc.h>)
#include <rpmalloc.h>
#else
#undef LIBRAPIDARCHIVE_WITH_RPMALLOC
#include <cstdlib>
#endif
#include <iterator>
#include <limits>
...
[[maybe_unused]] inline void clangd_unused_fix_FasterVector() {
  ...
  (void)sizeof(std::numeric_limits<int>);
  (void)sizeof(std::bad_array_new_length);
  (void)sizeof(std::logic_error);
  (void)sizeof(std::out_of_range);
  (void)sizeof(std::true_type);
  (void)sizeof(std::reverse_iterator<int *>);
  (void)sizeof(std::vector<int>);
  (void)sizeof(std::input_iterator_tag);
  (void)sizeof(std::iterator_traits<int *>);
  ...
  (void)&rpmalloc;
#else
  (void)std::realloc(nullptr, 0);
  (void)std::free(nullptr);
#endif
}
#ifdef LIBRAPIDARCHIVE_WITH_RPMALLOC
class RpmallocInit {
public:
  RpmallocInit() { rpmalloc_initialize(nullptr); }
...
inline void *rpmalloc_ensuring_initialization(size_t nBytes = 0) {
...
template <typename ElementType> class RpmallocAllocator {
public:
...
#ifdef LIBRAPIDARCHIVE_WITH_RPMALLOC
template <typename T> using FasterVector = std::vector<T, RpmallocAllocator<T>>;
#else
template <typename T> using FasterVector = std::vector<T>;
#endif

#else

[[nodiscard]] constexpr bool isPowerOf2(size_t value) {
...
public:
  static_assert(
      isPowerOf2(sizeof(T)),
      "Size of element type must be a power of 2 for alignment purpose!");
...
public:
  ...
  FasterVector &operator=(FasterVector &&other) noexcept {
    m_data = other.m_data;
  ...
  void resize(size_t size, const std::optional<T> &initialValue = {}) {
    if (size > m_size) {
      reserve(size);
      if (initialValue) {
  ...
  [[nodiscard]] constexpr auto rend() noexcept {
    return std::reverse_iterator<T *>(m_data);
  }
  ...
  template <typename InputIt, typename = RequireInputIterator<InputIt>>
  void insert(const T *position, const InputIt &inputBegin,
    ...
    if ((positionDistance < 0) ||
        (static_cast<size_t>(positionDistance) > m_size)) {
      throw std::logic_error("The insertion position must be inside the valid "
                             "range of this vector or end()!");
    }
    ...
    if (positionIndex < m_size) {
      std::memmove(m_data + positionIndex + inputSize, m_data + positionIndex,
                   inputSize * sizeof(T));
    }
  ...
  [[nodiscard]] bool operator==(const FasterVector &other) const {
...
private:
  ...
  void reallocate(const size_t newCapacity) {
    ...
    if (newCapacity == 0) {
      ...
      if (m_data == nullptr) {
        m_data = static_cast<T *>(
            rpmalloc_ensuring_initialization(newCapacity * sizeof(T)));
      } else {
...
[[nodiscard]] bool operator==(const FasterVector<T> &lhs,
                              const std::vector<T, Alloc> &rhs) {
  return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}

template <class T, class Alloc>
[[nodiscard]] bool operator==(const std::vector<T, Alloc> &lhs,
