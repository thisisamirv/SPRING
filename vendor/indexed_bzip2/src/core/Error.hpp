...
[[nodiscard]] inline std::string toString(Error error) {
  switch (error) {
  case Error::EOF_ZERO_STRING:
    return "End of file encountered when trying to read zero-terminated "
           "string!";
  case Error::EOF_UNCOMPRESSED:
    return "End of file encountered when trying to copy uncompressed block "
           "from file!";
  case Error::EMPTY_ALPHABET:
    return "All code lengths are zero!";
  case Error::EXCEEDED_CL_LIMIT:
    return "The number of code lengths may not exceed the maximum possible "
           "value!";
  case Error::EMPTY_INPUT:
    return "Container must not be empty!";
  case Error::EXCEEDED_SYMBOL_RANGE:
    return "The range of the symbol type cannot represent the implied "
           "alphabet!";
  ...
  case Error::INVALID_CODE_LENGTHS:
    return "Constructing a Huffman coding from the given code length sequence "
           "failed!";
...
inline std::ostream &operator<<(std::ostream &out, rapidgzip::Error error) {
  out << toString(error);
  return out;
}
} // namespace rapidgzip
