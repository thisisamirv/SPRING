...
public:
  ...
  /**
   * @return true if all of the memorized last access were sequential. On true,
   * more optimizations may be enabled.
   */
...
/**
 * Similar to @ref FetchNextFixed but the amount of returned subsequent indexes
 * is relative to the amount of consecutive accesses in the memory. If all are
 * consecutive, returns the specified amount to prefetch. Else, if there are
 * only random access in memory, then it will return nothing to prefetch to
 * avoid wasted computation. Between, interpolate exponentially, i.e., for a
 * memory size of 3 and 4 requested prefetch indexes: 1 consecutive pair -> 1 2
 * consecutive pair -> 2 3 consecutive pair -> 4
 */
...
public:
  ...
  void fetch(size_t index) override {
    ...
    /* Ignore duplicate accesses, which in the case of bzip2 blocks most likely
     * means that the caller reads only small parts from the block per call. */
  ...
  [[nodiscard]] bool isSequential() const noexcept override {
    /* Returning true for the empty case and only one element or very few is
     * desired behavior. */
  ...
  extrapolate(const Iterator rangeBegin, const Iterator rangeEnd,
    ...
    if (size == 1) {
      /* This enables parallel sequential decoding with only a single cache
       * miss. I think this is an often enough encountered use case to have this
       * special case justified. */
      std::vector<size_t> toPrefetch(maxAmountToPrefetch);
      std::iota(toPrefetch.begin(), toPrefetch.end(), *rangeBegin + 1);
      return toPrefetch;
    }

    /* Count adjacent decreasing elements (because the most recent is on the
     * left!). */
  ...
  /**
   * Updates indexes such that the given indexes is interpreted as four separate
   * indexes from now on. This also shifts all larger indexes accordingly and
   * might evict some indexes because of the new split ones.
   */
...
/**
 * Similar to @ref FetchNextAdaptive but it is able to detect multiple
 * interleaved consecutive accesses as might happen for parallel processes
 * accessing the cache. Detection works by sorting all last indexes and looking
 * for consecutive sequences there and return predictions for them at the same
 * time.
 */
...
public:
  ...
  prefetch(size_t maxAmountToPrefetch) const override {
    ...
    const auto extrapolateSubsequence = [&, this](const auto begin,
                                                  const auto end) {
      /* We only have the deduplicated and sorted indexes. We need to recheck
       * with the real saved indexes whether it is an increasing sequence, else
       * a perfect backward pattern might be detected as a forward pattern
      ...
      /* For the very first prefetches, fewer than the memory size, extrapolate
       * them fully for faster first-time decoding. */
