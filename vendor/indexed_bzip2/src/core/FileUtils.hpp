...
[[nodiscard]] inline bool stdinHasInput() { return isatty(STDIN_FILENO) == 0; }
...
[[nodiscard]] inline std::ios_base::seekdir toSeekdir(int origin) {
  switch (origin) {
  case SEEK_SET:
...
[[nodiscard]] inline const char *originToString(int origin) {
  switch (origin) {
  case SEEK_SET:
    return "SEEK_SET";
  case SEEK_CUR:
...
inline bool fileExists(const std::string &filePath) {
  return std::ifstream(filePath, std::ios_base::in | std::ios_base::binary)
      .good();
}

inline size_t fileSize(const std::string &filePath) {
  return std::filesystem::file_size(filePath);
}

inline size_t filePosition(std::FILE *file) {
  if (file == nullptr) {
...
inline void fileSeek(std::FILE *file, long long int offset, int origin) {
  if (file == nullptr) {
...
#else
  if (offset >
      static_cast<long long int>(std::numeric_limits<long int>::max())) {
    throw std::out_of_range(
        "std::fseek only takes long int, try compiling for 64 bit.");
  }

  const auto returnCode =
      std::fseek(file, static_cast<long int>(offset), origin);
#endif

  if (returnCode != 0) {
    std::stringstream message;
    message << "Seeking to " << offset << " from origin "
            << originToString(origin) << " failed with code: " << returnCode
            << ", " << std::strerror(errno) << "!";
...
struct unique_file_descriptor {
  explicit unique_file_descriptor(int fd) : m_fd(fd) {}

  ~unique_file_descriptor() { close(); }
  ...
  [[nodiscard]] constexpr int operator*() const noexcept { return m_fd; }
...
inline unique_file_ptr make_unique_file_ptr(char const *const filePath,
                                            char const *const mode) {
  if ((filePath == nullptr) || (mode == nullptr) ||
...
inline unique_file_ptr throwingOpen(const std::string &filePath,
                                    const char *mode) {
  if (mode == nullptr) {
  ...
  if (file == nullptr) {
    ...
    msg << "Opening file '" << filePath << "' with mode '" << mode
        << "' failed!";
...
inline unique_file_ptr throwingOpen(int fileDescriptor, const char *mode) {
  if (mode == nullptr) {
    throw std::invalid_argument("Mode must be a C-String and not null!");
  }
  ...
  if (file == nullptr) {
    std::stringstream msg;
    msg << "Opening file descriptor " << fileDescriptor << " with mode '"
        << mode << "' failed!";
    throw std::invalid_argument(std::move(msg).str());
  }
...
[[nodiscard]] Container readFile(const std::string &fileName) {
  Container contents(fileSize(fileName));
  const auto file = throwingOpen(fileName, "rb");
  const auto nBytesRead = std::fread(contents.data(), sizeof(contents[0]),
                                     contents.size(), file.get());
...
inline unique_file_ptr throwingOpen(const std::filesystem::path &filePath,
                                    const char *mode) {
  return throwingOpen(filePath.string(), mode);
}

inline size_t fileSize(const std::filesystem::path &filePath) {
  return fileSize(filePath.string());
}

template <typename Container = std::vector<char>>
[[nodiscard]] Container readFile(const std::filesystem::path &filePath) {
...
inline size_t fileSize(const int fileDescriptor) {
  ...
  if (result == -1) {
    std::stringstream message;
    message << "Failed to get file size because of: " << strerror(errno) << " ("
            << errno << ")";
    throw std::runtime_error(std::move(message).str());
  }
...
writeAllSpliceUnsafe([[maybe_unused]] const int outputFileDescriptor,
  ...
  while (dataToSplice.iov_len > 0) {
    ...
    dataToSplice.iov_base =
        reinterpret_cast<char *>(dataToSplice.iov_base) + nBytesWritten;
...
writeAllSpliceUnsafe([[maybe_unused]] const int outputFileDescriptor,
  ...
  for (size_t i = 0; i < dataToWrite.size();) {
    ...
    if (nBytesWritten < 0) {
      ...
      std::stringstream message;
      message << "Failed to write all bytes because of: " << strerror(errno)
              << " (" << errno << ")";
      throw std::runtime_error(std::move(message).str());
    }
    ...
    if ((i < dataToWrite.size()) && (nBytesWritten > 0)) {
      ...
      const auto remainingData =
          reinterpret_cast<char *>(iovBuffer.iov_base) + nBytesWritten;
      const auto errorCode =
          writeAllSpliceUnsafe(outputFileDescriptor, remainingData, size);
      if (errorCode != 0) {
        return errorCode;
      }
      ++i;
...
private:
  ...
  void account(const std::shared_ptr<T> &splicedData,
    ...
    if (!m_splicedData.empty() &&
      ...
      m_splicedData.emplace_back(splicedData, splicedData.get(),
                                 dataToWriteSize);
    }
  ...
  explicit SpliceVault(int fileDescriptor)
      : m_fileDescriptor(fileDescriptor),
        m_pipeBufferSize(fcntl(fileDescriptor, F_GETPIPE_SZ)) {}

  [[nodiscard]] VaultLock lock() { return VaultLock(m_mutex); }
...
[[nodiscard]] inline int writeAllToFd(const int outputFileDescriptor,
  ...
  for (uint64_t nTotalWritten = 0; nTotalWritten < dataToWriteSize;) {
    const auto *const currentBufferPosition =
        reinterpret_cast<const uint8_t *>(dataToWrite) + nTotalWritten;

    const auto nBytesToWritePerCall = static_cast<unsigned int>(std::min(
        static_cast<uint64_t>(std::numeric_limits<unsigned int>::max()),
        dataToWriteSize - nTotalWritten));

    const auto nBytesWritten = ::write(
        outputFileDescriptor, currentBufferPosition, nBytesToWritePerCall);
    if (nBytesWritten <= 0) {
      return errno;
    }
...
#ifdef HAVE_IOVEC
inline void pwriteAllToFd(const int outputFileDescriptor,
                          const void *const dataToWrite,
                          const uint64_t dataToWriteSize,
  ...
  for (uint64_t nTotalWritten = 0; nTotalWritten < dataToWriteSize;) {
    ...
    if (nBytesWritten <= 0) {
      ...
      message << "Unable to write all data to the given file descriptor. Wrote "
              << nTotalWritten << " out of " << dataToWriteSize << " ("
              << strerror(errno) << ").";
      throw std::runtime_error(std::move(message).str());
    }
...
public:
  ...
  [[nodiscard]] bool writingToStdout() const noexcept {
