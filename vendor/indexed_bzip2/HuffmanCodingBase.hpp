...
protected:
  ...
  initializeMinMaxCodeLengths(const VectorView<BitCount> &codeLengths) {
    ...
    /* A maximum code length of 0 is valid! It happens when encoding this with
     * pigz: python3 -c 'import sys; sys.stdout.buffer.write(bytes(range(256)))'
     * | pigz > 0CL.pigz */
  ...
  checkCodeLengthFrequencies(const CodeLengthFrequencies &bitLengthFrequencies,
     ...
     * 112/2 = 200
     *      -> Note that the rest, i.e., 200-152 = 48 = 24*2 values cannot be
     * used because the tree ends in the lower level for code length 7! fixed
     * tree has 279 - 256 + 1 = 24 symbols of length 7
     *   -> can store 128 - 152 / 2 - 112 / 4 = 24
     *      -> Exactly the amount actually being used!
     * -> The tree is full utilized on all levels! I think this makes it
     * possible to do stronger checks! The only reasonable case for unused tree
     * nodes is if there is only one letter. E.g., consider a single letter,
     * which only uses half the tree with one bit. But consider 3 letters.
     * Making all of CL 2 makes no sense, because the third could have been one
     * bit shorter without any problems.
     *      -> this basically corresponds to the check that the number of
     * letters with the longest code length should be even! I think with this,
     * it is always possible to exactly use up all leaf nodes for any alphabet,
    ...
    for (int bitLength = m_minCodeLength; bitLength <= m_maxCodeLength;
      ...
      unusedSymbolCount *=
          2; /* Because we go down one more level for all unused tree nodes! */
    }
  ...
  initializeMinimumCodeValues(CodeLengthFrequencies &bitLengthFrequencies) {
    ...
    /* minCodeLength might be zero for empty deflate blocks as can happen when
     * compressing an empty file! */
...
public:
  /**
   * Creates a Huffman coding for the implicit alphabet (0, ...,
   * codeLengths.size() - 1) using the respectively given code lengths for the
   * Huffman codes.
   *
   * A Huffman tree gives a variable-length binary code for each leaf node
   * representing a symbol. Further restrictions on Huffman tree by deflate:
   *  - Maximum code length: 15 because the deflate code length alphabet only
   * goes from 0 to 15.
   ...
   * Huffman tree can be exactly specified as a vector, whose elements represent
   * the ordered alphabet's symbol's code lengths, e.g, for alphabet ABCD:
   * (2,1,3,3) -> 0b10, 0b0, 0b110, 0b111
   */
  constexpr Error
  initializeFromLengths(const VectorView<BitCount> &codeLengths) {
    ...
    initializeMinimumCodeValues(
        bitLengthFrequencies); // resets bitLengthFrequencies[0] to 0!
