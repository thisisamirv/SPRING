...
public:
  ...
  /**
   * @return the next match and the requested bytes or
   * std::numeric_limits<size_t>::max() if at end of file.
   */
  [[nodiscard]] size_t find() override;

private:
  /**
   * The worker pushes found offsets during which it locks the mutex and the
   * reader locks and pops from the queue. After the worker has finished, which
   * can be queried with the future, and when the found offsets have all been
   * read, this struct can be deleted or reused. The worker thread sets finished
   * and notifies with the condition variable. It also notifies when pushing to
   * the queue.
   */
...
private:
  ...
  chunkSize(size_t const fileBufferSizeBytes, size_t const requestedBytes,
    ...
    /* This implementation has the limitation that it might at worst try to read
     * as many as bitStringSize bits from the buffered chunk. It makes no sense
     * to remove this limitation. It might slow things down. */
    const auto result = std::max(
        fileBufferSizeBytes,
        static_cast<size_t>(ceilDiv(bitStringSize, 8)) * parallelization);
    /* With the current implementation it is impossible to have a chunk size
     * smaller than the requested bytes and have it work for non-seekable
     * inputs. In the worst case, the bit string is at the end, so we have to
     * read almost everything of the next chunk. */
    return std::max(result, requestedBytes);
  }

  /**
   * Calls findBitStrings and handles all the parallel synchronization stuff
   * like sending the results to the reading thread through the result buffer.
   * When it is finished, it will return std::numeric_limits<size_t>::max() to
   * signal that the future can be waited for.
   *
   * @param buffer pointer data to look for bitstrings. [buffer, buffer+size)
   * will be searched.
   * @param firstBitsToIgnore This will effectively force matches to only be
   * returned if foundOffset >= firstBitsToIgnore
   * @param bitOffsetToAdd All found offsets relative to @p buffer should add
   * this in order to return the global bit offsets of interest.
   */
  static void
...
private:
  /** Return at least this amount of bytes after and including the found bit
   * strings. */
 ...
 *   2. Use the serial BitStringFinder in parallel on equal-sized sized sub
 * chunks.
 *   3. Filter out results we already could have found in the chunk before if
 * more than bitStringSize-1 bits were loaded from it.
 *   4. Translate the returned bit offsets of the BitStringFinders to global
 * offsets.
 *   5. Copy requested bytes after match into result buffer.
 *   6. Load the next chunk plus at least the last bitStringSize-1 bits from the
 * chunk before.
 *   7. Use that new chunk to append more of the requested bytes after matches
 * to the result buffer. More than one chunk should not be necessary for this!
 * This is ensured in the chunkSize method.
 */
template <uint8_t bitStringSize>
size_t ParallelBitStringFinder<bitStringSize>::find() {
  ...
  while (!BaseType::eof() || !m_threadResults.empty()) {
    /* Check whether there are results available and return those. Take care to
    ...
    while (!m_threadResults.empty()) {
      ...
      while (!result.foundOffsets.empty() || result.future.valid()) {
        ...
        /* Wait for thread to finish or push new results. Note that this may
         * hang if the worker thread crashes because the predicate check is only
         * done on condition variable notifies and on spurious wakeups but those
         * are not guaranteed. */
    ...
    /* Constructor might fill buffer already making a buffer refill unnecessary
     * the first time! */
    if (this->bufferEof()) {
    ...
    /* For very sub chunk sizes, it is more sensible to not parallelize them
     * using threads! */
    ...
    for (; !this->bufferEof();
      ...
      /* Try to seek m_movingBitsToKeep back in order to find bit strings going
       * over the previous border. On the very first iteration after refilling
       * the buffer, the number of read bits might be non-zero but smaller than
       * the bit string to find because BitReader prepends some bytes of the
       * last buffer in order to find bit strings across buffer boundaries. In
       * this case, i.e., in the first iteration, We do not need to seek back by
       * m_movingBitsToKeep only after the first chunk is this necessary just
      ...
      // std::cerr << "  Find from offset " << bufferOffsetInBytes << "B " <<
      // subChunkOffsetInBits << "b "
      ...
      result.future = m_threadPool.submit([=, &result]() {
        workerMain(
            ...
            subChunkSizeInBytes, subChunkOffsetInBits, this->m_bitStringToFind,
