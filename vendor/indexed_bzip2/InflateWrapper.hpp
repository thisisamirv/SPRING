...
namespace rapidgzip {
template <typename InflateWrapper, typename Container>
[[nodiscard]] Container inflateWithWrapper(
    const Container &toDecompress, const std::optional<size_t> decompressedSize,
  ...
  gzip::BitReader bitReader(std::make_unique<BufferViewFileReader>(
      toDecompress.data(), toDecompress.size()));
  ...
  case FileType::ZLIB:
    ...
    break;
  default:
    throw std::invalid_argument(std::string("Unsupported file type: ") +
                                toString(fileType));
  }
  ...
  while (true) {
    ...
    const auto [nBytesRead, footer] = inflateWrapper.readStream(
        result.data() + oldSize, result.size() - oldSize);
