...
struct distortion_t *generate_distortion_matrix(uint8_t symbols, int type) {
  ...
  case DISTORTION_CUSTOM:
    printf("Custom distortion matrices should be allocated with "
           "gen_custom_distortion() instead.\n");
    exit(1);
  default:
...
struct distortion_t *gen_manhattan_distortion(uint8_t symbols) {
  ...
  for (x = 0; x < symbols; ++x) {
    for (y = 0; y < symbols; ++y) {
      rtn->distortion[x + y * symbols] = (double)abs((int)x - (int)y);
    }
  }
...
struct distortion_t *gen_mse_distortion(uint8_t symbols) {
  ...
  for (x = 0; x < symbols; ++x) {
    for (y = 0; y < symbols; ++y) {
      double diff = (double)((int)x - (int)y);
      rtn->distortion[x + y * symbols] = diff * diff;
    }
  }
...
struct distortion_t *gen_custom_distortion(uint8_t symbols,
  ...
  while (x < symbols && fgets(line, 1024, fp) != NULL) {
    ...
    if (missing) {
      printf("Warning: one or more entries in the distortion matrix on line %d "
             "were missing",
             x);
