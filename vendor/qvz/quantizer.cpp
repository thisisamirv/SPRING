...
struct quantizer_t *generate_quantizer(struct pmf_t *restrict pmf,
  ...
  // Precompute weighted PMF sums for O(N) distortion calculation
  ...
  while (changed && iter < QUANTIZER_MAX_ITER) {
    ...
    for (j = 0; j < states; ++j) {
      ...
      if (dist->type == DISTORTION_MSE) {
        ...
        // Fallback for Lorentzian or Custom
        for (r = start; r < end; ++r) {
          mse = 0.0;
          for (i = start; i < end; ++i) {
...
/**
 * Calculate the PMF of the output when the given quantizer is used with symbols
 * generated
 * from the given input distribution. The input and output pmf structures cannot
 * be the
