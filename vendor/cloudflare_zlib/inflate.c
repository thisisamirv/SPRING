...
static int updatewindow(z_streamp strm, const Bytef *end, unsigned copy) {
  ...
  if (state->window == Z_NULL) {
    ...
    state->window = (unsigned char FAR *)ZALLOC(
        strm, wsize + CHUNKCOPY_CHUNK_SIZE, sizeof(unsigned char));
    if (state->window == Z_NULL)
      return 1;
...
inf_leave:
    ...
    (defined(INFLATE_CHUNK_SIMD_NEON) || defined(INFLATE_CHUNK_SIMD_SSE2) ||   \
     defined(INFLATE_CHUNK_GENERIC))
...
#endif
    window = (unsigned char FAR *)ZALLOC(source, wsize_padded,
                                         sizeof(unsigned char));
    if (window == Z_NULL) {
      ZFREE(source, copy);
      return Z_MEM_ERROR;
