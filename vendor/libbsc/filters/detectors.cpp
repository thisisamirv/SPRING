...
int bsc_detect_segments_serial(BscSegmentationModel *RESTRICT model,
                               const unsigned char *RESTRICT input, int n) {
  ...
  for (int context = 0; context < ALPHABET_SIZE; ++context) {
    ...
    for (int symbol = 0; symbol < ALPHABET_SIZE; ++symbol) {
      ...
      count += frequency;
      entropy -= bsc_entropy(frequency);
    }
    model->contextsCount[context].right = count;
    entropy += bsc_entropy(count);
  }

  int blockSize = n;

  long long localEntropy = entropy,
            bestEntropy = entropy - (entropy >> 5) - (65536LL * 12 * 1024);
  for (int context = 0, i = 0; i < n; ++i) {
    ...
    localEntropy +=
        bsc_delta(--model->contexts[context].Frequencies[symbol].right);
    localEntropy -=
        bsc_delta(model->contexts[context].Frequencies[symbol].left++);
...
int bsc_detect_segments_parallel(BscSegmentationModel *RESTRICT model0,
                                 BscSegmentationModel *RESTRICT model1,
                                 const unsigned char *RESTRICT input, int n) {
  int globalBlockSize = n;
  long long globalEntropy, globalBestEntropy;
    ...
    if (nThreads == 1) {
...

      {
        if (threadId == 0) {
          ...
          int context =
              (unsigned char)((input[median - 2] << 5) ^ input[median - 1]);
...
#pragma omp single
        {
          ...
          for (int context = 0; context < ALPHABET_SIZE; ++context) {
            ...
            model0->contextsCount[context].right =
                model1->contextsCount[context].left = count;
            entropy += bsc_entropy(count);
          }

          globalEntropy = entropy;
          globalBestEntropy = entropy - (entropy >> 5) - (65536LL * 12 * 1024);
        }
      }

      {
        int localBlockSize = n;
        long long localBestEntropy =
            globalEntropy - (globalEntropy >> 5) - (65536LL * 12 * 1024);
        ...
        if (threadId == 0) {
          ...
          for (int context = 0, i = 0; i < median; ++i) {
            ...
            localEntropy += bsc_delta(
                --model0->contexts[context].Frequencies[symbol].right);
            localEntropy -=
                bsc_delta(model0->contexts[context].Frequencies[symbol].left++);
          ...
          for (int i = n - 1; i >= median; --i) {
            ...
            localEntropy -= bsc_delta(
                model1->contexts[context].Frequencies[symbol].right++);
            localEntropy +=
                bsc_delta(--model1->contexts[context].Frequencies[symbol].left);
...
int bsc_detect_segments_recursive(BscSegmentationModel *model0,
                                  BscSegmentationModel *model1,
  ...
  int leftResult = bsc_detect_segments_recursive(
      model0, model1, input, blockSize, segments, k - 1, features);
  if (leftResult < LIBBSC_NO_ERROR)
    return leftResult;

  int rightResult = bsc_detect_segments_recursive(
      model0, model1, input + blockSize, n - blockSize, segments + leftResult,
      k - leftResult, features);
  if (rightResult < LIBBSC_NO_ERROR)
    return rightResult;

  return leftResult + rightResult;
}

int bsc_detect_segments(const unsigned char *input, int n, int *segments, int k,
                        int features) {
  if (n < DETECTORS_BLOCK_SIZE || k == 1) {
    segments[0] = n;
    return 1;
  ...
  if (BscSegmentationModel *model0 =
          (BscSegmentationModel *)bsc_malloc(sizeof(BscSegmentationModel))) {
    if (BscSegmentationModel *model1 =
            (BscSegmentationModel *)bsc_malloc(sizeof(BscSegmentationModel))) {
      int result = bsc_detect_segments_recursive(model0, model1, input, n,
                                                 segments, k, features);
...
int bsc_detect_contextsorder(const unsigned char *RESTRICT input, int n,
                             int features) {
  int sortingContexts = LIBBSC_NOT_ENOUGH_MEMORY;

  if ((n > DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE) &&
      (features & LIBBSC_FEATURE_FASTMODE)) {
    if (unsigned char *buffer = (unsigned char *)bsc_malloc(
            DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE *
            sizeof(unsigned char))) {
      int blockStride = (((n - DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE) /
                          DETECTORS_NUM_BLOCKS) /
                         48) *
                        48;

      for (int block = 0; block < DETECTORS_NUM_BLOCKS; ++block) {
        memcpy(buffer + block * DETECTORS_BLOCK_SIZE,
               input + block * (DETECTORS_BLOCK_SIZE + blockStride),
               DETECTORS_BLOCK_SIZE);
      }

      sortingContexts = bsc_detect_contextsorder(
          buffer, DETECTORS_NUM_BLOCKS * DETECTORS_BLOCK_SIZE, features);
  ...
  if (unsigned char *RESTRICT buffer =
          (unsigned char *)bsc_malloc(n * sizeof(unsigned char))) {
    if (int *RESTRICT bucket0 = (int *)bsc_zero_malloc(
      ...
      if (int *RESTRICT bucket1 = (int *)bsc_zero_malloc(
        ...
        sortingContexts = (preceding < following) ? LIBBSC_CONTEXTS_PRECEDING
                                                  : LIBBSC_CONTEXTS_FOLLOWING;
...
long long bsc_estimate_reordering(BscReorderingModel *model, int recordSize) {
  ...
  for (int record = 0; record < recordSize; ++record) {
    for (int context = 0; context < ALPHABET_SIZE; ++context) {
      ...
      entropy +=
          (65536LL * 8 * (count < 256 ? count : 256)) + bsc_entropy(count);
    }
  }
  return entropy;
}

int bsc_detect_recordsize(const unsigned char *RESTRICT input, int n,
                          int features) {
  ...
  if (BscReorderingModel *RESTRICT model =
          (BscReorderingModel *)bsc_malloc(sizeof(BscReorderingModel))) {
    long long Entropy[DETECTORS_MAX_RECORD_SIZE];

    if ((n % 48) != 0)
      n = n - (n % 48);

    for (int recordSize = 1; recordSize <= DETECTORS_MAX_RECORD_SIZE;
         ++recordSize) {
      ...
      if (recordSize == 1) {
        int ctx0 = 0;
        for (int i = 0; i < n; i += 8) {
          unsigned char c0 = input[i + 0];
          model->contexts[0][ctx0].frequencies[c0]++;
          ctx0 = c0;
          unsigned char c1 = input[i + 1];
          model->contexts[0][ctx0].frequencies[c1]++;
          ctx0 = c1;
          unsigned char c2 = input[i + 2];
          model->contexts[0][ctx0].frequencies[c2]++;
          ctx0 = c2;
          unsigned char c3 = input[i + 3];
          model->contexts[0][ctx0].frequencies[c3]++;
          ctx0 = c3;
          unsigned char c4 = input[i + 4];
          model->contexts[0][ctx0].frequencies[c4]++;
          ctx0 = c4;
          unsigned char c5 = input[i + 5];
          model->contexts[0][ctx0].frequencies[c5]++;
          ctx0 = c5;
          unsigned char c6 = input[i + 6];
          model->contexts[0][ctx0].frequencies[c6]++;
          ctx0 = c6;
          unsigned char c7 = input[i + 7];
          model->contexts[0][ctx0].frequencies[c7]++;
          ctx0 = c7;
        }
      }
      ...
      if (recordSize == 2) {
        ...
        for (int i = 0; i < n; i += 8) {
          ...
          ctx0 = c0;
          unsigned char c1 = input[i + 1];
          model->contexts[1][ctx1].frequencies[c1]++;
          ctx1 = c1;
          unsigned char c2 = input[i + 2];
          model->contexts[0][ctx0].frequencies[c2]++;
          ctx0 = c2;
          unsigned char c3 = input[i + 3];
          model->contexts[1][ctx1].frequencies[c3]++;
          ctx1 = c3;
          unsigned char c4 = input[i + 4];
          model->contexts[0][ctx0].frequencies[c4]++;
          ctx0 = c4;
          unsigned char c5 = input[i + 5];
          model->contexts[1][ctx1].frequencies[c5]++;
          ctx1 = c5;
          unsigned char c6 = input[i + 6];
          model->contexts[0][ctx0].frequencies[c6]++;
          ctx0 = c6;
          unsigned char c7 = input[i + 7];
          model->contexts[1][ctx1].frequencies[c7]++;
          ctx1 = c7;
        }
      }
      ...
      if (recordSize == 3) {
        ...
        for (int i = 0; i < n; i += 6) {
          ...
          ctx1 = c1;
          unsigned char c2 = input[i + 2];
          model->contexts[2][ctx2].frequencies[c2]++;
          ctx2 = c2;
          unsigned char c3 = input[i + 3];
          model->contexts[0][ctx0].frequencies[c3]++;
          ctx0 = c3;
          unsigned char c4 = input[i + 4];
          model->contexts[1][ctx1].frequencies[c4]++;
          ctx1 = c4;
          unsigned char c5 = input[i + 5];
          model->contexts[2][ctx2].frequencies[c5]++;
          ctx2 = c5;
        }
      }
      ...
      if (recordSize == 4) {
        ...
        for (int i = 0; i < n; i += 8) {
          ...
          ctx2 = c2;
          unsigned char c3 = input[i + 3];
          model->contexts[3][ctx3].frequencies[c3]++;
          ctx3 = c3;
          unsigned char c4 = input[i + 4];
          model->contexts[0][ctx0].frequencies[c4]++;
          ctx0 = c4;
          ...
          ctx1 = c5;
          unsigned char c6 = input[i + 6];
          model->contexts[2][ctx2].frequencies[c6]++;
          ctx2 = c6;
          unsigned char c7 = input[i + 7];
          model->contexts[3][ctx3].frequencies[c7]++;
          ctx3 = c7;
        }
      }
    ...
    for (int recordSize = 1; recordSize <= DETECTORS_MAX_RECORD_SIZE;
         ++recordSize) {
      if (bestSize > Entropy[recordSize - 1]) {
        bestSize = Entropy[recordSize - 1];
        result = recordSize;
      }
    }
