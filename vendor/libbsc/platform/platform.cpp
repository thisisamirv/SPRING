...
int bsc_get_cpu_features(void) { return LIBBSC_CPU_FEATURE; }
...
static void *bsc_default_malloc(size_t size) {
  ...
  if ((bsc_LargePageSize != 0) && (size >= 256 * 1024)) {
    void *address = VirtualAlloc(
        0, (size + bsc_LargePageSize - 1) & (~(bsc_LargePageSize - 1)),
...
void *bsc_malloc(size_t size) { return bsc_malloc_fn(size); }

void *bsc_zero_malloc(size_t size) { return bsc_zero_malloc_fn(size); }

void bsc_free(void *address) { return bsc_free_fn(address); }
...

    {
      if (HMODULE hKernel = GetModuleHandle(TEXT("kernel32.dll"))) {
        ...
        GetLargePageMinimumProcT largePageMinimumProc =
            (GetLargePageMinimumProcT)GetProcAddress(hKernel,
                                                     "GetLargePageMinimum");
        if (largePageMinimumProc != NULL) {
          SIZE_T largePageSize = largePageMinimumProc();

          if ((largePageSize & (largePageSize - 1)) != 0)
            largePageSize = 0;
