...
int bsc_coder_encode_block(const unsigned char *input, unsigned char *output,
                           int inputSize, int outputSize, int coder) {
  if (coder == LIBBSC_CODER_QLFC_STATIC)
    return bsc_qlfc_static_encode_block(input, output, inputSize, outputSize);
  if (coder == LIBBSC_CODER_QLFC_ADAPTIVE)
    return bsc_qlfc_adaptive_encode_block(input, output, inputSize, outputSize);
  if (coder == LIBBSC_CODER_QLFC_FAST)
    return bsc_qlfc_fast_encode_block(input, output, inputSize, outputSize);

  return LIBBSC_BAD_PARAMETER;
}

void bsc_coder_split_blocks(const unsigned char *input, int n, int nBlocks,
  ...
  if (rankSize > nBlocks) {
    ...
    for (int p = 0; p < nBlocks; ++p) {
      ...
      blockSize[p] =
          (p != nBlocks - 1) ? n / nBlocks : n - (n / nBlocks) * (nBlocks - 1);
    }
  }
}

int bsc_coder_compress_serial(const unsigned char *input, unsigned char *output,
                              int n, int coder) {
  ...
  for (int blockId = 0; blockId < nBlocks; ++blockId) {
    ...
    int result = bsc_coder_encode_block(input + inputStart, output + outputPtr,
                                        inputSize, outputSize, coder);
    if (result < LIBBSC_NO_ERROR) {
...
int bsc_coder_compress_parallel(const unsigned char *input,
                                unsigned char *output, int n, int coder) {
...
#pragma omp single
        {
          bsc_coder_split_blocks(input, n, nBlocks, compressedStart,
                                 compressedSize);
        }
...
#pragma omp for ordered schedule(dynamic, 1)
        for (int blockId = 0; blockId < nBlocks; ++blockId) {
          ...
          compressionResult[blockId] =
              bsc_coder_encode_block(input + blockStart, buffer + blockStart,
                                     blockSize, blockSize, coder);
...
int bsc_coder_decode_block(const unsigned char *input, unsigned char *output,
                           int coder) {
  if (coder == LIBBSC_CODER_QLFC_STATIC)
    return bsc_qlfc_static_decode_block(input, output);
  if (coder == LIBBSC_CODER_QLFC_ADAPTIVE)
    return bsc_qlfc_adaptive_decode_block(input, output);
  if (coder == LIBBSC_CODER_QLFC_FAST)
    return bsc_qlfc_fast_decode_block(input, output);
    ...
    for (int blockId = 0; blockId < nBlocks; ++blockId) {
      ...
      if (inputSize != outputSize) {
        decompressionResult[blockId] =
            bsc_coder_decode_block(input + inputPtr, output + outputPtr, coder);
      } else {
