...
public:
  static INLINE void UpdateBit(unsigned int bit, short &probability,
                               const int threshold0, const int adaptationRate0,
                               const int threshold1,
                               const int adaptationRate1) {
    int delta0 = probability * adaptationRate0 -
                 ((4096 - threshold0) * adaptationRate0 - 4095);
    int delta1 = probability * adaptationRate1 - (threshold1 * adaptationRate1);

    probability = probability - ((bit ? delta1 : delta0) >> 12);
  }

  static INLINE void UpdateBit0(short &probability, const int threshold,
                                const int adaptationRate) {
    probability = probability +
                  (((4096 - threshold - probability) * adaptationRate) >> 12);
  };

  static INLINE void UpdateBit1(short &probability, const int threshold,
                                const int adaptationRate) {
    probability =
  ...
  template <int R>
  static INLINE void UpdateBit(unsigned int bit, short &probability,
                               const int threshold0, const int threshold1) {
    probability =
        probability - ((probability - (bit ? threshold1 : threshold0)) >> R);
  }

  template <int R>
  static INLINE void UpdateBit(short &probability, const int threshold) {
...
public:
  ...
  INLINE int Mixup(const int probability0, const int probability1,
                   const int probability2) {
    ...
    short stretchedProbability =
        (stretchedProbability0 * weight0 + stretchedProbability1 * weight1 +
         stretchedProbability2 * weight2) >>
        17;

    if (stretchedProbability < -2047)
      stretchedProbability = -2047;
    if (stretchedProbability > 2047)
      stretchedProbability = 2047;
    ...
    const int mappedProbability =
        probabilityMap[index] +
        (((probabilityMap[index + 1] - probabilityMap[index]) * weight) >> 8);

    return mixedProbability = (3 * probability + mappedProbability) >> 2;
  };

  INLINE int MixupAndUpdateBit0(const int probability0, const int probability1,
                                const int probability2, const int learningRate0,
                                const int learningRate1,
                                const int learningRate2, const int threshold,
                                const int adaptationRate) {
    ...
    ProbabilityCounter::UpdateBit0(probabilityMap[index], threshold,
                                   adaptationRate);
    ProbabilityCounter::UpdateBit0(probabilityMap[index + 1], threshold,
                                   adaptationRate);
  ...
  INLINE int MixupAndUpdateBit1(const int probability0, const int probability1,
                                const int probability2, const int learningRate0,
                                const int learningRate1,
                                const int learningRate2, const int threshold,
                                const int adaptationRate) {
    ...
    ProbabilityCounter::UpdateBit1(probabilityMap[index], threshold,
                                   adaptationRate);
    ProbabilityCounter::UpdateBit1(probabilityMap[index + 1], threshold,
                                   adaptationRate);
  ...
  INLINE void UpdateBit1(const int learningRate0, const int learningRate1,
                         const int learningRate2, const int threshold,
                         const int adaptationRate) {
    ProbabilityCounter::UpdateBit1(probabilityMap[index], threshold,
                                   adaptationRate);
