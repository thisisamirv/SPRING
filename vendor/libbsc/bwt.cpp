...
#error                                                                         \
    "LIBBSC_OPENMP requires LIBSAIS_OPENMP to be defined. Please define LIBSAIS_OPENMP and enable OpenMP support for libsais."
#endif
...
int bsc_bwt_init(int features) { return LIBBSC_NO_ERROR; }

#endif

int bsc_bwt_gpu_encode(unsigned char *T, int n, unsigned char *num_indexes,
  ...
  if (features & LIBBSC_FEATURE_CUDA) {
    ...
    if (num_indexes != NULL && indexes != NULL) {
      ...
      if (bwt_cuda_device_storage_size < n) {
        ...
        if (libcubwt_allocate_device_storage(
                &bwt_cuda_device_storage, n + (n / 32)) == LIBCUBWT_NO_ERROR) {
          bwt_cuda_device_storage_size = n + (n / 32);
        }
      }

      if (bwt_cuda_device_storage_size >= n) {
        index = (int)libcubwt_bwt_aux(bwt_cuda_device_storage, T, T, n, mod + 1,
                                      (unsigned int *)I);
      }
      ...
      if (libcubwt_allocate_device_storage(&bwt_cuda_device_storage, n) ==
          LIBCUBWT_NO_ERROR) {
        index = (int)libcubwt_bwt_aux(bwt_cuda_device_storage, T, T, n, mod + 1,
                                      (unsigned int *)I);
...
#ifdef LIBBSC_OPENMP
      int numThreads = (features & LIBBSC_FEATURE_MULTITHREADING) > 0
                           ? omp_get_max_threads() / omp_get_num_threads()
                           : 1;
      index = libsais_bwt_aux_omp(
          T, T, A, n, 0, NULL, mod + 1, I,
          numThreads < 1 ? 1 : (numThreads > 8 ? 8 : numThreads));
#else
...
int bsc_bwt_gpu_decode(unsigned char *T, int n, int index, int features) {
  ...
  if (features & LIBBSC_FEATURE_CUDA) {
    ...
    if (bwt_cuda_device_storage_size < storage_approx_length) {
      ...
      if (libcubwt_allocate_device_storage(&bwt_cuda_device_storage,
                                           storage_approx_length +
                                               (storage_approx_length / 32)) ==
          LIBCUBWT_NO_ERROR) {
        bwt_cuda_device_storage_size =
            storage_approx_length + (storage_approx_length / 32);
      }
    }

    if (bwt_cuda_device_storage_size >= storage_approx_length) {
      result =
          (int)libcubwt_unbwt(bwt_cuda_device_storage, T, T, n, NULL, index);
    }
...
int bsc_bwt_decode(unsigned char *T, int n, int index,
  ...
  if (int *P = (int *)bsc_malloc((n + 1) * sizeof(int))) {
    ...
    if (num_indexes == (unsigned char)((n - 1) / (mod + 1)) &&
        indexes != NULL) {
      int I[256];
      I[0] = index;
      ...
      int numThreads = (features & LIBBSC_FEATURE_MULTITHREADING) > 0
                           ? omp_get_max_threads() / omp_get_num_threads()
                           : 1;
      index = libsais_unbwt_aux_omp(T, T, P, n, NULL, mod + 1, I,
      ...
      int numThreads = (features & LIBBSC_FEATURE_MULTITHREADING) > 0
                           ? omp_get_max_threads() / omp_get_num_threads()
                           : 1;
      index = libsais_unbwt_omp(T, T, P, n, NULL, index,
                                numThreads > 1 ? numThreads : 1);
#else
