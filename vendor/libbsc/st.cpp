...
static int bsc_st3_transform_serial(unsigned char *RESTRICT T,
                                    unsigned short *RESTRICT P,
                                    int *RESTRICT bucket, int n) {
...
static int bsc_st4_transform_serial(unsigned char *RESTRICT T,
...
static int bsc_st5_transform_serial(unsigned char *RESTRICT T,
                                    unsigned int *RESTRICT P,
                                    int *RESTRICT bucket, int n) {
  for (int i = 0; i < LIBBSC_HEADER_SIZE; ++i)
    T[n + i] = T[i];
  ...
  for (int sum = 0, i = 0;
       i < ALPHABET_SQRT_SIZE * ALPHABET_SIZE * ALPHABET_SIZE; ++i) {
    int tmp = sum;
    sum += bucket[i];
    bucket[i] = tmp;
  }
...
static int bsc_st6_transform_serial(unsigned char *RESTRICT T,
                                    unsigned int *RESTRICT P,
                                    int *RESTRICT bucket, int n) {
  for (int i = 0; i < LIBBSC_HEADER_SIZE; ++i)
    T[n + i] = T[i];
  ...
  for (int sum = 0, i = 0; i < ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE;
       ++i) {
    int tmp = sum;
    sum += bucket[i];
    bucket[i] = tmp;
...
static int bsc_st3_transform_parallel(unsigned char *RESTRICT T,
                                      unsigned short *RESTRICT P,
                                      int *RESTRICT bucket0, int n) {
  unsigned int count0[ALPHABET_SIZE];
  memset(count0, 0, ALPHABET_SIZE * sizeof(unsigned int));
  unsigned int count1[ALPHABET_SIZE];
  memset(count1, 0, ALPHABET_SIZE * sizeof(unsigned int));
...
static int bsc_st4_transform_parallel(unsigned char *RESTRICT T,
                                      unsigned int *RESTRICT P,
                                      int *RESTRICT bucket, int n) {
  if (int *RESTRICT bucket0 =
          (int *)bsc_zero_malloc(ALPHABET_SIZE * ALPHABET_SIZE * sizeof(int))) {
...
static int bsc_st5_transform_parallel(unsigned char *RESTRICT T,
                                      unsigned int *RESTRICT P,
                                      int *RESTRICT bucket0, int n) {
  if (int *RESTRICT bucket1 = (int *)bsc_zero_malloc(
          ALPHABET_SQRT_SIZE * ALPHABET_SIZE * ALPHABET_SIZE * sizeof(int))) {
...
#pragma omp single
...

        {
          if (threadId == 0) {
            ...
            memset(bucket0, 0,
                   ALPHABET_SQRT_SIZE * ALPHABET_SIZE * ALPHABET_SIZE *
                       sizeof(int));
...
static int bsc_st6_transform_parallel(unsigned char *RESTRICT T,
                                      unsigned int *RESTRICT P,
                                      int *RESTRICT bucket, int n) {
        ...
        if (nThreads == 1) {
...

          {
            if (threadId == 0) {
              ...
              unsigned int W =
                  (T[median - 2] << 16) | (T[median - 1] << 8) | T[median];
              for (int i = median; i < n; ++i) {
                W = (W << 8) | T[i + 1];
                bucket1[W >> 8]++;
              }
...
#pragma omp barrier
...

          {
            if (threadId == 0) {
              for (int sum = 0, i = 0;
                   i < ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE / 2; ++i) {
                int tmp = sum;
                sum = sum + bucket0[i] + bucket1[i];
                bucket[i] = bucket0[i] = tmp;
                bucket1[i] = sum - 1;
              }
            } else {
              for (int sum = n,
                       i = ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE - 1;
                   i >= ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE / 2;
                   --i) {
                int tmp = sum;
                sum = sum - bucket0[i] - bucket1[i];
                bucket[i] = bucket0[i] = sum;
                bucket1[i] = tmp - 1;
              }
            }
...
#pragma omp barrier
          ...
          {
            if (threadId == 0) {
              memcpy(bucket1, bucket + 1,
                     (ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE / 2) *
                         sizeof(int));
            } else {
              memcpy(bucket1 +
                         ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE / 2,
                     bucket +
                         ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE / 2 + 1,
                     (ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE / 2 - 1) *
                         sizeof(int));
...
int bsc_st3_encode(unsigned char *T, int n, int features) {
  if (unsigned short *P =
          (unsigned short *)bsc_malloc(n * sizeof(unsigned short))) {
    if (int *bucket = (int *)bsc_zero_malloc(ALPHABET_SIZE * ALPHABET_SIZE *
                                             sizeof(int))) {
...
int bsc_st5_encode(unsigned char *T, int n, int features) {
  if (unsigned int *P = (unsigned int *)bsc_malloc(n * sizeof(unsigned int))) {
    if (int *bucket = (int *)bsc_zero_malloc(
            ALPHABET_SQRT_SIZE * ALPHABET_SIZE * ALPHABET_SIZE * sizeof(int))) {
      int index = LIBBSC_NO_ERROR;
...
int bsc_st_encode(unsigned char *T, int n, int k, int features) {
  ...
  if (k == 3)
    return bsc_st3_encode(T, n, features);
  if (k == 4)
    return bsc_st4_encode(T, n, features);
  if (k == 5)
    return bsc_st5_encode(T, n, features);
  if (k == 6)
    return bsc_st6_encode(T, n, features);

  return LIBBSC_NOT_SUPPORTED;
}

static bool bsc_unst_sort_serial(unsigned char *RESTRICT T,
                                 unsigned int *RESTRICT P,
                                 unsigned int *RESTRICT count,
                                 unsigned int *RESTRICT bucket, int n, int k) {
  ...
  for (int c = 0; c < ALPHABET_SIZE; ++c) {
    for (int d = 0; d < c; ++d) {
      int tmp = bucket[(d << 8) | c];
      bucket[(d << 8) | c] = bucket[(c << 8) | d];
      bucket[(c << 8) | d] = tmp;
    }
  }
...
static void bsc_unst_reconstruct_case1_serial(unsigned char *RESTRICT T,
                                              unsigned int *RESTRICT P,
                                              unsigned int *RESTRICT count,
                                              int n, int start) {
  unsigned int index[ALPHABET_SIZE];
...
static void bsc_unst_reconstruct_case2_serial(unsigned char *RESTRICT T,
                                              unsigned int *RESTRICT P,
                                              unsigned int *RESTRICT count,
                                              int n, int start) {
  unsigned int index[ALPHABET_SIZE];
...
static void bsc_unst_reconstruct_case3_serial(unsigned char *RESTRICT T,
                                              unsigned int *RESTRICT P,
                                              unsigned int *RESTRICT count,
                                              int n, int start) {
...
static void bsc_unst_reconstruct_serial(unsigned char *T, unsigned int *P,
                                        unsigned int *count, int n, int index,
                                        bool failBack) {
  if (n < 0x800000)
    return bsc_unst_reconstruct_case1_serial(T, P, count, n, index);
  if (!failBack)
    return bsc_unst_reconstruct_case2_serial(T, P, count, n, index);
  if (failBack)
    return bsc_unst_reconstruct_case3_serial(T, P, count, n, index);
}

#ifdef LIBBSC_OPENMP

static bool bsc_unst_sort_parallel(unsigned char *RESTRICT T,
                                   unsigned int *RESTRICT P,
                                   unsigned int *RESTRICT count,
                                   unsigned int *RESTRICT bucket, int n,
                                   int k) {
  bool failBack = false;
  {
#pragma omp parallel
...
#pragma omp parallel for schedule(static, 1)
    for (int c = 0; c < ALPHABET_SIZE; ++c) {
      unsigned int index[ALPHABET_SIZE];
      memcpy(index, &bucket[c << 8], ALPHABET_SIZE * sizeof(unsigned int));
      int group[ALPHABET_SIZE];
      memset(group, 0xff, ALPHABET_SIZE * sizeof(int));
...
static void bsc_unst_reconstruct_case1_parallel(unsigned char *RESTRICT T,
                                                unsigned int *RESTRICT P,
                                                unsigned int *RESTRICT count,
                                                unsigned int *RESTRICT bucket,
                                                int n, int start) {
#pragma omp parallel for schedule(static, 1)
  for (int c = 0; c < ALPHABET_SIZE; ++c) {
    unsigned int index[ALPHABET_SIZE];
...
static void bsc_unst_reconstruct_case2_parallel(unsigned char *RESTRICT T,
                                                unsigned int *RESTRICT P,
                                                unsigned int *RESTRICT count,
                                                unsigned int *RESTRICT bucket,
                                                int n, int start) {
...
static void bsc_unst_reconstruct_case3_parallel(unsigned char *RESTRICT T,
                                                unsigned int *RESTRICT P,
                                                unsigned int *RESTRICT count,
                                                unsigned int *RESTRICT bucket,
                                                int n, int start) {
...
static void bsc_unst_reconstruct_parallel(unsigned char *T, unsigned int *P,
                                          unsigned int *count,
                                          unsigned int *bucket, int n,
                                          int index, bool failBack) {
  if (n < 0x800000)
    return bsc_unst_reconstruct_case1_parallel(T, P, count, bucket, n, index);
  if (!failBack)
    return bsc_unst_reconstruct_case2_parallel(T, P, count, bucket, n, index);
  if (failBack)
    return bsc_unst_reconstruct_case3_parallel(T, P, count, bucket, n, index);
}
...
int bsc_st_decode(unsigned char *T, int n, int k, int index, int features) {
  ...
  if (unsigned int *P =
          (unsigned int *)bsc_zero_malloc(n * sizeof(unsigned int))) {
    if (unsigned int *bucket = (unsigned int *)bsc_zero_malloc(
            ALPHABET_SIZE * ALPHABET_SIZE * sizeof(unsigned int))) {
      unsigned int count[ALPHABET_SIZE];
      memset(count, 0, ALPHABET_SIZE * sizeof(unsigned int));
