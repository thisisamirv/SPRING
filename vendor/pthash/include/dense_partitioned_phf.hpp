...
struct dense_partitioned_phf //
{
  static_assert(std::is_base_of<dense_encoder, Encoder>::value,
                "A dense encoder must be specified for dense_partitioned_phf. "
  ...
  // template <typename Iterator>
  // build_timings build_in_external_memory(Iterator keys, const uint64_t
  // num_keys,
  //                                        build_configuration const& config) {
  //     // TODO
  // }
  ...
  uint64_t build(Builder &builder, build_configuration const &config) //
  {
    ...
    const uint64_t num_buckets_per_partition =
        builder.num_buckets_per_partition();
    ...
    m_pilots.encode(builder.interleaving_pilots_iterator_begin(),
                    num_partitions, num_buckets_per_partition,
                    config.num_threads);

    if (Minimal and m_num_keys < m_table_size) {
      assert(builder.free_slots().size() == m_table_size - m_num_keys);
      m_free_slots.encode(builder.free_slots().begin(),
  ...
  uint64_t operator()(T const &key) const //
  {
    ...
    const uint64_t partition_offset =
        partition << constants::log2_table_size_per_partition;
                    ...
                    const uint64_t partition) const  //
  {
    ...
    return remap128(mix(hash.second() ^ hashed_pilot),
  ...
  uint64_t num_bits_for_mapper() const {
    return m_partitioner.num_bits() + m_bucketer.num_bits() +
...
private:
  ...
  set_build_configuration(build_configuration const &config) {
