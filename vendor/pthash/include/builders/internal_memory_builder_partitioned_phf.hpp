...
struct internal_memory_builder_partitioned_phf {
                                ...
                                build_configuration const &config) //
  {
    ...
    const uint64_t num_partitions =
        compute_num_partitions(num_keys, avg_partition_size);
    ...
    m_num_buckets_per_partition =
        compute_num_buckets(avg_partition_size, config.lambda);

    std::vector<std::vector<typename hasher_type::hash_type>> partitions(
        num_partitions);
    {
      const uint64_t p =
          max_partition_size_estimate(avg_partition_size, num_partitions);
      if (config.verbose)
        std::cout << "largest_partition_size_estimate " << p << std::endl;
    ...
    for (int attempt = 0; attempt < max_num_attempts; ++attempt) //
    {
      ...
      if constexpr (std::is_same_v<typename Iterator::iterator_category,
        ...
        parallel_hash_and_partition(it, partitions, num_keys,
                                    config.num_threads, m_seed, num_partitions,
                                    m_bucketer);
      } else {
      ...
      if (config.dense_partitioning) {
        // if (config.verbose) std::cout << "alpha value ignored for --dense" <<
        // std::endl;
      ...
      if (config.verbose) {
        std::cout << "smallest_partition_size = " << smallest_partition_size
                  << std::endl;
        std::cout << "largest_partition_size = " << largest_partition_size
                  << std::endl;
        std::cout << "num_buckets_per_partition = "
                  << partition_config.num_buckets << std::endl;
      }

      if (largest_partition_size <= partition_config.table_size) {
        if (config.verbose and config.dense_partitioning) {
          std::cout << "load factor of partitions: "
                    << (smallest_partition_size * 1.0) /
                           partition_config.table_size
                    << " <= alpha <= "
                    << (largest_partition_size * 1.0) /
                           partition_config.table_size
                    << std::endl;
        }
        break;
    ...
    timings.partitioning_microseconds =
        to_microseconds(clock_type::now() - start);

    auto t =
        build_partitions(partitions.begin(), m_builders.begin(),
                         partition_config, config.num_threads, num_partitions);
  ...
  static void parallel_hash_and_partition(
      ...
      std::vector<std::vector<typename hasher_type::hash_type>> &partitions,
      const uint64_t num_keys, const uint64_t num_threads,
      const uint64_t m_seed, const uint64_t num_partitions,
      const range_bucketer partitioner) //
  {
    std::vector<std::vector<std::vector<typename hasher_type::hash_type>>>
        split;
    split.resize(num_threads);
    uint64_t partitions_per_thread =
    ...
    auto hash_and_split = [&](uint64_t id, uint64_t begin, uint64_t end) {
      for (; begin != end; ++begin) {
        typename hasher_type::hash_type hash =
            hasher_type::hash(keys[begin], m_seed);
  ...
  static build_timings
  build_partitions(PartitionsIterator partitions, BuildersIterator builders,
                   build_configuration const &config,
                   const uint64_t num_threads, const uint64_t num_partitions) {
    build_timings timings;
    ...
    if (num_threads > 1) { // parallel
      ...
      for (auto const &t : thread_timings) {
        if (t.mapping_ordering_microseconds >
  ...
  uint64_t avg_partition_size() const { return m_avg_partition_size; }

  uint64_t num_buckets_per_partition() const {
    return m_num_buckets_per_partition;
  }

  range_bucketer bucketer() const { return m_bucketer; }
  ...
  std::vector<
      internal_memory_builder_single_phf<hasher_type, bucketer_type>> const &
  builders() const {
    return m_builders;
  }
  ...
  struct interleaving_pilots_iterator //
  {
    ...
    interleaving_pilots_iterator(
        std::vector<internal_memory_builder_single_phf<
            hasher_type, bucketer_type>> const *builders,
        uint64_t m_curr_partition = 0, uint64_t curr_bucket_in_partition = 0)
        : m_builders(builders), m_curr_partition(m_curr_partition),
          m_curr_bucket_in_partition(curr_bucket_in_partition),
          m_num_partitions(builders->size()) {}

    uint64_t operator*() const {
      auto const &pilots_of_partition =
          (*m_builders)[m_curr_partition].pilots();
  ...
  */
  struct taken {
    taken(std::vector<internal_memory_builder_single_phf<
              hasher_type, bucketer_type>> const &builders)
        : m_builders(builders), m_size(0) {
      for (auto const &b : m_builders)
        m_size += b.taken().num_bits();
    }

    struct iterator {
      iterator(taken const *taken, const uint64_t pos = 0)
          : m_taken(taken), m_curr_pos(pos), m_curr_offset(0),
            m_curr_partition(0) //
      {
        while (m_curr_pos >=
               m_curr_offset +
                   m_taken->m_builders[m_curr_partition].taken().num_bits()) {
          m_curr_offset +=
              m_taken->m_builders[m_curr_partition].taken().num_bits();
          m_curr_partition += 1;
        }
...
private:
  ...
  std::vector<internal_memory_builder_single_phf<hasher_type, bucketer_type>>
      m_builders;
};
