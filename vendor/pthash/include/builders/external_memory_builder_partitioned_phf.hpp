...
struct external_memory_builder_partitioned_phf {
                                ...
                                build_configuration const &config) //
  {
    ...
    if (config.verbose) {
      ...
      std::cout << "using " << static_cast<double>(config.ram) / 1'000'000'000
                << " GB of RAM" << std::endl;
    }
    ...
    for (uint64_t partition_id = 0; partition_id != num_partitions;
         ++partition_id) {
    ...
    if (config.num_threads > 1) { // parallel
      ...
      auto build_partitions = [&]() {
        ...
        auto t =
            internal_memory_builder_partitioned_phf<hasher_type, Bucketer>::
                build_partitions(in_memory_partitions.begin(),
                                 in_memory_builders.begin(), partition_config,
                                 config.num_threads,
        ...
        for (auto &builder : in_memory_builders) {
          ...
          internal_memory_builder_single_phf<hasher_type, Bucketer>().swap(
              builder);
          ++id;
        }
        timings.partitioning_microseconds +=
      ...
      for (; i != num_partitions; ++i) {
        ...
        uint64_t partition_bytes =
            internal_memory_builder_single_phf<hasher_type, Bucketer>::
                estimate_num_bytes_for_construction(size, partition_config);
...
private:
  template <typename Builder> struct builders_files_manager {
...
public:
  builders_files_manager<
      internal_memory_builder_single_phf<hasher_type, Bucketer>> const &
  builders() const {
    return m_builders;
  }
