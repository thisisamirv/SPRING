...
constexpr bucket_size_type MAX_BUCKET_SIZE =
    (1ULL << 8 * sizeof(bucket_size_type)) - 1;
...
static inline uint64_t compute_num_buckets(const uint64_t num_keys,
...
static inline uint64_t find_avg_partition_size(const uint64_t n) {
  ...
  if (n < c)
    throw std::runtime_error(
        "n is too small for --dense; does not use this option");
  static_assert(c > 500);
  const uint64_t a_initial_guess = c - 500;
  const double eps = 0.5;
  uint64_t a_sol = 0;
  for (uint64_t a = a_initial_guess; a != c; ++a) {
    if (max_partition_size_estimate(a, compute_num_partitions(n, a)) + eps >=
        c) {
      a_sol = a;
      break;
    }
...
void fill_free_slots(Taken const &taken, const uint64_t num_keys,
                     FreeSlots &free_slots, const uint64_t table_size) {
