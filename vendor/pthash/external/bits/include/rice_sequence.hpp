...
template <typename DArray1 = darray1> struct rice_sequence {
  template <typename Iterator> void encode(Iterator begin, const uint64_t n) {
    if (n == 0)
      return;
    ...
    for (uint64_t i = 0; i != n; ++i, ++begin) {
      auto v = *begin;
      if (l > 0)
        cv_builder_low_bits.set(i, v & low_mask);
      auto unary = v >> l;
      for (uint64_t j = 0; j != unary; ++j)
