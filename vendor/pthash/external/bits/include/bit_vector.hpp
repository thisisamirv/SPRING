...
struct bit_vector //
{
      ...
      if (pos_in_word == 0) {
        ...
        *m_cur_word |= bits << pos_in_word;
        if (len > 64 - pos_in_word)
          m_data.push_back(bits >> (64 - pos_in_word));
      }
      m_cur_word = &m_data.back();
    }
    ...
    void append(builder const &rhs) {
      ...
      if (shift == 0) { // word-aligned, easy case
        ...
        *cur_word |= rhs.m_data.back() << shift;
        if (cur_word < &m_data.back())
          *++cur_word = rhs.m_data.back() >> (64 - shift);
      }
      m_cur_word = &m_data.back();
    }
  ...
  [[nodiscard]] uint64_t get_bits(uint64_t pos, uint64_t len) const {
    ...
    if (shift + len <= 64) {
      ...
      return (m_data[block] >> shift) |
             (m_data[block + 1] << (64 - shift) & mask);
    }
  }
  ...
  struct iterator {
    iterator()
        : m_data(nullptr), m_num_64bit_words(0), m_pos(0), m_buf(0),
          m_avail(0) {}
