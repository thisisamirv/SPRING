...
/*
    ...
    - We keep an array `hints_0` of size ceil(U/2^8), where `hints_0[i]`
   indicates the position of the i-th 0 in the high bit array. (Each
   `hints_0[i]` is written in log(n+U/2^8+1) bits.)
*/

template <typename DArray1 = darray1> struct endpoints_sequence {
  ...
  void encode(Iterator begin, uint64_t n, uint64_t universe) {
    ...
    compact_vector::builder cvb_hints_0((universe + 256 - 1) /
                                            256, // ceil(U/2^8)
  ...
  struct iterator {
    ...
    iterator(endpoints_sequence const *ptr, uint64_t pos = 0)
        : m_ptr(ptr), m_pos(pos), m_val(0) //
    {
      if (!has_next() or m_ptr->m_high_bits_d1.num_positions() == 0)
        return;
    ...
    iterator(endpoints_sequence const *ptr, uint64_t pos,
             uint64_t pos_in_high_bits /* hint */)
        : m_ptr(ptr), m_pos(pos), m_val(0) //
    {
      /*
      ...
      */
      assert(has_next());
      assert(pos_in_high_bits == 0 ||
             m_ptr->m_high_bits.get(pos_in_high_bits) == 0);
  ...
  */
  return_value next_geq(const uint64_t x) const {
    return next_geq_helper(x).first;
  }
  ...
  */
  ...
  uint64_t num_bytes() const {
    return sizeof(m_back) + m_high_bits.num_bytes() +
           m_high_bits_d1.num_bytes() + m_hints_0.num_bytes() +
           essentials::vec_bytes(m_low_bits);
  }
