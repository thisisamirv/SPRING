...
/*
    ...
    just need to keep the cumulative counts for 512/64-1=7 blocks (the
   cumulative count to the left of the first block is always 0). Now, the
   maximum cumulative sum is clearly 64*7=448 which fits into 9 bits, thus we
...
struct rank9 {
  ...
  */
  ...
  uint64_t num_bytes() const {
    return essentials::vec_bytes(m_block_rank_pairs);
  }
