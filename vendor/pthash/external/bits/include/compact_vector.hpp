...
struct compact_vector //
{
  template <typename Vec> struct enumerator {
    ...
    enumerator()
        : m_i(0), m_cur_val(0), m_cur_block(0), m_cur_shift(0), m_vec(nullptr) {
    }
  ...
  private:
    ...
    void read() {
      if (m_cur_shift + m_vec->m_width <= 64) {
        ...
        m_cur_val =
            (m_vec->m_data[m_cur_block] >> m_cur_shift) |
  ...
  struct builder {
    ...
    */
    ...
    template <typename Iterator> void fill(Iterator begin, uint64_t n) {
      if (m_width == 0)
      ...
      for (uint64_t i = 0, pos = 0; i < m_size; ++i, pos += old_width) {
        ...
        uint64_t old_elem =
            shift + old_width <= 64
                ...
                : (m_data[block] >> shift) |
                      (m_data[block + 1] << (64 - shift) & old_mask);
