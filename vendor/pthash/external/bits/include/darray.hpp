...
/*
    ...
    - An array `block_inventory[0..m/L)` such that `block_inventory[i]`
      holds the result of Select(iL) if block i is dense; it holds the start
   position in `overflow_positions` of the 1-bit positions of the block i if it
   is sparse. Its space is m/L*64 bits.
    ...
    `overflow_positions[block_inventory[i/L] + i%L]`; otherwise the
   corresponding sub-block is accessed and a sequential scan of at most L2 bits
   is performed. If p is the position computed during the scan, the final result
   is `block_inventory[i/L] + subblock_inventory[i/L3] + p`.

    If Scan(Q) = Q/B is the number of cache-misses involved in a sequential scan
    of Q bits with a cache-line of B bits, it follows that the number of
   cache-misses per Select(i) query are: 2, if position i belongs to a sparse
   block; 2 + Scan(L2), if i belongs to a dense block.
    ...
    (When used to index the high bitvector of Elias-Fano, sparse blocks are
   rare; so the space usage is likely close to 9/16 m.)

    If 0.0 <= d = m/n <= 1.0 is the density of the bitvector, the index costs at
   most 25/16 dn extra bits, for a total of n(1+25/16d) bits.
*/
...
struct darray {
  ...
  void build(bit_vector const &B) {
    ...
    //               << " (expected = " << ((m_positions + block_size - 1) /
    //               block_size) << ")"
    //               << std::endl;
    //     std::cout << "num_subblocks = " << m_subblock_inventory.size()
    //               << " (expected = " << ((m_positions + subblock_size - 1) /
    //               subblock_size)
    //               << ")" << std::endl;

    //     std::cout << "block_inventory: got "
    //               << (essentials::vec_bytes(m_block_inventory) * 8.0 - 64) /
    //               m_positions
    //               << " bits/bit (expected "
    //               << ((m_positions + block_size - 1) / block_size * 64.0) /
    //               m_positions <<
    //               ")"
    //               << std::endl;

    //     std::cout << "overflow_positions: got "
    //               << (essentials::vec_bytes(m_overflow_positions) * 8.0 - 64)
    //               / m_positions
    //               << " bits/bit (at most 1)" << std::endl;

    //     std::cout << "subblock_inventory: got "
    //               << (essentials::vec_bytes(m_subblock_inventory) * 8.0 - 16)
    //               / m_positions
    //               << " bits/bit (expected "
    //               << ((m_positions + subblock_size - 1) / subblock_size
    //               * 16.0) / m_positions
    //               << ")" << std::endl;
    // }
  }
