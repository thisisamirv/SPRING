...
/*
   ...
   SEA 2025. https://arxiv.org/abs/2502.15539.
...
struct cache_line_elias_fano {
  ...
  void encode(Iterator begin, uint64_t n, uint64_t universe = uint64_t(-1)) {
    ...
    for (uint64_t i = 0; i != n; ++i, ++begin) {
      ...
      if (word >= 2)
        throw std::runtime_error("the high part must fit within 128 bits");
  ...
  uint64_t access(uint64_t i) const {
    ...
    uint64_t lower_bound_high_part =
        *reinterpret_cast<uint32_t const *>(&m_bits[block_begin]);
    uint64_t W1 = *reinterpret_cast<uint64_t const *>(&m_bits[block_begin + 4]);
    uint64_t W2 =
        *reinterpret_cast<uint64_t const *>(&m_bits[block_begin + 4 + 8]);
    uint64_t high = 0;
