...
const Byte PPMD8_kExpEscape[16] = {25, 14, 9, 7, 5, 5, 4, 4,
                                   4,  3,  3, 3, 2, 2, 2, 2};
...
static void SplitBlock(CPpmd8 *p, void *ptr, unsigned oldIndx,
                       unsigned newIndx) {
...
static void GlueFreeBlocks(CPpmd8 *p) {
  ...
  /* Order-0 context is always at top UNIT, so we don't need guard NODE at the
     end. All blocks up to p->LoUnit can be free, so we need guard NODE at
     LoUnit. */
...
static void *ShrinkUnits(CPpmd8 *p, void *oldPtr, unsigned oldNU,
                         unsigned newNU) {
...
static void Refresh(CPpmd8 *p, CTX_PTR ctx, unsigned oldNU, unsigned scale) {
  ...
  CPpmd_State *s =
      (CPpmd_State *)ShrinkUnits(p, STATS(ctx), oldNU, (i + 2) >> 1);
  ctx->Stats = REF(s);
#ifdef PPMD8_FREEZE_SUPPORT
  /* fixed over Shkarin's code. Fixed code is not compatible with original code
   * for some files in FREEZE mode. */
...
static CPpmd_Void_Ref CutOff(CPpmd8 *p, CTX_PTR ctx, unsigned order) {
  ...
  ctx->Stats = STATS_REF(
      MoveUnitsUp(p, STATS(ctx), tmp = ((unsigned)ctx->NumStats + 2) >> 1));
...
#ifdef PPMD8_FREEZE_SUPPORT
static CPpmd_Void_Ref RemoveBinContexts(CPpmd8 *p, CTX_PTR ctx,
                                        unsigned order) {
  CPpmd_State *s;
  if (!ctx->NumStats) {
    s = ONE_STATE(ctx);
    ...
    /* Suffix context can be removed already, since different (high-order)
       Successors may refer to same context. So we check Flags == 0xFF (Stamp ==
       EMPTY_NODE) */
    if (!SUCCESSOR(s) &&
        (!SUFFIX(ctx)->NumStats || SUFFIX(ctx)->Flags == 0xFF)) {
      FreeUnits(p, ctx, 1);
      return 0;
    } else
      return REF(ctx);
...
static UInt32 GetUsedMemory(const CPpmd8 *p) {
  ...
  for (i = 0; i < PPMD_NUM_INDEXES; i++)
    v += p->Stamps[i] * I2U(i);
  return p->Size - (UInt32)(p->HiUnit - p->LoUnit) -
         (UInt32)(p->UnitsStart - p->Text) - U2B(v);
}
...
static void RestoreModel(CPpmd8 *p, CTX_PTR c1
  ...
  for (; c != p->MinContext; c = SUFFIX(c))
    if (!c->NumStats)
      ONE_STATE(c)->Freq =
          (Byte)(ONE_STATE(c)->Freq - (ONE_STATE(c)->Freq >> 1));
...
static CTX_PTR CreateSuccessors(CPpmd8 *p, Bool skip, CPpmd_State *s1,
                                CTX_PTR c) {
  CPpmd_State upState;
  Byte flags;
  ...
  flags = (Byte)(0x10 * (p->FoundState->Symbol >= 0x40) +
                 0x08 * (upState.Symbol >= 0x40));
  ...
  if (c->NumStats == 0)
    ...
    upState.Freq =
        (Byte)(1 + ((2 * cf <= s0) ? (5 * cf > s0) : ((cf + 2 * s0 - 3) / s0)));
  }
...
static void Rescale(CPpmd8 *p) {
  ...
  if (s->Freq == 0) {
    ...
    if (p->MinContext->NumStats == 0) {
      ...
      p->MinContext->Flags =
          (Byte)((p->MinContext->Flags & 0x10) + 0x08 * (tmp.Symbol >= 0x40));
...
CPpmd_See *Ppmd8_MakeEscFreq(CPpmd8 *p, unsigned numMasked1, UInt32 *escFreq) {
  ...
  if (p->MinContext->NumStats != 0xFF) {
    see = p->See[(unsigned)p->NS2Indx[(unsigned)p->MinContext->NumStats + 2] -
                 3] +
          (p->MinContext->SummFreq >
           11 * ((unsigned)p->MinContext->NumStats + 1)) +
          2 * (unsigned)(2 * (unsigned)p->MinContext->NumStats <
                         ((unsigned)SUFFIX(p->MinContext)->NumStats +
                          numMasked1)) +
          p->MinContext->Flags;
    {
...
void Ppmd8_UpdateBin(CPpmd8 *p) {
  p->FoundState->Freq =
      (Byte)(p->FoundState->Freq + (p->FoundState->Freq < 196));
  p->PrevSuccess = 1;
  p->RunLength++;
  NextContext(p);
}
...
int Ppmd8_DecodeSymbol(CPpmd8 *p) {
  ...
  if (p->MinContext->NumStats != 0) {
    ...
    unsigned i;
    UInt32 count, hiCnt;
    if ((count = RangeDec_GetThreshold(p, p->MinContext->SummFreq)) <
        (hiCnt = s->Freq)) {
      Byte symbol;
...
const IPpmd8 __archive_ppmd8_functions = {
    &Ppmd8_Construct, &Ppmd8_Alloc,         &Ppmd8_Free,
    &Ppmd8_Init,      &Ppmd8_RangeDec_Init, &Ppmd8_DecodeSymbol,
};
