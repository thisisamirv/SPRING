...
static const Byte PPMD7_kExpEscape[16] = {25, 14, 9, 7, 5, 5, 4, 4,
                                          4,  3,  3, 3, 2, 2, 2, 2};
...
static CPpmd_See *Ppmd7_MakeEscFreq(CPpmd7 *p, unsigned numMasked,
  ...
  if (p->MinContext->NumStats != 256) {
    see = p->See[p->NS2Indx[nonMasked - 1]] +
          (nonMasked < (unsigned)SUFFIX(p->MinContext)->NumStats -
                           p->MinContext->NumStats) +
          2 * (p->MinContext->SummFreq < 11 * p->MinContext->NumStats) +
          4 * (numMasked > nonMasked) + p->HiBitsFlag;
    {
...
const IPpmd7 __archive_ppmd7_functions = {&Ppmd7_Construct,
                                          &Ppmd7_Alloc,
                                          &Ppmd7_Free,
                                          &Ppmd7_Init,
