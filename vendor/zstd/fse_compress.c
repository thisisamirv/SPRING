...
size_t FSE_buildCTable_wksp(FSE_CTable *ct, const short *normalizedCounter,
  ...
  ZSTD_memset(tableSymbol, 0,
              sizeof(*tableSymbol) * tableSize); /* useless initialization, just
                                                    to keep scan-build happy */
#endif
  ...
  if (highThreshold == tableSize - 1) {
    ...
    BYTE *const spread =
        tableSymbol +
        tableSize; /* size = tableSize + 8 (may write beyond tableSize) */
    {
  ...
  /* Build table */
  {
    ...
    for (u = 0; u < tableSize; u++) {
      ...
      tableU16[cumul[s]++] =
          (U16)(tableSize + u); /* TableU16 : sorted by symbol order; gives next
                                   state value */
    }
  }
...
size_t FSE_normalizeCount(short *normalizedCounter, unsigned tableLog,
...

  {
    ...
    U64 const step =
        ZSTD_div64((U64)1 << 62, (U32)total); /* <== here, one division ! */
