  ...
  for (; ip + fastHashFillStep < iend + 2; ip += fastHashFillStep) {
    ...
    /* Only load extra positions for ZSTD_dtlm_full */
    {
      ...
      for (p = 1; p < fastHashFillStep; ++p) {
        ...
        if (hashTable[hashAndTag >> ZSTD_SHORT_CACHE_TAG_BITS] ==
            0) { /* not yet filled */
...
static int ZSTD_match4Found_cmov(const BYTE *currentPtr,
   ...
   * in which case it becomes critical that they are executed in the right order
   * (test1 then test2). So we have to write these tests in a specific manner to
   * ensure their ordering.
   */
  if (MEM_read32(currentPtr) != MEM_read32(mvalAddr))
    return 0;
  /* force ordering of these tests, which matters once the function is inlined,
   * as they become branches */
#if defined(__GNUC__)
  __asm__("");
#endif
...
static int ZSTD_match4Found_branch(const BYTE *currentPtr,
   ...
   * because it's faster in scenarios where matchIdx >= idxLowLimit is generally
   * true, aka almost all candidates are within range */
  U32 mval;
...
size_t ZSTD_compressBlock_fast_noDict_generic(ZSTD_MatchState_t *ms,
  ...
  size_t const stepSize =
      cParams->targetLength + !(cParams->targetLength) + 1; /* min 2 */
  const BYTE *const base = ms->window.base;
  ...
  do {
    ...
    if (matchFound(ip0, base + matchIdx, matchIdx, prefixStartIndex)) {
      ...
      if (step <= 4) {
         ...
         * The minimum possible match has length 4, so search can resume at ip0
         * + 4.
         */
        hashTable[hash1] = (U32)(ip1 - base);
      }
...
_cleanup:
  ...
  /* When the repcodes are outside of the prefix, we set them to zero before the
   * loop. When the offsets are still zero, we need to restore them after the
   * block to have a correct repcode history. If only one offset was invalid, it
   * is easy. The tricky case is when both offsets were invalid. We need to
   * figure out which offset to refill with.
   *     - If both offsets are zero they are in the same order.
   *     - If both offsets are non-zero, we won't restore the offsets from
   * `offsetSaved[12]`.
   *     - If only one is zero, we need to decide which offset to restore.
   *         - If rep_offset1 is non-zero, then rep_offset2 must be
   * offsetSaved1.
   *         - It is impossible for rep_offset2 to be non-zero.
   *
   * So if rep_offset1 started invalid (offsetSaved1 != 0) and became valid
   * (rep_offset1 != 0), then set rep[0] = rep_offset1 and rep[1] =
   * offsetSaved1.
   */
  offsetSaved2 =
...
size_t ZSTD_compressBlock_fast_dictMatchState_generic(
  ...
  assert(stepSize >= 1);
  while (ip1 <=
         ilimit) { /* repcode check at (ip0 + 1) is safe because ip0 < ip1 */
    size_t mLength;
    ...
    while (1) {
      ...
      if (dictTagsMatch) {
        ...
        if (dictMatchIndex > dictStartIndex &&
          ...
          /* To replicate extDict parse behavior, we only use dict matches when
           * the normal matchIndex is invalid */
