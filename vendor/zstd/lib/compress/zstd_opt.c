...
#define ZSTD_PREDEF_THRESHOLD                                                  \
  8 /* if srcSize < ZSTD_PREDEF_THRESHOLD, symbols' cost is assumed static,    \
...
static void ZSTD_rescaleFreqs(optState_t *const optPtr, const BYTE *const src,
  ...
  if (optPtr->litLengthSum ==
    ...
    if (optPtr->symbolCosts->huf.repeatMode == HUF_repeat_valid) {
      ...
      /* huffman stats covering the full value set : table presumed generated by
       * dictionary */
      optPtr->priceType = zop_dynamic;
...
static U32 ZSTD_litLengthPrice(U32 const litLength,
   ...
   * So instead just pretend it would cost 1 bit more than ZSTD_BLOCKSIZE_MAX
   * - 1. In such a case, the block would be all literals.
   */
...
FORCE_INLINE_TEMPLATE U32 ZSTD_getMatchPrice(U32 const offBase,
  ...
  if (optPtr->priceType ==
      zop_predef) /* fixed scheme, does not use statistics */
...
U32 ZSTD_insertBtAndGetAllMatches(
    ...
    const U32 ll0, /* tells if associated literal length is 0 or not. This value
                      must be 0 or 1 */
    ...
    for (repCode = ll0; repCode < lastR; repCode++) {
      ...
      if (repOffset - 1 /* intentional overflow, discards 0 and -1 */ <
        ...
        if (dictMode == ZSTD_dictMatchState &&
            (((repOffset - 1) /*intentional overflow*/ <
              curr -
                  (dmsLowLimit + dmsIndexDelta)) /* equivalent to `curr >
      ...
      if (repLen > bestLength) {
        ...
        bestLength = repLen;
        matches[mnum].off = REPCODE_TO_OFFBASE(
            repCode - ll0 + 1); /* expect value between 1 and 3 */
...
static void ZSTD_opt_getNextMatchAndUpdateSeqStore(ZSTD_optLdm_t *optLdm,
  ...
  /* Matches may be < minMatch by this process. In that case, we will reject
     them when we are deciding whether or not to add the ldm */
...
static void ZSTD_optLdm_maybeAddMatch(ZSTD_match_t *matches, U32 *nbMatches,
  ...
  /* Note: ZSTD_match_t actually contains offBase and matchLength (before
...
ZSTD_optLdm_processMatchCandidate(ZSTD_optLdm_t *optLdm, ZSTD_match_t *matches,
  ...
  if (currPosInBlock >= optLdm->endPosInBlock) {
    if (currPosInBlock > optLdm->endPosInBlock) {
       ...
       * not necessarily at the end of a match from the ldm seq store, and will
       * often be some bytes over beyond matchEndPosInBlock. As such, we need to
       * correct for these "overshoots"
       */
...
size_t ZSTD_compressBlock_opt_generic(ZSTD_MatchState_t *ms,
  ...
  while (ip < ilimit) {
    ...
    /* find first match */
    {
      ...
      /* Match found: let's store this solution, and eventually find more
       * candidates. During this forward pass, @opt is used to store stretches,
       * defined as "a match followed by N literals".
       * Note how this is different from a Sequence, which is "N literals
       * followed by a match". Storing stretches allows us to store different
       * match predecessors for each literal position part of a literals run. */

      /* initialize opt[0] */
      opt[0].mlen = 0; /* there are only literals so far */
      opt[0].litlen = litlen;
      /* No need to include the actual price of the literals before the first
       * match because it is static for the duration of the forward pass, and is
       * included in every subsequent price. But, we include the literal length
       * because the cost variation of litlen depends on the value of litlen.
       */
          ...
          for (mlen = lastML; mlen >= startML; mlen--) { /* scan downward */
            ...
            if ((pos > last_pos) || (price < opt[pos].price)) {
              ...
              while (last_pos < pos) {
                ...
                opt[last_pos].litlen = !0; /* just needs to be != 0, to mean
                                              "not an end of match" */
              }
              opt[pos].mlen = mlen;
              opt[pos].off = offset;
 ...
 * function cannot error out, its narrow contract must be respected.
 */
...
size_t ZSTD_compressBlock_btultra2(ZSTD_MatchState_t *ms, SeqStore_t *seqStore,
  ...
  if ((ms->opt.litLengthSum == 0)                          /* first block */
      ...
      && (curr == ms->window.dictLimit)    /* start of frame, nothing already
                                              loaded nor skipped */
      && (srcSize > ZSTD_PREDEF_THRESHOLD) /* input large enough to not employ
                                              default stats */
  ) {
