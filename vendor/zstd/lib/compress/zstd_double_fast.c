...
ZSTD_fillDoubleHashTableForCDict(ZSTD_MatchState_t *ms, void const *end,
...
static ZSTD_ALLOW_POINTER_OVERFLOW_ATTR void
ZSTD_fillDoubleHashTableForCCtx(ZSTD_MatchState_t *ms, void const *end,
                                ZSTD_dictTableLoadMethod_e dtlm) {
  const ZSTD_compressionParameters *const cParams = &ms->cParams;
  U32 *const hashLarge = ms->hashTable;
...
void ZSTD_fillDoubleHashTable(ZSTD_MatchState_t *ms, const void *const end,
                              ZSTD_dictTableLoadMethod_e dtlm,
...
size_t ZSTD_compressBlock_doubleFast_noDict_generic(
  ...
  const BYTE dummy[] = {0x12, 0x34, 0x56, 0x78, 0x9a,
                        0xbc, 0xde, 0xf0, 0xe2, 0xb4};
  ...
  while (1) {
    ...
    do {
      ...
      {
        const BYTE *const matchl0_safe =
            ZSTD_selectAddr(idxl0, prefixLowestIndex, matchl0, &dummy[0]);
      ...
      matchs0_safe =
          ZSTD_selectAddr(idxs0, prefixLowestIndex, matchs0, &dummy[0]);
  ...
  _cleanup:
     ...
     * (offset_1 != 0), rotate saved offsets. See comment in
     * ZSTD_compressBlock_fast_noDict for more context. */
    offsetSaved2 =
...
size_t ZSTD_compressBlock_doubleFast_dictMatchState_generic(
  ...
  if (ms->prefetchCDictTables) {
    ...
    size_t const chainTableBytes =
        (((size_t)1) << dictCParams->chainLog) * sizeof(U32);
  ...
  while (ip < ilimit) { /* < instead of <=, because repcode check at (ip+1) */
    ...
    U32 const dictMatchIndexAndTagL =
        dictHashLong[dictHashAndTagL >> ZSTD_SHORT_CACHE_TAG_BITS];
    U32 const dictMatchIndexAndTagS =
        dictHashSmall[dictHashAndTagS >> ZSTD_SHORT_CACHE_TAG_BITS];
    int const dictTagsMatchL =
        ZSTD_comparePackedTags(dictMatchIndexAndTagL, dictHashAndTagL);
    int const dictTagsMatchS =
        ZSTD_comparePackedTags(dictMatchIndexAndTagS, dictHashAndTagS);
    U32 const curr = (U32)(ip - base);
    ...
    if ((matchIndexL >= prefixLowestIndex) &&
      ...
      U32 const dictMatchIndexL =
          dictMatchIndexAndTagL >> ZSTD_SHORT_CACHE_TAG_BITS;
  ...
  _search_next_long: {
    ...
    U32 const dictMatchIndexAndTagL3 =
        dictHashLong[dictHashAndTagL3 >> ZSTD_SHORT_CACHE_TAG_BITS];
    int const dictTagsMatchL3 =
        ZSTD_comparePackedTags(dictMatchIndexAndTagL3, dictHashAndTagL3);
...
size_t ZSTD_compressBlock_doubleFast(ZSTD_MatchState_t *ms,
  ...
  case 4:
    return ZSTD_compressBlock_doubleFast_noDict_4(ms, seqStore, rep, src,
                                                  srcSize);
  case 5:
    return ZSTD_compressBlock_doubleFast_noDict_5(ms, seqStore, rep, src,
                                                  srcSize);
  case 6:
    return ZSTD_compressBlock_doubleFast_noDict_6(ms, seqStore, rep, src,
                                                  srcSize);
  case 7:
    return ZSTD_compressBlock_doubleFast_noDict_7(ms, seqStore, rep, src,
                                                  srcSize);
  }
}
...
size_t ZSTD_compressBlock_doubleFast_dictMatchState(ZSTD_MatchState_t *ms,
  ...
  default: /* includes case 3 */
  case 4:
    return ZSTD_compressBlock_doubleFast_dictMatchState_4(ms, seqStore, rep,
                                                          src, srcSize);
  case 5:
    return ZSTD_compressBlock_doubleFast_dictMatchState_5(ms, seqStore, rep,
                                                          src, srcSize);
  case 6:
    return ZSTD_compressBlock_doubleFast_dictMatchState_6(ms, seqStore, rep,
                                                          src, srcSize);
  case 7:
    return ZSTD_compressBlock_doubleFast_dictMatchState_7(ms, seqStore, rep,
                                                          src, srcSize);
  }
}
...
ZSTD_compressBlock_doubleFast_extDict_generic(ZSTD_MatchState_t *ms,
  ...
  while (ip < ilimit) { /* < instead of <=, because (ip+1) */
    ...
    if (((ZSTD_index_overlap_check(prefixStartIndex, repIndex)) &
         (offset_1 <=
          curr + 1 - dictStartIndex)) /* note: we are searching at curr+1 */
        && (MEM_read32(repMatch) == MEM_read32(ip + 1))) {
...
size_t ZSTD_compressBlock_doubleFast_extDict(ZSTD_MatchState_t *ms,
  ...
  default: /* includes case 3 */
  case 4:
    return ZSTD_compressBlock_doubleFast_extDict_4(ms, seqStore, rep, src,
                                                   srcSize);
  case 5:
    return ZSTD_compressBlock_doubleFast_extDict_5(ms, seqStore, rep, src,
                                                   srcSize);
  case 6:
    return ZSTD_compressBlock_doubleFast_extDict_6(ms, seqStore, rep, src,
                                                   srcSize);
  case 7:
    return ZSTD_compressBlock_doubleFast_extDict_7(ms, seqStore, rep, src,
                                                   srcSize);
  }
}
