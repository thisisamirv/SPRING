 ...
 * literals.
 *
 *  We write the header when writeEntropy=1 and set entropyWritten=1 when we
...
static size_t sizeBlockSequences(const SeqDef *sp, size_t nbSeqs,
  ...
  size_t const headerSize =
      (size_t)firstSubBlock * 120 * BYTESCALE; /* generous estimate */
 ...
 *  @return : compressed size of the super block (which features multiple ZSTD
 * blocks) or 0 if it failed to compress. */
static size_t ZSTD_compressSubBlock_multi(
  ...
  const SeqDef *sp =
      sstart; /* tracks progresses within seqStorePtr->sequences */
  ...
  size_t const minTarget =
      ZSTD_TARGETCBLOCKSIZE_MIN; /* enforce minimum size, to reduce undesirable
                                    side effects */
  ...
  if (nbSeqs > 0) {
    ...
    /* simplification: if estimates states that the full superblock doesn't
     * compress, just bail out immediately this will result in the production of
     * a single uncompressed block covering @srcSize.*/
    ...
    for (n = 0; n < nbSubBlocks - 1; n++) {
      /* determine nb of sequences for current sub-block + nbLiterals from next
       * sequence */
      ...
      /* otherwise : do not compress yet, coalesce current sub-block with
       * following one */
    }
  } /* if (nbSeqs > 0) */
           ...
           (unsigned)(send - sp));
  {
    ...
    /* update pointers, the nb of literals borrowed from next sequence must be
     * preserved */
    if (cSize > 0 && cSize < decompressedSize) {
...
size_t ZSTD_compressSuperBlock(ZSTD_CCtx *zc, void *dst, size_t dstCapacity,
  ...
  return ZSTD_compressSubBlock_multi(
      ...
      zc->bmi2, lastBlock, zc->tmpWorkspace,
      zc->tmpWkspSize /* statically allocated in resetCCtx */);
}
