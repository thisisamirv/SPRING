 ...
 * decompression, the literals block will be stored in the dstBuffer.  If there
 * is room to do so, it will be stored in full in the excess dst space after
 * where the current block will be output.  Otherwise it will be stored at the
 * end of the current dst blockspace, with a small portion being stored in
 * dctx->litExtraBuffer to help keep it "ahead" of the current output write.
 *
 * @return : nb of bytes read from src (< srcSize )
 *  note : symbol not declared but exposed for fullbench */
static size_t
ZSTD_decodeLiteralsBlock(ZSTD_DCtx *dctx, const void *src,
 ...
 * case with dst after src Kept separate from more common ZSTD_safecopy case to
 * avoid performance impact to the safecopy common case */
...
/**
 * ZSTD_decodeSequence():
 * @p longOffsets : tells the decoder to reload more bit while decoding large
 * offsets only used in 32-bit mode
      ...
      if (ofBits > 1) {
        ...
        if (LIKELY((ofBits == 0))) {
          ...
          offset = ofBase + ll0 + BIT_readBitsFast(&seqState->DStream, 1);
          {
            ...
            temp -= !temp; /* 0 is not valid: input corrupted => force offset to
                              -1 => corruption detected at execSequence */
            if (offset != 1)
              seqState->prevOffset[2] = seqState->prevOffset[1];
            seqState->prevOffset[1] = seqState->prevOffset[0];
...
size_t DONT_VECTORIZE ZSTD_decompressSequences_bodySplitLitBuffer(
  ...
  if (nbSeq) {
    ...
    {
      seq_t sequence = {0, 0,
                        0}; /* some static analyzer believe that @sequence is
                               not initialized (it necessarily is, since for(;;)
                               loop as at least one iteration) */
...
size_t ZSTD_prefetchMatch(size_t prefetchPos, seq_t const sequence,
  ...
  prefetchPos += sequence.litLength;
  {
     ...
     * which can only happen when input is corrupted. No consequence though :
     * memory address is only used for prefetching, not for dereferencing */
    ...
    PREFETCH_L1(match);
    PREFETCH_L1(ZSTD_wrappedPtrAdd(
        match, CACHELINE_SIZE)); /* note : it's safe to invoke PREFETCH() on any
 ...
 * long-distance matches or when coupled with a "cold" dictionary */
...
#define ADVANCED_SEQS STORED_SEQS
  ...
  if (dctx->litBufferLocation ==
      ZSTD_split) { /* first deplete literal buffer in dst, then copy
                       litExtraBuffer */
    size_t const lastLLSize = (size_t)(litBufferEnd - litPtr);
 ...
 * without reloading more bits in the middle of the offset bits read. Any
 * offsets larger than this must use the long offset decoder.
 */
...
size_t ZSTD_decompressBlock_internal(
   ...
   * since an uncompressed block would feature same size and have no
   * decompression cost. Also, note that decoder from reference libzstd before <
   * v1.5.4 would consider this edge case as an error. As a consequence, avoid
   * generating compressed blocks of size ZSTD_blockSizeMax(dctx) for broader
   * compatibility with the deployed ecosystem of zstd decoders */
  ...
  {
    /* Compute the maximum block size, which must also work when !frame and
     * fParams are unset. Additionally, take the min with dstCapacity to ensure
     * that the totalHistorySize fits in a size_t.
     */
     ...
     *
     * We check here to see if our history is large enough to allow long
     * offsets. If it isn't, then we can't possible have (valid) long offsets.
     * If the offset is invalid, then it is okay to read it incorrectly.
     *
     * If isLongOffsets is true, then we will later check our decoding table to
     * see if it is even possible to generate long offsets.
     */
    ...
    /* If we could potentially have long offsets, or we might want to use the
     * prefetch decoder, compute information about the share of long offsets,
     * and the maximum nbAdditionalBits. NOTE: could probably use a larger nbSeq
     * limit
     */
    if (isLongOffset || (!usePrefetchDecoder &&
      ...
      if (isLongOffset && info.maxNbAdditionalBits <= STREAM_ACCUMULATOR_MIN) {
        /* If isLongOffset, but the maximum number of additional bits that we
         * see in our table is small enough, then we know it is impossible to
         * have too long an offset in this block, so we can use the regular
         * offset decoder.
         */
