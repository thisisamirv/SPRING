...
FORCE_INLINE_TEMPLATE void addEvents_generic(Fingerprint *fp, const void *src,
                                             size_t srcSize,
                                             size_t samplingRate,
                                             unsigned hashLog) {
...
typedef void (*RecordEvents_f)(Fingerprint *fp, const void *src,
                               size_t srcSize);

#define FP_RECORD(_rate) ZSTD_recordFingerprint_##_rate

#define ZSTD_GEN_RECORD_FINGERPRINT(_rate, _hSize)                             \
  static void FP_RECORD(_rate)(Fingerprint * fp, const void *src,              \
                               size_t srcSize) {                               \
...
static U64 fpDistance(const Fingerprint *fp1, const Fingerprint *fp2,
  ...
  for (n = 0; n < ((size_t)1 << hashLog); n++) {
    distance += abs64((S64)fp1->events[n] * (S64)fp2->nbEvents -
                      (S64)fp2->events[n] * (S64)fp1->nbEvents);
  }
  return distance;
}
...
static int compareFingerprints(const Fingerprint *ref, const Fingerprint *newfp,
  ...
  assert(newfp->nbEvents > 0);
  {
    ...
    U64 threshold =
        p50 * (U64)(THRESHOLD_BASE + penalty) / THRESHOLD_PENALTY_RATE;
...
static size_t ZSTD_splitBlock_byChunks(const void *blockStart, size_t blockSize,
                                       int level, void *workspace,
                                       size_t wkspSize) {
  static const RecordEvents_f records_fs[] = {FP_RECORD(43), FP_RECORD(11),
                                              FP_RECORD(5), FP_RECORD(1)};
  static const unsigned hashParams[] = {8, 9, 10, 10};
  const RecordEvents_f record_f =
      (assert(0 <= level && level <= 3), records_fs[level]);
...
#define SEGMENT_SIZE 512
  FPStats *const fpstats = (FPStats *)workspace;
  Fingerprint *middleEvents =
      (Fingerprint *)(void *)((char *)workspace + 512 * sizeof(unsigned));
  assert(blockSize == (128 << 10));
  assert(workspace != NULL);
  assert((size_t)workspace % ZSTD_ALIGNOF(FPStats) == 0);
  ZSTD_STATIC_ASSERT(ZSTD_SLIPBLOCK_WORKSPACESIZE >= sizeof(FPStats));
  ...
  HIST_add(fpstats->newEvents.events,
           (const char *)blockStart + blockSize - SEGMENT_SIZE, SEGMENT_SIZE);
  ...
  HIST_add(middleEvents->events,
           (const char *)blockStart + blockSize / 2 - SEGMENT_SIZE / 2,
           SEGMENT_SIZE);
