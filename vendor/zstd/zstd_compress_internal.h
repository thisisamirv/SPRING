...
struct ZSTD_MatchState_t {
  ...
  U32 loadedDictEnd;    /* index of end of dictionary, within context's
                         ...
                         * mechanism    to set    loadedDictEnd=0 when dictionary
                         * is    no longer    within    distance.    Such mechanism is
  ...
  int forceNonContiguous; /* Non-zero if we should force non-contiguous load for
...
struct ZSTD_CCtx_params_s {
   ...
   * then levels are sorted in increasing cpu budget, from 2 (fastest) to 6
   * (slowest). Highest @preBlockSplitter_level combines well with
   * @postBlockSplitter.
   */
...
#define TMP_WORKSPACE_SIZE                                                     \
  (MAX(ENTROPY_WORKSPACE_SIZE, ZSTD_SLIPBLOCK_WORKSPACESIZE))
...
struct ZSTD_CCtx_s {
  ...
  void *tmpWorkspace; /* used as substitute of stack space - must be aligned for
                         S64 type */
  size_t tmpWkspSize;
...
MEM_STATIC const BYTE *ZSTD_selectAddr(U32 index, U32 lowLimit,
...
ZSTD_storeSeq(SeqStore_t *seqStorePtr, size_t litLength, const BYTE *literals,
              const BYTE *litLimit, U32 offBase, size_t matchLength) {
  ...
  if (litEnd <= litLimit_w) {
    ...
    if (litLength > 16) {
      ZSTD_wildcopy(seqStorePtr->lit + 16, literals + 16, litLength - 16,
                    ZSTD_no_overlap);
    }
  } else {
...
MEM_STATIC size_t ZSTD_count_2segments(const BYTE *ip, const BYTE *match,
  ...
  DEBUGLOG(7,
           ...
           matchLength);
  DEBUGLOG(7, "distance from match beginning to end dictionary = %i",
           (int)(mEnd - match));
 ...
 *                 reducing risks of side effects in case of signed operations
 * on indexes. In 64-bit mode: we want to ensure that adding the maximum job
 * size (512 MB) doesn't overflow U32 index capacity (4 GB) */
...
MEM_STATIC
ZSTD_ALLOW_POINTER_OVERFLOW_ATTR
U32 ZSTD_window_update(ZSTD_window_t *window, const void *src, size_t srcSize,
  ...
  if ((ip + srcSize > window->dictBase + window->lowLimit) &
    ...
    U32 const lowLimitMax = (highInputIdx > (size_t)window->dictLimit)
                                ? window->dictLimit
                                : (U32)highInputIdx;
 ...
 * 0 otherwise */
MEM_STATIC int ZSTD_index_overlap_check(const U32 prefixLowestIndex,
                                        const U32 repIndex) {
