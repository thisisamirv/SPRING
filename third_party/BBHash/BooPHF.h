...
namespace boomphf {
	...
	T* checked_malloc(size_t count) {
		T* pointer = static_cast<T*>(malloc(count * sizeof(T)));
		if (!pointer) {
			throw std::bad_alloc();
		}
		return pointer;
	...
	template <typename T>
	T* checked_realloc(T* pointer, size_t count) {
		T* resized_pointer = static_cast<T*>(realloc(pointer, count * sizeof(T)));
		if (!resized_pointer) {
			throw std::bad_alloc();
		}
...
////////////////////////////////////////////////////////////////
	...
	public:
		...
		bfile_iterator()
		: _is(nullptr)
		, _pos(0), _buffer(nullptr), _inbuff(0), _cptread(0), _buffsize(10000)
		{
			_buffer = checked_malloc<basetype>(_buffsize);
		}
		
		bfile_iterator(const bfile_iterator& cr)
		: _elem(cr._elem), _is(cr._is), _pos(cr._pos),
		  _buffer(checked_malloc<basetype>(cr._buffsize)), _inbuff(cr._inbuff),
		  _cptread(cr._cptread), _buffsize(cr._buffsize)
		{
			memcpy(_buffer,cr._buffer,_buffsize*sizeof(basetype) );
		}
		
		bfile_iterator(FILE* is)
		: _is(is), _pos(0), _buffer(nullptr), _inbuff(0), _cptread(0),
		  _buffsize(10000)
		{
			//printf("bf it %p\n",_is);
			_buffer = checked_malloc<basetype>(_buffsize);
	...
	public:
		...
		Progress ()
		: timer_mode(0), timestamp(), heure_debut(0), heure_actuelle(0),
		  message(), done(0), todo(0), subdiv(0), partial(0), _nthreads(0),
		  partial_threaded(), done_threaded(), steps(0) {}
...
////////////////////////////////////////////////////////////////
	...
	public:
		...
		bitVector() : _bitArray(nullptr), _size(0), _nchar(0)
		{
		}

		bitVector(uint64_t n) : _bitArray(nullptr), _size(n), _nchar(1ULL+n/64ULL)
		{
			_bitArray = checked_calloc<uint64_t>(_nchar);
		}
		...
		~bitVector()
		{
		 ...
		 : _bitArray(checked_calloc<uint64_t>(r._nchar)), _size(r._size),
		   _nchar(r._nchar), _ranks(r._ranks)
		 {
		...
		void resize(uint64_t newsize)
		{
			...
			const uint64_t new_nchar = (1ULL+newsize/64ULL);
			_bitArray = checked_realloc<uint64_t>(_bitArray, new_nchar);
			_nchar = new_nchar;
			_size = newsize;
		}
...
////////////////////////////////////////////////////////////////
	...
	class level{
	public:
		level() : idx_begin(0), hash_domain(0), bitset() {}
...
//#define NBBUFF 2
	...
	public:
		mphf()
		: _levels(), _nb_levels(0), _hasher(), _tempBitset(nullptr), _gamma(0),
		  _hash_domain(0), _nelem(0), _final_hash(), _progressBar(),
		  _nb_living(0), _num_thread(0), _hashidx(0), _proba_collision(0),
		  _lastbitsetrank(0), _idxLevelsetLevelFastmode(0), _cptLevel(0),
		  _cptTotalProcessed(0), _percent_elem_loaded_for_fastMode(0),
		  _fastmode(false), setLevelFastmode(), bufferperThread(),
		  _fastModeLevel(0), _withprogress(false), _built(false),
		  _writeEachLevel(false), _currlevelFile(nullptr), _pid(0), _mutex()
		{}
		...
		_levels(), _nb_levels(0), _hasher(), _tempBitset(nullptr), _gamma(gamma), _hash_domain(size_t(ceil(double(n) * gamma))), _nelem(n), _final_hash(), _progressBar(), _nb_living(0), _num_thread(num_thread), _hashidx(0), _proba_collision(0), _lastbitsetrank(0), _idxLevelsetLevelFastmode(0), _cptLevel(0), _cptTotalProcessed(0), _percent_elem_loaded_for_fastMode (perc_elem_loaded), _fastmode(false), setLevelFastmode(), bufferperThread(), _fastModeLevel(0), _withprogress(progress), _built(false), _writeEachLevel(false), _currlevelFile(nullptr), _pid(0), _mutex()
		{
			if(n ==0) return;
...
//				return minimal_hp;
		...
		uint64_t totalBitSize()
		{
			...
			const size_t final_hash_bits = _final_hash.size() * size_t(42) * size_t(8);
			printf("Last level hash  %12zu  bits (%.2f %%) (nb in last level hash %zu)\n",
				   final_hash_bits, 100*(float)(final_hash_bits)/totalsize,
				   _final_hash.size() );
			return totalsize;
		}
