...
int compress_id(Arithmetic_stream as, id_models models, char *id, char *prev_ID,
  ...
  // set rest of prev_tokens_ptr to 0. Otherwise we have bugs due to garbage
  // value.
...
int decompress_id(Arithmetic_stream as, id_models model, char *id,
  ...
  while ((tok = uint8t2token(
    ...
    case ID_DIGIT:
      digit_value = 0;
      digit_value |=
          ((decompress_uint8t(as, model->integer[(token_ctr << 2) | 0]) & 0xff)
           << 0);
      digit_value |=
          ((decompress_uint8t(as, model->integer[(token_ctr << 2) | 1]) & 0xff)
           << 8);
      digit_value |=
          ((decompress_uint8t(as, model->integer[(token_ctr << 2) | 2]) & 0xff)
           << 16);
      digit_value |=
          ((decompress_uint8t(as, model->integer[(token_ctr << 2) | 3]) & 0xff)
           << 24);
