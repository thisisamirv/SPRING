...
// Progress reporting helper used during MPHF construction.
class Progress {
public:
  int timer_mode;
  ...
  uint64_t done;
  uint64_t todo;
  int subdiv; // Emit a progress update every 1/subdiv of the total work.
  double partial;
  int _nthreads;
  ...
  void inc(uint64_t completed_tasks) {
    ...
    while (partial >= steps) {
      if (timer_mode) {
        ...
        fprintf(stderr,
                "%c[%s]  %-5.3g%%   elapsed: %3i min %-2.0f sec   remaining: "
                "%3i min %-2.0f sec",
                13, message.c_str(), 100 * (double)done / todo, min_e, elapsed,
                min_r, rem);
    ...
    while (partial_threaded[thread_id] >= steps) {
      if (timer_mode) {
        ...
        fprintf(stderr,
                "%c[%s]  %-5.3g%%   elapsed: %3i min %-2.0f sec   remaining: "
                "%3i min %-2.0f sec",
                13, message.c_str(), 100 * (double)total_done / todo, min_e,
                elapsed, min_r, rem);
      } else {
        fprintf(stderr, "-");
        fflush(stderr);
...
input.
we need this 2-functors scheme because HashFunctors won't work with
unordered_map.
(rayan)
*/

// Wrapper around HashFunctors that exposes a single seeded hash.
...
public:
  ...
  // Return the next hash and update the xorshift state.
...
template <typename Range, typename Iterator> struct thread_args {
  void *boophf;
  Range const *range;
  std::shared_ptr<void> it_p;    /* used to be "Iterator it" but because of
                                    fastmode, iterator is polymorphic; TODO: think
                                    about whether it should be a unique_ptr
                                    actually */
...
public:
  ...
  // Load a small fraction of elements into RAM for faster later levels.
  ...
  mphf(size_t n, Range const &input_range, int num_thread = 1,
    ...
    if (_withprogress) {
      ...
      printf("for info, total work write each  : %.3f    total work inram from "
             "level %i : %.3f  total work raw : %.3f \n",
    ...
    for (int ii = 0; ii < _nb_levels; ii++) {
      _tempBitset = new bitVector(
          _levels[ii].hash_domain); // Tracks collisions while this level fills.
  ...
  uint64_t totalBitSize() {
    ...
    uint64_t totalsize =
        totalsizeBitset +
        _final_hash.size() * 42 * 8; // unordered map takes approx 42B per elem
  ...
  void load(std::istream &is) {
    ...
    // Reconstruct the derived per-level sizing metadata.
    _proba_collision =
...
private:
  void setup() {
    ...
    for (int ii = 0; ii < _nb_levels; ii++) {
      ...
      // Round up to a whole number of 64-bit words for faster clearing.
      _levels[ii].hash_domain =
          (((uint64_t)(_hash_domain * pow(_proba_collision, ii)) + 63) / 64) *
          64;
      if (_levels[ii].hash_domain == 0)
  ...
  template <typename Range> void processLevel(Range const &input_range, int i) {
    ...
    if (_writeEachLevel && (i > 1)) {
      ...
      for (int ii = 0; ii < _num_thread; ii++)
        pthread_create(
          ...
          // Join before the iterator-owning file wrapper goes out of scope.
      for (int ii = 0; ii < _num_thread; ii++) {
      ...
      if (_fastmode && i >= (_fastModeLevel + 1)) {
        /* Fast mode swaps to the in-memory iterator type used for later
           levels. */
...
private:
  ...
  // of the keys to fit in RAM for the later levels.
