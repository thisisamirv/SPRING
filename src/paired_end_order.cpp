...
void load_reordered_positions(const std::string &read_order_path,
  ...
  if (!order_input.is_open()) {
    SPRING_LOG_DEBUG(
        "block_id=" + block_id + ", pe_encode read-order open failure: path=" +
        read_order_path + ", expected_bytes=1, actual_bytes=0, index=0");
    throw std::runtime_error("Failed to open paired-end read order file.");
  }

  std::error_code file_ec;
  const uint64_t actual_bytes =
      std::filesystem::file_size(read_order_path, file_ec);
  const uint64_t expected_bytes =
      static_cast<uint64_t>(reordered_positions.size()) * sizeof(uint32_t);
  if (file_ec || actual_bytes < expected_bytes) {
    SPRING_LOG_DEBUG(
        "block_id=" + block_id + ", pe_encode read-order size mismatch: path=" +
        read_order_path + ", expected_bytes=" + std::to_string(expected_bytes) +
  ...
  for (uint32_t read_index = 0; read_index < reordered_positions.size();
    ...
    if (!order_input.read(byte_ptr(&current_order), sizeof(uint32_t))) {
      SPRING_LOG_DEBUG(
          "block_id=" + block_id +
          ", pe_encode read-order short read: path=" + read_order_path +
    ...
    if (current_order >= reordered_positions.size()) {
      SPRING_LOG_DEBUG(
          "block_id=" + block_id +
          ", pe_encode read-order out-of-range value: path=" + read_order_path +
          ", expected_bytes=" + std::to_string(reordered_positions.size()) +
    ...
    if (read_index_by_order[current_order] != UINT32_MAX) {
      SPRING_LOG_DEBUG(
          "block_id=" + block_id +
          ", pe_encode read-order duplicate value: path=" + read_order_path +
          ", expected_bytes=" + std::to_string(reordered_positions.size()) +
          ", actual_bytes=" + std::to_string(current_order) +
          ", index=" + std::to_string(read_index));
...
void persist_reordered_positions(
  ...
  if (!order_output.is_open()) {
    SPRING_LOG_DEBUG(
        "block_id=" + block_id + ", pe_encode write open failure: path=" +
        temporary_output_path + ", expected_bytes=1, actual_bytes=0, index=0");
  ...
  for (uint32_t read_index = 0; read_index < reordered_positions.size();
    ...
    if (!order_output.good()) {
      SPRING_LOG_DEBUG(
          ...
          ", expected_bytes=" + std::to_string(sizeof(uint32_t)) +
          ", actual_bytes=0, index=" + std::to_string(read_index));
  ...
  if (file_ec || actual_bytes != expected_bytes) {
    SPRING_LOG_DEBUG(
        "block_id=" + block_id +
        ", pe_encode write size mismatch: path=" + temporary_output_path +
        ", expected_bytes=" + std::to_string(expected_bytes) +
        ", actual_bytes=" + std::to_string(actual_bytes));
    throw std::runtime_error(
        "Paired-end order write produced unexpected size.");
  }
...
void pe_encode(const std::string &temp_dir, const compression_params &cp) {
  ...
  for (uint32_t order = 0; order < num_reads; order++) {
    if (read_index_by_order[order] == UINT32_MAX) {
      SPRING_LOG_DEBUG(
          "block_id=" + block_id + ", pe_encode missing order entry: path=" +
          read_order_path + ", expected_bytes=" + std::to_string(num_reads) +
